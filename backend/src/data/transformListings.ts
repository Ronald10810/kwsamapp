  ...
  await runInTransaction(async (client) => {
    await client.query(`
      ...
      SELECT DISTINCT ON (source_listing_id)
        ...
        CONCAT_WS(
          ' ',
          COALESCE(street_number, NULLIF(TRIM(COALESCE(raw_payload->>'StreetNumber', raw_payload->>'street_number', '')), '')),
          COALESCE(street_name, NULLIF(TRIM(COALESCE(raw_payload->>'StreetName', raw_payload->>'street_name', '')), ''))
        ) AS address_line,
        NULLIF(TRIM(COALESCE(raw_payload->>'ErfNumber', raw_payload->>'erf_number', '')), '') AS erf_number,
        NULLIF(TRIM(COALESCE(raw_payload->>'UnitNumber', raw_payload->>'unit_number', '')), '') AS unit_number,
        NULLIF(TRIM(COALESCE(raw_payload->>'DoorNumber', raw_payload->>'door_number', '')), '') AS door_number,
        NULLIF(TRIM(COALESCE(raw_payload->>'EstateName', raw_payload->>'estate_name', '')), '') AS estate_name,
        COALESCE(street_number, NULLIF(TRIM(COALESCE(raw_payload->>'StreetNumber', raw_payload->>'street_number', '')), '')) AS street_number,
        COALESCE(street_name, NULLIF(TRIM(COALESCE(raw_payload->>'StreetName', raw_payload->>'street_name', '')), '')) AS street_name,
        NULLIF(TRIM(COALESCE(raw_payload->>'PostalCode', raw_payload->>'postal_code', '')), '') AS postal_code,
        suburb,
        city,
        province,
        country,
        NULLIF(regexp_replace(COALESCE(raw_payload->>'Longitude', raw_payload->>'longitude', ''), '[^0-9.\-]', '', 'g'), '')::numeric(10, 7) AS longitude,
        NULLIF(regexp_replace(COALESCE(raw_payload->>'Latitude', raw_payload->>'latitude', ''), '[^0-9.\-]', '', 'g'), '')::numeric(10, 7) AS latitude,
        price,
        expiry_date,
        COALESCE(
          property_title,
