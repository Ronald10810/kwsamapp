...
async function scalar(clientQuery: (sql: string) => Promise<{ rows: Array<{ value: string }> }>, sql: string): Promise<number> {
...
async function main(): Promise<void> {
  await withClient(async (client) => {
    ...
    const duplicateAssociates = await scalar(
      query,
      `
    ...
    const duplicateListings = await scalar(
      query,
      `
      SELECT COUNT(*)::text AS value
      FROM (
    ...
    const duplicateMarketCenters = await scalar(
      query,
      `
      SELECT COUNT(*)::text AS value
      FROM (
    ...
    const duplicateTeams = await scalar(
      query,
      `
      SELECT COUNT(*)::text AS value
      FROM (
    ...
    const missingAssociateNames = await scalar(
      query,
      `
      SELECT COUNT(*)::text AS value
    ...
    const missingListingNumbers = await scalar(
      query,
      `
      SELECT COUNT(*)::text AS value
