...
type LegacyIdRow = { legacy_id: string };
...
async function getTargetLegacyIds(batchPrefix?: string): Promise<{
  listingIds: string[];
  associateIds: string[];
  ...
  return withClient(async (client) => {
    ...
    const listings = await client.query<LegacyIdRow>(
      `SELECT m.legacy_listing_id AS legacy_id FROM migration.id_map_legacy_listings m ${liFilter.clause}`,
      liFilter.params
    );

    const associates = await client.query<LegacyIdRow>(
      `SELECT m.legacy_associate_id AS legacy_id FROM migration.id_map_legacy_associates m ${asFilter.clause}`,
      asFilter.params
    );

    const marketCenters = await client.query<LegacyIdRow>(
      `SELECT m.legacy_market_center_id AS legacy_id FROM migration.id_map_legacy_market_centers m ${mcFilter.clause}`,
      mcFilter.params
    );
...
async function reportLegacyCounts(label: string): Promise<void> {
...
async function rollback(batchPrefix?: string): Promise<void> {
  ...
  await runInTransaction(async (client) => {
    ...
    if (ids.marketCenterIds.length > 0) {
      const deletableMc = await client.query<LegacyIdRow>(
        `
