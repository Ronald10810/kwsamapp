...
type KeyValueRow = { key: string; value: number };

type RejectionSummaryRow = {
  entity_name: string;
  reason: string;
  count: string;
};
...
async function scalar(
  query: (sql: string, params?: unknown[]) => Promise<{ rows: ScalarRow[] }>,
  sql: string,
  params: unknown[] = []
): Promise<number> {
...
async function main(): Promise<void> {
  ...
  await withClient(async (client) => {
    const query = async (sql: string, params: unknown[] = []) => {
      ...
      return { rows: result.rows as ScalarRow[] };
    };
    ...
    const rawCounts = [
      {
        ...
        value: await scalar(
          query,
          `SELECT COUNT(*)::text AS value FROM staging.market_centers_raw ${batchFilterClause}`,
          batchParams
        ),
      },
      {
      ...
      },
      {
        ...
        value: await scalar(
          query,
          `SELECT COUNT(*)::text AS value FROM staging.associates_raw ${batchFilterClause}`,
          batchParams
        ),
      },
      {
    ...
    const orphanTeams = await scalar(
      query,
      `
    ...
    const unresolvedAssociateMarketCenter = await scalar(
      query,
      `
      SELECT COUNT(*)::text AS value
    ...
    const unresolvedListingMarketCenter = await scalar(
      query,
      `
      SELECT COUNT(*)::text AS value
    ...
    const rejectionSummary = await client.query<RejectionSummaryRow>(
      `
    ...
    const rejectionSamples = await client.query<RejectionSampleRow>(
      `
