export interface AssociateEmailLookupClient {
  query<T>(text: string, values?: unknown[]): Promise<{ rowCount: number | null; rows: T[] }>;
}
...
export async function resolveAssociateIdForAuth(
  client: AssociateEmailLookupClient,
  email: string,
  ...
  const lookup = await client.query<ResolvedAssociateMatch>(
...
export async function isRegisteredAssociateEmail(
  client: AssociateEmailLookupClient,
  email: string,
  displayName?: string | null
): Promise<boolean> {
...
export async function getAssociateAccessState(
  client: AssociateEmailLookupClient,
  email: string,
  displayName?: string | null
): Promise<AssociateAccessState> {
  const normalizedEmail = normalizeAuthEmail(email);
  if (!normalizedEmail) {
    return {
      isRegistered: false,
  ...
  try {
    const suspension = await client.query<{
      ...
      `SELECT is_temporarily_suspended, suspended_reason
