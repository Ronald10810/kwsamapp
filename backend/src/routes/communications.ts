...
function runCommunicationsUpload(req: Request, res: Response): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    communicationsImageUpload.single('image')(req as any, res as any, (error?: unknown) => {
      if (error) {
        reject(error);
        return;
      }
...
async function ensureCommunicationsTables(): Promise<void> {
  if (communicationsTablesReady) return;
  await getRequiredPgPool().query(COMMUNICATIONS_TABLES_SQL);
  communicationsTablesReady = true;
}

function normalizeDraftStatus(raw: string | undefined): 'draft' | 'scheduled' {
...
function resolveUploadsPathFromSrc(src: string): string | null {
  if (!src) return null;
...
function inlineUploadsAsDataUris(html: string): string {
  if (!html.includes('<img')) {
    return html;
  }

  return html.replace(/(<img\b[^>]*\bsrc=["'])([^"']+)(["'][^>]*>)/gi, (_full, before: string, src: string, after: string) => {
...
async function resolveMergeValuesByEmail(email: string): Promise<Record<string, string>> {
  const normalized = email.trim().toLowerCase();
  const result = await getRequiredPgPool().query<{ full_name: string | null }>(
    `SELECT full_name
...
function applyMergeValues(template: string, values: Record<string, string>): string {
...
async function resolveAudienceEmails(
  filterJson: Record<string, unknown> | null | undefined,
  ...
  const result = await getRequiredPgPool().query<{ email: string }>(
    `SELECT DISTINCT LOWER(TRIM(COALESCE(a.kwsa_email, a.private_email, a.email))) AS email
      ...
      LIMIT $4`,
    [query, marketCenterIds, roles, limit],
  );
...
async function sendDraftToRecipients(input: {
  ...
  htmlContent: string;
  recipients: string[];
  eventTypePrefix: 'test-send' | 'scheduled-send' | 'send-now';
  ...
  for (const recipient of input.recipients) {
    try {
      ...
      await getRequiredPgPool().query(
        `INSERT INTO app.communications_events
...
async function processScheduledDrafts(): Promise<void> {
  ...
  try {
    ...
    const dueDrafts = await getRequiredPgPool().query<{
      id: string;
      ...
      `SELECT id::text, template_id::text, subject, html_content, audience_filter_json, updated_by_email
    ...
    for (const draft of dueDrafts.rows) {
      ...
      const selected = normalizeEmails((filterJson as Record<string, unknown>).selectedEmails);
      const manual = normalizeEmails((filterJson as Record<string, unknown>).manualRecipients);
      ...
      if (recipients.length === 0) {
        await getRequiredPgPool().query(
          `INSERT INTO app.communications_events
             (draft_id, template_id, event_type, recipient_email, meta_json)
           VALUES ($1, $2, 'scheduled-send-skipped', NULL, $3::jsonb)`,
          [draftId, templateId, JSON.stringify({ reason: 'No recipients resolved from schedule filters.' })],
        );
      } else {
      ...
      await getRequiredPgPool().query(
        `UPDATE app.communications_drafts
            SET status = 'draft',
                scheduled_for = NULL,
                updated_at = NOW()
          WHERE id = $1`,
...
function buildFrontendRedirect(returnPath: string, params: Record<string, string>): string {
...
function ensureRegionalAdminAccess(req: Request, res: Response): boolean {
...
router.get('/gmail/callback', async (req, res) => {
  ...
  let state: GmailStatePayload;

  try {
    state = jwt.verify(rawState, env.jwtSecret) as GmailStatePayload;
  } catch {
...
router.get('/templates', async (req, res) => {
  ...
  try {
    ...
    const result = await getRequiredPgPool().query<{
      ...
      `SELECT id::text, name, subject, html_content, is_shared, created_by_email, updated_by_email, updated_at::text
...
router.post('/templates', async (req, res) => {
  ...
  try {
    await ensureCommunicationsTables();
    const insertResult = await getRequiredPgPool().query<{ id: string }>(
      `INSERT INTO app.communications_templates
         (name, subject, html_content, is_shared, created_by_email, updated_by_email, updated_at)
       VALUES ($1, $2, $3, TRUE, $4, $4, NOW())
       RETURNING id::text`,
      [name, subject, htmlContent, req.user!.email],
    );
...
router.put('/templates/:templateId', async (req, res) => {
  ...
  try {
    await ensureCommunicationsTables();
    const updateResult = await getRequiredPgPool().query(
      `UPDATE app.communications_templates
          SET name = $2,
              subject = $3,
              html_content = $4,
              updated_by_email = $5,
      ...
      [templateId, name, subject, htmlContent, req.user!.email],
    );
...
router.delete('/templates/:templateId', async (req, res) => {
  ...
  try {
    await ensureCommunicationsTables();
    const deleteResult = await getRequiredPgPool().query(
      `UPDATE app.communications_templates
          SET archived_at = NOW(),
              updated_by_email = $2,
              updated_at = NOW()
...
router.post('/drafts', async (req, res) => {
  ...
  try {
    await ensureCommunicationsTables();
    const insertResult = await getRequiredPgPool().query<{ id: string }>(
      `INSERT INTO app.communications_drafts
...
router.post('/drafts/:draftId/test-send', async (req, res) => {
  ...
  try {
    ...
    const draftResult = await getRequiredPgPool().query<{
      id: string;
      template_id: string | null;
      subject: string;
      html_content: string;
    }>(
      `SELECT id::text, template_id::text, subject, html_content
         FROM app.communications_drafts
        WHERE id = $1
          AND archived_at IS NULL`,
      [draftId],
...
router.get('/audience/options', async (req, res) => {
  ...
  try {
    ...
    const [rolesResult, marketCentersResult] = await Promise.all([
      getRequiredPgPool().query<{ role_name: string }>(
        `SELECT DISTINCT UPPER(TRIM(role_name)) AS role_name
           FROM migration.associate_roles
          WHERE role_name IS NOT NULL
            AND TRIM(role_name) <> ''
          ORDER BY 1`,
      ),
      getRequiredPgPool().query<{ source_market_center_id: string; name: string | null }>(
...
router.get('/analytics/summary', async (req, res) => {
  ...
  try {
    await ensureCommunicationsTables();
    const totals = await getRequiredPgPool().query<{
      event_type: string;
      total: string;
    }>(
...
const communicationsUploadErrorHandler: ErrorRequestHandler = (error, _req, res, next) => {
  if (!error) {
    return next();
  }
