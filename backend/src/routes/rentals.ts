...
let rentalAddressSchemaEnsured: Promise<void> | null = null;
...
type ParticipantInput = Record<string, unknown>;

type ParticipantComputed = {
  associate_id: string | null;
  associate_name: string | null;
  participant_role: string;
  agent_deal_split: number;
  market_center_split: number;
  agent_split: number;
...
function computeParticipantAllocations(
  ...
  totalAgentNetAmount: number;
  rentalCountsTowardCap: boolean;
} {
...
function toDocumentType(value: unknown): string {
...
function extensionFromMimeType(mimeType: string | undefined): string {
  ...
  if (n.includes('officedocument.wordprocessingml.document')) return '.docx';
  if (n.includes('png')) return '.png';
  if (n.includes('jpeg') || n.includes('jpg')) return '.jpg';
  return '.bin';
}

function decodeBase64File(input: string | undefined): Buffer | null {
...
async function storeRentalDocumentFile(file: UploadFilePayload): Promise<{ fileUrl: string; storagePath: string | null; fileName: string }> {
...
async function getNextRentalNumber(db: NonNullable<typeof pool>): Promise<string> {
  ...
  const result = await db.query<{ nextval: string }>(
    `SELECT nextval('app.rental_number_seq')::text AS nextval`
  );
  const seq = parseInt(result.rows[0]?.nextval ?? '1', 10);
  return `RNT${String(seq).padStart(5, '0')}`;
}

async function ensureRentalAddressColumns(db: NonNullable<typeof pool>): Promise<void> {
...
function buildDueDate(monthDate: Date, dueDay: number | null): string {
  if (!dueDay) return formatDate(monthDate);
...
async function canWriteRental(
  ...
  const result = await db.query<{ has_access: boolean }>(
    ...
    [rentalId, perms.marketCenterId]
  );
  return Boolean(result.rows[0]?.has_access);
}

function hasRentalAdminAccess(perms: UserPermissions): boolean {
  return perms.scope === 'GLOBAL' || perms.scope === 'MARKET_CENTRE';
}

function addMarketCentreRentalAccessFilter(
  whereClauses: string[],
  params: unknown[],
...
// ---------------------------------------------------------------------------
async function refreshPaymentStatuses(db: NonNullable<typeof pool>): Promise<void> {
  await db.query(`
  ...
  try {
    ...
    const [totalResult, dataResult] = await Promise.all([
      pool.query<{ total: string }>(
        `SELECT COUNT(*)::text AS total FROM app.rentals r ${where}`,
        countParams
      ),
      pool.query(
        `SELECT r.*,
...
router.get('/:id', resolvePermissions, async (req, res) => {
  ...
  if (!Number.isFinite(rentalId)) return res.status(400).json({ error: 'Invalid rental ID.' });
...
router.post('/', resolvePermissions, async (req, res) => {
  ...
  if (!propertyAddress) return res.status(400).json({ error: 'property_address is required.' });
  ...
  try {
    ...
    const paymentDueDay = toNumber(body.payment_due_day) as number | null;

    // Insert rental
    const rentalResult = await pool.query<{ id: number }>(
      `INSERT INTO app.rentals (
    ...
    if (participants.length > 0) {
      for (const p of participantAllocations.computed) {
        await pool.query(
          `INSERT INTO app.rental_participants (
    ...
    const scheduleRows = generatePaymentSchedule(
      newRentalId, rentalType, frequency,
      leaseStartDate, leaseEndDate, paymentDueDay,
      rentalAmount, commissionAmount, grossCommission, companyDollar,
      royalty, growthShare, agentNetAmount, userEmail
    );

    for (const row of scheduleRows) {
      await pool.query(
        `INSERT INTO app.rental_payment_schedule (
    ...
    // Audit log
    await pool.query(
      `INSERT INTO app.rental_audit_log (rental_id, action, new_value, changed_by_user_id)
       VALUES ($1, 'RENTAL_CREATED', $2::jsonb, $3)`,
      [newRentalId, JSON.stringify({ rental_number: rentalNumber, rental_type: rentalType }), userEmail]
    );
...
router.put('/:id', resolvePermissions, async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO app.rental_audit_log (rental_id, action, new_value, changed_by_user_id)
       VALUES ($1, 'RENTAL_EDITED', $2::jsonb, $3)`,
      [rentalId, JSON.stringify(body), userEmail]
    );
...
router.post('/:id/cancel', resolvePermissions, async (req, res) => {
  ...
  if (!cancelledReason) return res.status(400).json({ error: 'cancelled_reason is required.' });

  const userEmail = (req as unknown as { user: { email: string } }).user?.email ?? null;

  try {
    await pool.query(
      `UPDATE app.rentals SET rental_status = 'CANCELLED', cancelled_at = NOW(), cancelled_reason = $1, updated_by_user_id = $2, updated_at = NOW() WHERE id = $3`,
      [cancelledReason, userEmail, rentalId]
    );

    await pool.query(
      `INSERT INTO app.rental_audit_log (rental_id, action, new_value, changed_by_user_id)
       VALUES ($1, 'RENTAL_CANCELLED', $2::jsonb, $3)`,
      [rentalId, JSON.stringify({ cancelled_reason: cancelledReason }), userEmail]
    );
...
router.post('/payment-schedule/:scheduleId/mark-paid', resolvePermissions, async (req, res) => {
  ...
  try {
    ...
    const scheduleResult = await pool.query(
      `SELECT ps.*, r.rental_number, r.rental_type, r.market_centre_id, r.market_centre_name,
    ...
    const txTableExists = await pool.query<{ exists: string | null }>(
    ...
    if (txTableExists.rows[0]?.exists) {
      ...
      await pool.query(
        `CREATE SEQUENCE IF NOT EXISTS migration.rental_transaction_number_seq
      ...
      const txNumResult = await pool.query<{ next_number: string }>(
        `SELECT nextval('migration.rental_transaction_number_seq')::text AS next_number`
      );
      ...
      const txResult = await pool.query<{ id: string }>(
      ...
      const participantsResult = await pool.query(
        `SELECT * FROM app.rental_participants WHERE rental_id = $1 ORDER BY id`,
        [schedule.rental_id]
      );

      for (const p of participantsResult.rows) {
        if (!p.associate_id) continue;
        // Look up associate DB id
        const assocResult = await pool.query<{ id: string }>(
          `SELECT id::text FROM migration.core_associates WHERE source_associate_id = $1 LIMIT 1`,
          [p.associate_id]
        );
    ...
    await pool.query(
      `UPDATE app.rental_payment_schedule
          SET payment_status = 'PAID',
              ...
              updated_at = NOW()
        WHERE id = $5`,
      [paidDate, userEmail, transactionId !== null, transactionId, scheduleId]
    );

    // Update rental status to ACTIVE if it was DRAFT
    await pool.query(
      `UPDATE app.rentals SET rental_status = 'ACTIVE', updated_at = NOW()
        WHERE id = $1 AND rental_status = 'DRAFT'`,
      [schedule.rental_id]
    );

    // Audit log
    await pool.query(
      `INSERT INTO app.rental_audit_log (rental_id, payment_schedule_id, transaction_id, action, new_value, changed_by_user_id)
       VALUES ($1,$2,$3,'PAYMENT_MARKED_PAID',$4::jsonb,$5)`,
      [schedule.rental_id, scheduleId, transactionId, JSON.stringify({ paid_date: paidDate, transaction_id: transactionId }), userEmail]
    );
...
router.post('/payment-schedule/:scheduleId/cancel', resolvePermissions, async (req, res) => {
  ...
  try {
    const scheduleResult = await pool.query(
      `SELECT ps.*, r.market_centre_id FROM app.rental_payment_schedule ps
       JOIN app.rentals r ON r.id = ps.rental_id WHERE ps.id = $1 LIMIT 1`,
      [scheduleId]
    );
    ...
    await pool.query(
      `UPDATE app.rental_payment_schedule
          SET payment_status = 'CANCELLED',
              ...
              updated_at = NOW()
        WHERE id = $3`,
      [cancelledReason, userEmail, scheduleId]
    );

    await pool.query(
      `INSERT INTO app.rental_audit_log (rental_id, payment_schedule_id, action, new_value, changed_by_user_id)
       VALUES ($1,$2,'PAYMENT_CANCELLED',$3::jsonb,$4)`,
      [schedule.rental_id, scheduleId, JSON.stringify({ cancelled_reason: cancelledReason }), userEmail]
    );
...
router.get('/:id/documents', resolvePermissions, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `SELECT * FROM app.rental_documents WHERE rental_id = $1 ORDER BY uploaded_at DESC, id DESC`,
      [rentalId]
    );
...
router.post('/:id/documents', resolvePermissions, async (req, res) => {
  ...
  if (!documentName) return res.status(400).json({ error: 'document_name is required.' });
  ...
  try {
    const insertResult = await pool.query(
      `INSERT INTO app.rental_documents (
        rental_id, document_name, document_type, file_name, file_url, storage_path, uploaded_by_user_id
      ) VALUES ($1,$2,$3,$4,$5,$6,$7)
      RETURNING *`,
      [rentalId, documentName, documentType, fileName, fileUrl, storagePath, userEmail]
    );
...
router.post('/:id/documents/upload', resolvePermissions, async (req, res) => {
  ...
  const documentType = toDocumentType(req.body?.document_type);
  const uploadFile: UploadFilePayload = {
  ...
  try {
    ...
    const insertResult = await pool.query(
      ...
      RETURNING *`,
      [rentalId, documentName, documentType, uploaded.fileName, uploaded.fileUrl, uploaded.storagePath, userEmail]
    );
...
router.delete('/documents/:documentId', resolvePermissions, async (req, res) => {
  ...
  try {
    const existing = await pool.query<{
      ...
      `SELECT id, rental_id, document_name, document_type, file_name, file_url
    ...
    const userEmail = (req as unknown as { user: { email: string } }).user?.email ?? null;
    await pool.query(
      `INSERT INTO app.rental_audit_log (rental_id, action, old_value, changed_by_user_id)
...
router.get('/:id/audit-log', resolvePermissions, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `SELECT * FROM app.rental_audit_log WHERE rental_id = $1 ORDER BY changed_at DESC LIMIT 100`,
      [rentalId]
    );
    return res.json({ items: result.rows });
  } catch (err) {
    ...
    const result = await pool.query(
      `WITH scoped_rentals AS (
       ...
       SELECT
         ...
         (SELECT COALESCE(SUM(company_dollar), 0) FROM scoped_schedule WHERE payment_status = 'PAID' AND paid_date >= $2::date) AS rental_co_dollar_this_month`,
      [mcId, monthStart]
    );
