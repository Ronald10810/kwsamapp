...
async function saveSubTables(pg: Pool, listingId: number, b: Record<string, unknown>): Promise<void> {
  ...
  if (Array.isArray(b.property_areas)) {
    ...
    for (const [i, pa] of (b.property_areas as PropertyAreaEntry[]).entries()) {
      ...
      await pg.query(
        `INSERT INTO migration.listing_property_areas (listing_id, area_type, count, size, description, sub_features, sort_order) VALUES ($1,$2,$3,$4::numeric,$5,$6,$7)`,
        [listingId, toText(pa.area_type), pa.count ?? null, toNumber(pa.size), toText(pa.description), subFeatures, pa.sort_order ?? i]
      );
    }
  }
