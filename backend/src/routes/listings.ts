  ...
  try {
    ...
    const listingResult = await pool.query(
      `SELECT
        ...
        cl.construction_date::text, cl.height_restriction::text, cl.out_building_size::text, cl.zoning_type,
        cl.is_furnished, cl.pet_friendly, cl.retirement_living,
        cl.has_standalone_building, cl.has_flatlet, cl.property_auction,
        cl.has_borehole, cl.has_gas_geyser, cl.has_solar_panels,
        cl.has_backup_battery_or_inverter, cl.has_backup_water, cl.has_solar_geyser, cl.has_water_tank,
        cl.wheelchair_accessible, cl.has_generator,
        cl.adsl, cl.fibre, cl.isdn, cl.dialup, cl.fixed_wimax, cl.satellite,
        cl.nearby_bus_service, cl.nearby_minibus_taxi_service, cl.nearby_train_service,
    ...
    const areasResult = await pool.query(
      `SELECT area_type, MAX(count)::numeric AS count
       ...
       WHERE listing_id = $1
         AND LOWER(TRIM(COALESCE(area_type, ''))) IN ('bedroom', 'bathroom', 'garage', 'parking', 'pool', 'garden', 'study', 'office')
       GROUP BY area_type`,
      [id]
    );
    ...
    const sustainabilityInfoFeature = {
      ...
      waterTank: toOptionalBool(listing.has_water_tank),
    ...
    const p24Payload: Record<string, unknown> = {
      ...
      propertyFeatures: {
        ...
        domesticBathrooms: 0,
        heightRestrictions: toNumber(listing.height_restriction),
        receptionRooms: null,
        studies: (areaCounts.study ?? 0) + (areaCounts.office ?? 0),
        kitchens: {
          kitchens: 0,
          description: null,
          dishwasher: null,
        ...
        domesticRooms: 0,
        outsideToilets: 0,
        garden: areaCounts.garden != null ? areaCounts.garden > 0 : false,
        pool: hasPoolFeature,
        flatlet: toBool(listing.has_flatlet),
        secondHouse: false,
        outBuildingsSize: toNumber(listing.out_building_size),
        bedroomsDescription: '',
        ...
        hasStandaloneBuilding: toBool(listing.has_standalone_building),
        numberOfFloors: null,
        publicTransport: {
          nearbyBusService: toBool(listing.nearby_bus_service),
          nearbyMinibusTaxiService: toBool(listing.nearby_minibus_taxi_service),
          nearbyTrainService: toBool(listing.nearby_train_service),
        },
        internetAccess: internetAccessFeature,
        isWheelchairAccessible: toBool(listing.wheelchair_accessible),
        hasGenerator: toBool(listing.has_generator),
