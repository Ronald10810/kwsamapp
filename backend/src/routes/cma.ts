...
import { generateCmaData, type CmaFormData } from '../services/openai-cma.js';
...
// ---- Helper: resolve agent profile from DB ----------------------------------
...
async function resolveAgentProfile(email: string, name?: string): Promise<AgentProfile | null> {
  ...
  let assocResult = await pool.query<{
    ...
    `SELECT id::text, full_name, email, kwsa_email, private_email, mobile_number, source_market_center_id
  ...
  const [jobTitleResult, mcResult] = await Promise.all([
    ...
    assoc.source_market_center_id
      ? pool.query<{ name: string | null; logo_image_url: string | null; document_logo_image_url: string | null }>(
          `SELECT name, logo_image_url, document_logo_image_url
             FROM migration.core_market_centers
            WHERE source_market_center_id = $1
            LIMIT 1`,
          [assoc.source_market_center_id]
        )
      : Promise.resolve({ rows: [] as Array<{ name: string | null; logo_image_url: string | null; document_logo_image_url: string | null }> }),
  ]);
...
function buildFileName(propertyAddress: string | undefined): string {
...
router.post(
  '/generate',
  (req, res, next) => {
    uploadLoom.single('loomReport')(req as never, res as never, (err?: unknown) => {
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (storageConfig.localUploadsEnabled) {
        ...
        const loomUploadResult = await uploadToGcs(
          loomFile.buffer,
          loomFile.originalname || 'loom-report.pdf',
          'loom-reports',
          loomFile.mimetype || 'application/pdf'
        );
      ...
      const pool = getRequiredPgPool();
      await pool.query(
        `INSERT INTO public.cma_documents (
           associate_email,
           associate_db_id,
           property_address,
           seller_name,
      ...
      res.status(500).json({ error: (error as Error).message || 'Failed to generate CMA.' });
    }
  }
);
...
router.get('/history', async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query<{
      ...
      `SELECT id, property_address, seller_name, loom_file_url, loom_original_name, file_name, file_url, created_at::text
