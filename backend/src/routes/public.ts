...
let leadMailer: ReturnType<typeof nodemailer.createTransport> | null = null;
...
async function fetchListingMarketingUrls(pg: Pool, listingId: number): Promise<PublicMarketingUrl[]> {
...
function pickPrimaryYouTubeUrl(marketingUrls: PublicMarketingUrl[]): string | null {
...
function normalizeWhatsAppMsisdn(value: string | null | undefined): string | null {
...
function buildWhatsAppHref(phone: string | null | undefined, message: string): string | null {
  const msisdn = normalizeWhatsAppMsisdn(phone);
  if (!msisdn) return null;
  return `https://wa.me/${msisdn}?text=${encodeURIComponent(message)}`;
}

function buildLandingShareUrl(kwuid: string, listingNumber: string): string {
...
function parseTruthyEnv(value: string | null | undefined, fallback: boolean): boolean {
  const normalized = (value ?? '').trim().toLowerCase();
  if (!normalized) return fallback;
  return ['1', 'true', 'yes', 'on'].includes(normalized);
}

function normalizeLeadInput(value: unknown): z.input<typeof leadFormSchema> {
...
function buildSort(sortBy: SortValue, prioritizeFeatured: boolean): string {
...
router.post(['/landing/:kwuid/:listingNumber/lead', '/landing/:kwuid/listing/:listingNumber/lead'], async (req, res) => {
  ...
  const listingResult = await withRetry(() => pool.query<{
    id: number;
    listing_number: string | null;
    property_title: string | null;
    short_title: string | null;
  ...
  const siteOwnerSignatureName = (
    siteOwner.fullName ?? `${siteOwner.firstName ?? ''} ${siteOwner.lastName ?? ''}`.trim()
