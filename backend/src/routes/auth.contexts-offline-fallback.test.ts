...
vi.mock('../config/db.js', () => ({
  ...
  withPgPoolRetry: async (work: (pool: { query: ReturnType<typeof vi.fn> }) => Promise<unknown>) => work({ query: vi.fn() }),
}));
...
function getAuthContextsHandler(): (req: any, res: any) => Promise<void> {
  const layer = (router as any).stack.find(
    (entry: any) => entry.route?.path === '/contexts' && entry.route?.methods?.get
  );
