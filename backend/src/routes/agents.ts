...
function toNullableBool(value: unknown): boolean | null {
  ...
  if (typeof value === 'string') {
    const normalized = value.trim().toLowerCase();
    if (!normalized || normalized === 'not applicable' || normalized === 'n/a' || normalized === 'na') return null;
...
async function resolveMarketCenterScopeMatch(
  client: { query: PoolClient['query'] },
  targetAssociateId: number,
  activeMarketCenterId: string
): Promise<{ allowed: boolean; notFound: boolean }> {
  const targetResult = await client.query<{ source_market_center_id: string | null; market_center_id: string | null }>(
    `SELECT
  ...
  let activeMcDbId: number | null = null;
  let activeResolvedSourceNorm = '';

  const activeBySource = await client.query<{ id: string; source_market_center_id: string | null }>(
    `SELECT id::text AS id, source_market_center_id
  ...
  if (activeBySource.rows[0]) {
    ...
    const activeById = await client.query<{ id: string; source_market_center_id: string | null }>(
      `SELECT id::text AS id, source_market_center_id
         FROM migration.core_market_centers
        WHERE id::text = $1
        LIMIT 1`,
  ...
  const sourceMatch = Boolean(targetSourceNorm) && (
    targetSourceNorm === activeMcNorm
...
async function resolveAdminMarketCenterIds(client: PoolClient, values: string[]): Promise<string[]> {
  const requested = uniqueNormalizedStrings(values);
  if (requested.length === 0) return [];

  const result = await client.query<{ raw_value: string; source_market_center_id: string | null }>(
    `SELECT
       input.raw_value,
  ...
  return uniqueNormalizedStrings(
    result.rows.map((row) => (row.source_market_center_id ?? row.raw_value).trim()).filter(Boolean)
  );
}
...
function normalizeProperty24Fields(
  property24OptIn: boolean,
  rawAgentProperty24Id: unknown,
  rawProperty24Status: unknown,
): { agentProperty24Id: string | null; property24Status: string } {
  if (!property24OptIn) {
    return {
...
async function ensureAssociateAccessSuspensionTable(): Promise<void> {
  if (!pool || !LOCAL_ASSOCIATE_SUSPENSION_ENABLED) return;
  await pool.query(`
...
async function getMappAccessPayload(associateId: number): Promise<{
  ...
  const suspension = await pool.query<MappAccessRow>(
    `
    SELECT
...
async function resolveCurrentAssociateByEmail(userEmail: string): Promise<{ id: number; kwuid: string | null } | null> {
  if (!pool) return null;
  const associateResult = await pool.query<{ id: string; kwuid: string | null }>(
    `
...
async function fetchFeaturedListingsForAssociate(associateId: number): Promise<Array<{
  listingId: number;
...
router.get('/me/home', async (req, res) => {
  ...
  try {
    const associateResult = await pool.query<{
      ...
      market_center_name: string | null;
      market_center_logo_image_url: string | null;
      market_center_document_logo_image_url: string | null;
    }>(
      `
      SELECT
        a.id::text,
    ...
    if (isTeamContext && Number.isFinite(associateId)) {
      ...
      const allowedTeamsResult = await pool.query<{
        source_team_id: string | null;
...
router.get('/me/featured-listings', async (req, res) => {
  ...
  try {
    ...
    if (!associate) return res.json({ items: [] });
  ...
  try {
    ...
    try {
      ...
      if (uniqueListingIds.length > 0) {
        const validListings = await client.query<{ id: string }>(
          `
        ...
        for (const listingId of uniqueListingIds) {
          if (!validSet.has(listingId)) continue;
          await client.query(
            `
  ...
  try {
    ...
    const [socialMedia, roles, jobTitles, serviceCommunities, adminMarketCenters, adminTeams, documents, notes] = await Promise.all([
      ...
      pool.query<{ source_market_center_id: string }>(
        `SELECT COALESCE(resolved.source_market_center_id, amc.source_market_center_id) AS source_market_center_id
