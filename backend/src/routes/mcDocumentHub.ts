...
function checkAdmin(perms: { isOfficeAdmin: boolean; isRegionalAdmin: boolean }): boolean {
...
router.get('/agent', resolvePermissions, async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query<{
      ...
      `SELECT id::text, folder_id::text, title, description, file_url, original_file_name, mime_type,
...
router.get('/', resolvePermissions, async (req, res) => {
  ...
  try {
    const foldersResult = await pool.query<{
      ...
      `SELECT id::text, name, description, created_by, created_at::text
       FROM migration.mc_document_hub_folders
       WHERE source_market_center_id = $1
       ORDER BY name ASC, created_at DESC`,
      [mcId]
...
router.post('/', resolvePermissions, async (req, res) => {
  ...
  const title = ((req.body as Record<string, unknown>)?.title ?? '').toString().trim();
  const description = ((req.body as Record<string, unknown>)?.description ?? '').toString().trim() || null;
  const folderIdRaw = ((req.body as Record<string, unknown>)?.folder_id ?? '').toString().trim();
  ...
  try {
    if (folderId !== null) {
      ...
      const folderRes = await pool.query<{ id: string }>(
        `SELECT id::text
         FROM migration.mc_document_hub_folders
    ...
    if (isGcs) {
      const { publicUrl, objectName } = await uploadToGcs(
        req.file.buffer,
    ...
    const result = await pool.query<{ id: string }>(
      `INSERT INTO migration.mc_document_hub
    ...
    if (!isGcs && req.file.path) await fs.unlink(req.file.path).catch(() => undefined);
    const msg = err instanceof Error ? err.message : String(err);
    return res.status(500).json({ error: msg });
  }
});
...
router.post('/folders', resolvePermissions, async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query<{
      ...
      `INSERT INTO migration.mc_document_hub_folders
         (source_market_center_id, name, description, created_by)
       VALUES ($1, $2, $3, $4)
       RETURNING id::text, name, description, created_by, created_at::text`,
      [mcId, name, description, req.user?.email ?? 'unknown']
    );
...
router.patch('/folders/:id', resolvePermissions, async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query<{
      ...
      `UPDATE migration.mc_document_hub_folders
       SET name = $1,
           description = $2
       WHERE id = $3
       RETURNING id::text, name, description, created_by, created_at::text`,
...
router.delete('/folders/:id', resolvePermissions, async (req, res) => {
  ...
  try {
    ...
    const docsRes = await pool.query<{ total: string }>(
...
router.patch('/:id/move', resolvePermissions, async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query<{ folder_id: string | null }>(
      `UPDATE migration.mc_document_hub
       SET folder_id = $1
       WHERE id = $2
       RETURNING folder_id::text`,
      [folderId, docId]
    );
...
router.delete('/:id', resolvePermissions, async (req, res) => {
  ...
  try {
    const docRes = await pool.query<{
      ...
      `SELECT source_market_center_id, local_file_path
       FROM migration.mc_document_hub WHERE id = $1 LIMIT 1`,
      [docId]
    );
