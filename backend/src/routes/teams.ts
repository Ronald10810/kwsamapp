...
const REQUIRED_TEAM_COLUMNS: Record<string, string> = {
...
let teamSchemaPromise: Promise<void> | null = null;
let teamColumnCache: Set<string> | null = null;

async function ensureTeamSchema(): Promise<void> {
  if (!pool) return;

  // core_teams extra columns
  const existingResult = await pool.query<{ column_name: string }>(
    `SELECT column_name
...
function runEnsureTeamSchema(): Promise<void> {
...
async function getTeamColumns(): Promise<Set<string>> {
  if (!pool) return new Set();
  await runEnsureTeamSchema();
  if (teamColumnCache) return teamColumnCache;
...
function toStringArray(value: unknown): string[] {
  ...
  return value.map((e) => toText(e)).filter((e): e is string => Boolean(e));
}

function optCol(columns: Set<string>, col: string): string {
  return columns.has(col) ? `t.${col}` : `NULL::text AS ${col}`;
}

type TeamEditorContext = {
...
async function getTeamEditorContext(email: string): Promise<TeamEditorContext | null> {
  if (!pool) return null;

  const assocResult = await pool.query<{ id: string; source_team_id: string | null; source_market_center_id: string | null }>(
    `SELECT id::text, source_team_id, source_market_center_id
  ...
  const [titlesResult, adminTeamsResult] = await Promise.all([
    ...
    pool.query<{ source_team_id: string }>(
      `SELECT source_team_id FROM migration.associate_admin_teams WHERE associate_id = $1`,
      [assoc.id]
    ),
  ]);
...
function canEditTeamDetails(
  permissions: NonNullable<Express.Request['permissions']>,
  editor: TeamEditorContext | null,
  teamSourceMarketCenterId: string | null,
  teamSourceTeamId: string | null
): boolean {
  ...
  if (permissions.scope === 'MARKET_CENTRE') {
    ...
    const withinMc = Boolean(
      permissions.marketCenterId
  ...
  if (!editor || !teamSourceTeamId) return false;
...
router.get('/options', async (req, res) => {
  ...
  try {
    const tableCheck = await pool.query<{ exists: string | null }>(
      `SELECT to_regclass('migration.core_teams') AS exists`
    );
    if (!tableCheck.rows[0]?.exists) return res.json({ items: [] });

    const sourceMarketCenterId = toText(req.query.source_market_center_id as unknown);
    const params: string[] = [];
    const whereClauses = [
...
router.get('/permissions', resolvePermissions, async (req, res) => {
  ...
  if (!permissions || !userEmail) return res.status(401).json({ error: 'Unauthorised' });
  ...
  try {
    ...
    if (permissions.scope === 'MARKET_CENTRE') {
      ...
      if (permissions.marketCenterId) {
        const scopedResult = await pool.query<{ source_team_id: string }>(
    ...
    const ownEditableIds = canEditDetails
      ? Array.from(
          new Set(
            [editor?.sourceTeamId ?? null, ...(editor ? Array.from(editor.adminTeams) : [])]
...
router.get('/', async (req, res) => {
  ...
  const mcInput = toText(req.query.market_center_id as unknown);
  ...
  try {
    ...
    const totalResult = await pool.query<{ total: string }>(
      `SELECT COUNT(*)::text AS total FROM migration.core_teams t ${whereSql}`,
      params
    );
...
router.get('/:id', async (req, res) => {
  ...
  if (!Number.isFinite(idParam)) return res.status(400).json({ error: 'Invalid team id.' });
  ...
  try {
    ...
    // Cap history
    const capHistResult = await pool.query<{
      id: string;
      commission_split_to_team: string | null;
      team_cap_amount: string | null;
      manual_cap: boolean;
    ...
    const commResult = await pool.query<{
      ...
      `SELECT has_individual_cap, associate_default_cap::text, associate_default_split::text, productivity_coach
    ...
    const datesResult = await pool.query<{
      ...
      `SELECT open_date::text, close_date::text, cap_date::text, anniversary_date::text, anniversary_comment
    ...
    const portalResult = await pool.query<{
      ...
      `SELECT use_mc_account_p24, p24_agency_id, feed_to_p24, p24_auction_approved,
    ...
    const notesResult = await pool.query<{
      ...
      `SELECT id::text, note_text, note_type, created_by, created_at::text
    ...
    const membersResult = await pool.query<{
      ...
      `SELECT
       ...
       FROM migration.core_associates a
       WHERE a.team_id = $1
       ORDER BY a.full_name ASC NULLS LAST`,
      [idParam]
    ).catch(() => ({ rows: [] as never[] }));
...
router.post('/', resolvePermissions, async (req, res) => {
  ...
  const permissions = req.permissions;
  if (!permissions) return res.status(403).json({ error: 'Permission denied.' });
  ...
  try {
    ...
    try {
      ...
      if (sourceMarketCenterId) {
        const mcResult = await client.query<{ id: string }>(
...
router.put('/:id', resolvePermissions, async (req, res) => {
  ...
  try {
    ...
    const existingTeamResult = await pool.query<{ source_market_center_id: string | null; source_team_id: string | null }>(
      `SELECT source_market_center_id, source_team_id FROM migration.core_teams WHERE id = $1 LIMIT 1`,
      [idParam]
    );
    ...
    try {
      ...
      await client.query(
        `UPDATE migration.core_teams SET
           name = $1,
           status_name = $2,
           market_center_id = $3,
...
// ─── Shared sub-record saver ──────────────────────────────────────────────────

import type { PoolClient } from 'pg';

async function saveTeamSubRecords(client: PoolClient, teamId: number, body: Record<string, unknown>): Promise<void> {
  // Cap
  ...
  if (capAmount !== null || splitToTeam !== null) {
    await client.query(
      `INSERT INTO migration.team_caps
       ...
       DO UPDATE SET
         ...
         updated_at = NOW()`,
      [teamId, splitToTeam, capAmount, toBool(body.manual_cap), capYear]
    );
  }
  ...
  if (defaultCap !== null || defaultSplit !== null || productivityCoach !== null) {
    await client.query(
      `INSERT INTO migration.team_associate_commissions
       ...
       DO UPDATE SET
         ...
         updated_at = NOW()`,
      [teamId, toBool(body.has_individual_cap), defaultCap, defaultSplit, productivityCoach]
    );
  }
  ...
  if (openDate || closeDate || capDate || anniversaryDate) {
    await client.query(
      `INSERT INTO migration.team_dates
       ...
       DO UPDATE SET
         ...
         updated_at = NOW()`,
      [teamId, openDate, closeDate, capDate, anniversaryDate, toText(body.anniversary_comment)]
    );
  }

  // Portal settings
  await client.query(
    `INSERT INTO migration.team_portal_settings
  ...
  if (notes.length > 0 || body.notes !== undefined) {
    ...
    for (const note of notes) {
      await client.query(
        `INSERT INTO migration.team_notes (team_id, note_text, note_type, created_by)
         VALUES ($1, $2, $3, $4)`,
        [teamId, note, 'general', 'console-user']
      );
    }
  }
}
