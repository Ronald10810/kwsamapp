...
describe('CAP-002 approved rules regression ledger', () => {
  ...
  it('CAP002-T014 CAPDATE-01: audit artifact still exposes unresolved cap_date anomalies (no auto-remediation)', () => {
    const audit = JSON.parse(readWorkspaceFile('tmp_cap_audit_result_20260707.json')) as {
      data_quality?: { cap_dates?: { summary?: Record<string, number> } };
    };
  ...
  async function fetchAssociateRows(client: Client, selector: string): Promise<Array<{
    ...
    const result = await client.query<{
      ...
      is_capped: boolean;
      team_id: string | null;
    }>(associateCappersQuery, [selector]);
    return result.rows;
  }
