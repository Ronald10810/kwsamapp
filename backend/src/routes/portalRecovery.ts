...
function normalizeInputs(rawInputs: unknown): string[] {
...
function isKwlNumber(value: string): boolean {
...
function kwwHeaders(apiKey: string, apiSecret: string, kwuid: string): Record<string, string> {
  return {
...
type KwwLookupSource = {
  list_id?: number | string | null;
  list_uuid?: string | null;
  list_key?: string | null;
  list_kw_uid?: number | string | null;
  list_status?: string | null;
  list_status_id?: number | null;
  manual_entry?: boolean | null;
};

function extractKwwSource(body: unknown): KwwLookupSource | null {
  if (!body || typeof body !== 'object') return null;
  const root = body as Record<string, unknown>;
  const hitsObj = root.hits;
  if (!hitsObj || typeof hitsObj !== 'object') return null;
  const innerHits = (hitsObj as Record<string, unknown>).hits;
  if (!Array.isArray(innerHits) || innerHits.length === 0) return null;
  const first = innerHits[0];
  if (!first || typeof first !== 'object') return null;
  const source = (first as Record<string, unknown>)._source;
  if (!source || typeof source !== 'object') return null;
  return source as KwwLookupSource;
}
...
function ensureRegionalAdminAccess(req: Parameters<typeof router.post>[1] extends never ? never : any, res: any): boolean {
...
router.post('/run', async (req, res) => {
  ...
  if (!ensureRegionalAdminAccess(req, res)) return;

  const mode = String(req.body?.mode ?? '').trim().toLowerCase() as RecoveryMode;
  ...
  const requesterKwuidResult = await pool.query<{ kwuid: string | null }>(
    `SELECT kwuid::text
       FROM migration.core_associates
      WHERE id::text = $1
      LIMIT 1`,
    [req.permissions?.associateDbId ?? '']
  );
    ...
    try {
      if (mode === 'fix-owner-publish') {
        ...
        const localResult = await pool.query<{
          ...
          kww_sync_status: string | null;
          target_kwuid: string | null;
        }>(
          `SELECT cl.id::text,
                  cl.listing_number,
                  cl.kww_sync_status,
        ...
        const lookupBody = await parseJsonSafely<unknown>(lookupResponse);
        ...
        let patchBody = await parseJsonSafely<Record<string, unknown>>(patchResponse);
        ...
        if (!patchOk) {
          ...
          if (initialErrorCode === 'FORBIDDEN' && requesterKwuid && requesterKwuid !== targetKwuid) {
            ...
            patchBody = await parseJsonSafely<Record<string, unknown>>(retryPatchResponse);
        ...
        if (!patchOk) {
          ...
          if (errorCode === 'FORBIDDEN') {
            ...
            const verifyOwnerBody = await parseJsonSafely<unknown>(verifyOwnerResponse);
        ...
        const publishBody = (publishResult.body && typeof publishResult.body === 'object')
          ? (publishResult.body as Record<string, unknown>)
          : null;
        ...
        if (!publishOk) {
          ...
          if (isKwwPermissionDenied) {
            ...
            const verifyAfterOwnerBody = await parseJsonSafely<unknown>(verifyAfterOwnerResponse);
            ...
            await pool.query(
              `UPDATE migration.core_listings
                 SET kww_property_reference = COALESCE($1, kww_property_reference),
                     ...
                     updated_at = NOW()
               WHERE id::text = $4`,
              [syncedReferenceId, syncedListUuid, syncedListKey, local.id]
            );
        ...
        const verifyBody = await parseJsonSafely<unknown>(verifyResponse);
        ...
        const patchBody = await parseJsonSafely<Record<string, unknown>>(patchResponse);
