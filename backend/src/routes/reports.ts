...
function aggregateDetailRowsToTopDownProductionRows(detailRows: MonthEndDetailRow[]): TopDownAgentProductionRow[] {
  const grouped = new Map<string, {
    source_associate_id: string;
    associate_name: string;
    team_name: string;
  ...
  for (const row of detailRows) {
    const source_associate_id = (row.source_associate_id || '').trim() || 'unassigned';
    const associate_name = (row.associate_name || '').trim() || 'Unknown Associate';
    const team_name = (row.team_name || '').trim() || 'No Team';
    const market_center_name = (row.market_center_name || '').trim() || 'Unassigned / Unknown';
    const mc_source_id = (row.mc_source_id || '').trim();
    const key = `${source_associate_id}::${mc_source_id}::${associate_name}`;
    const txKey = (row.transaction_number || '').trim()
      || `${row.kwl_number}|${row.transaction_date ?? ''}|${row.status_change_date ?? ''}|${associate_name}`;
  ...
  const rows: TopDownAgentProductionRow[] = [];
  for (const bucket of grouped.values()) {
    let units = 0;
    for (const flags of bucket.txSides.values()) {
      units += resolveUnitContribution(flags);
    ...
    rows.push({
      source_associate_id: bucket.source_associate_id,
      associate_name: bucket.associate_name,
  ...
  return rows.sort((a, b) => b.total_gci - a.total_gci || a.associate_name.localeCompare(b.associate_name));
}
...
function isRegisteredTransactionStatus(status: string | null | undefined): boolean {
...
function buildRegisteredOnlyOrSelectedStatusSql(txAlias: string, selectedStatusesParam: string): string {
  return `EXISTS (
    ...
    OR (
      LOWER(TRIM(selected_status)) <> 'registered'
      AND LOWER(TRIM(COALESCE(${txAlias}.transaction_status, ''))) = LOWER(TRIM(selected_status))
    )
  )`;
}
...
router.get('/month-end/transactions', resolvePermissions, requireReportAccess(REPORT_KEYS.MONTH_END), async (req, res) => {
  ...
  try {
    ...
    const historicalStatusesForStartMixed = useFullLiveWindowForStart
      ? transactionStatuses.filter((status) => normalizeForMatch(status) !== 'start')
      : transactionStatuses;
    const liveDateFrom = useUnified
      ? (useFullLiveWindowForStart
        ? dateFrom
    ...
    if (
      useUnified
      && allowHistoricalDetailMerge
      && windows.historicalWindow
      && historicalStatusesForStartMixed.length > 0
    ) {
      historicalForUnified = await fetchHistoricalMonthEndDetailRows(
        windows.historicalWindow.from,
        windows.historicalWindow.to,
        historicalStatusesForStartMixed,
        saleTypes,
        selectedHistoricalPreviewBatchId,
      );
    }
    ...
    if (
      ...
      && allowHistoricalDetailMerge
      && historicalStatusesForStartMixed.length > 0
      && shouldUseHistoricalGapFallback(windows.historicalWindow)
    ) {
      const historicalWindow = windows.historicalWindow as UnifiedWindow;
    ...
    if (
      ...
      && historicalStatusesForStartMixed.length > 0
      && !historicalIsolationOnly
      && windows.transitionExceptionWindow
    ) {
    ...
    const liveRowsWithMeta = liveDetailSourceRows.map((row) => ({
      ...
      company_dollar: Number(row.company_dollar),
      team_dollar: Number(row.team_dollar),
      cap_remaining: isRegisteredTransactionStatus(row.transaction_status) ? Number(row.cap_remaining) : 0,
      listing_office: row.listing_office,
      bond_originator: row.bond_originator,
      bond_attorney: row.bond_attorney,
      bond_attorney_email: row.bond_attorney_email,
...
router.get('/top-down-agent', resolvePermissions, requireReportAccess(REPORT_KEYS.TOP_DOWN_PERFORMANCE), async (req, res) => {
  ...
  try {
    ...
    let productionRows: TopDownAgentProductionRow[] = productionRowsResult.rows.map((row) => ({
      source_associate_id: row.source_associate_id,
      associate_name: row.associate_name,
      team_name: row.team_name,
      market_center_name: row.market_center_name,
    ...
    if (monthEndHandler) {
      ...
      if (monthEndResState.statusCode < 400 && monthEndResState.payload && typeof monthEndResState.payload === 'object') {
        ...
        if (monthEndTransactionsHandler) {
          ...
          if (monthEndTransactionsResState.statusCode < 400 && monthEndTransactionsResState.payload && typeof monthEndTransactionsResState.payload === 'object') {
            const detailPayload = monthEndTransactionsResState.payload as { rows?: MonthEndDetailRow[] };
            let canonicalRows = aggregateDetailRowsToTopDownProductionRows(detailPayload.rows ?? []);

            if (associateQuery) {
              const normalizedAssociateQuery = associateQuery.trim().toLowerCase();
              canonicalRows = canonicalRows.filter((row) => row.associate_name.toLowerCase().includes(normalizedAssociateQuery));
            }
            if (teamQuery) {
              const normalizedTeamQuery = teamQuery.trim().toLowerCase();
              canonicalRows = canonicalRows.filter((row) => row.team_name.toLowerCase().includes(normalizedTeamQuery));
            }

            productionRows = canonicalRows;
          }
        }
      }
    }
