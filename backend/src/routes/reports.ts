...
async function computeMcDashboardData(mcSourceId: string, dateFrom: string, dateTo: string): Promise<McDashboardData> {
  if (!pool) {
  ...
  const summaryResult = await pool.query<{
    ...
    selected_mc AS (
      SELECT id, source_market_center_id, COALESCE(NULLIF(TRIM(name), ''), 'Unknown Market Centre') AS name, logo_image_url
    ...
    SELECT
      ...
      (
        SELECT COUNT(*)
        FROM associates_in_mc a
        WHERE COALESCE(a.start_date, a.anniversary_date) IS NOT NULL
          AND EXTRACT(MONTH FROM COALESCE(a.start_date, a.anniversary_date)) = EXTRACT(MONTH FROM $3::date)
          AND LOWER(TRIM(COALESCE(a.status_name, ''))) IN ('active', '1')
  ...
  const anniversaryPeopleResult = await pool.query<{
    ...
    SELECT
      ...
      COALESCE(NULLIF(TRIM(t.name), ''), 'No Team') AS team_name,
      NULLIF(TRIM(ca.mobile_number), '') AS mobile_number,
      COALESCE(ca.start_date::date, ca.anniversary_date::date)::text AS anniversary_date,
      GREATEST(
        EXTRACT(YEAR FROM AGE($2::date, COALESCE(ca.start_date::date, ca.anniversary_date::date)))::int,
        0
      )::text AS anniversary_years
    FROM migration.core_associates ca
    LEFT JOIN migration.core_teams t ON t.id = ca.team_id
    LEFT JOIN migration.core_market_centers mc_assoc ON mc_assoc.id = ca.market_center_id
    WHERE COALESCE(ca.start_date, ca.anniversary_date) IS NOT NULL
      AND REGEXP_REPLACE(LOWER(TRIM(COALESCE(mc_assoc.source_market_center_id, ''))), '[^a-z0-9]+', '', 'g') = $1
      AND EXTRACT(MONTH FROM COALESCE(ca.start_date::date, ca.anniversary_date::date)) = EXTRACT(MONTH FROM $2::date)
      AND LOWER(TRIM(COALESCE(ca.status_name, ''))) IN ('active', '1')
    ORDER BY EXTRACT(DAY FROM COALESCE(ca.start_date::date, ca.anniversary_date::date)) ASC, associate_name ASC
    LIMIT 150
    `,
    [normalizedMcId, dateTo]
  );
  ...
  return {
    ...
    people: {
      ...
      anniversaries_this_month: anniversaryPeopleResult.rows.map((row) => ({
        ...
        years: Number(row.anniversary_years),
      })),
    },
...
router.get('/month-end', resolvePermissions, async (req, res) => {
  ...
  try {
    ...
    const params: Array<string | string[]> = [dateFrom, dateTo, transactionStatuses, saleTypes];
    ...
    const sql = `
      ...
      base AS (
        SELECT
          tac.transaction_id,
          COALESCE(NULLIF(TRIM(ct.market_center_name), ''), NULLIF(TRIM(mc_tx_primary.name), ''), NULLIF(TRIM(mc_tx.name), ''), NULLIF(TRIM(mc_assoc.name), ''), NULLIF(TRIM(mc_office.name), ''), NULLIF(TRIM(tac.office_name), ''), 'Unassigned / Unknown') AS tx_mc_name,
          COALESCE(NULLIF(TRIM(ct.source_market_center_id), ''), NULLIF(TRIM(mc_tx_primary.source_market_center_id), ''), NULLIF(TRIM(mc_tx.source_market_center_id), ''), NULLIF(TRIM(mc_assoc.source_market_center_id), ''), NULLIF(TRIM(mc_office.source_market_center_id), ''), '') AS tx_mc_source_id,
          ...
          COALESCE(tac.transaction_gci_before_fees, tac.gci_after_fees_excl_vat, 0) AS transaction_gci_before_fees,
          COALESCE(tac.growth_share, 0) AS growth_share,
          COALESCE(tac.production_royalties, 0) AS production_royalties,
          -- company_dollar = MCDollar per vBI_Transactions (TAPD.MCDollar), NOT gci_after_fees_excl_vat
          COALESCE(tac.market_center_dollar, 0) AS company_dollar,
          COALESCE(tac.associate_dollar, 0) AS associate_dollar,
          COALESCE(tac.team_dollar, 0) AS team_dollar,
        ...
        WHERE COALESCE(ct.status_change_date::date, tac.effective_reporting_date::date) >= $1::date
          ...
          AND (
            ...
            OR REGEXP_REPLACE(LOWER(TRIM(COALESCE(mc_assoc.source_market_center_id, ''))), '[^a-z0-9]+', '', 'g') = REGEXP_REPLACE(LOWER(TRIM(${scopeMcParam}::text)), '[^a-z0-9]+', '', 'g')
...
router.get('/month-end/transaction-summary', resolvePermissions, async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query<{
      ...
      base AS (
        SELECT
          ...
          COALESCE(tac.split_percentage, 0)::numeric(18,4) AS split_percentage,
          COALESCE(NULLIF(TRIM(ta.agent_role), ''), NULLIF(TRIM(tac.transaction_side), ''), NULLIF(TRIM(ct.transaction_type), ''), 'Unspecified') AS transaction_side
        FROM tac_dedup tac
        JOIN migration.core_transactions ct ON ct.id = tac.transaction_id
        LEFT JOIN migration.transaction_agents ta ON ta.id = tac.transaction_agent_id
        ...
        WHERE COALESCE(${dateFilterExpr}, tac.effective_reporting_date::date) >= $1::date
          ...
          AND (
            ...
            OR REGEXP_REPLACE(LOWER(TRIM(COALESCE(ct.source_market_center_id, mc_tx_primary.source_market_center_id, mc_tx.source_market_center_id, mc_office.source_market_center_id, mc_assoc.source_market_center_id, ''))), '[^a-z0-9]+', '', 'g') = ANY($5::text[])
          )
      ...
      by_mc AS (
        SELECT
          ...
          ROUND(COALESCE(SUM(team_dollar), 0)::numeric, 2) AS team_dollar
        FROM per_tx
        WHERE CARDINALITY($7::text[]) = 0 OR mc_source_id = ANY($7::text[])
        GROUP BY market_center_name, mc_source_id
      )
      SELECT
        market_center_name,
    ...
    const rows: MonthEndSummaryRow[] = result.rows.map((row) => ({
      market_center_name: row.market_center_name,
      mc_source_id: row.mc_source_id,
...
router.get('/month-end/transactions', resolvePermissions, async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query<MonthEndDetailRow & { transaction_id: string }>(
      `
      WITH ${transactionAgentCalculationDedupCte},
      ...
      SELECT
        ...
        ROUND(COALESCE(ct.sales_price, 0)::numeric(18,2) * (COALESCE(tac.split_percentage, 0)::numeric / 100), 2)::numeric(18,2) AS agent_sales_volume,
        COALESCE(tac.average_commission_pct, 0)::numeric(18,4) AS comm_pct,
        CASE
          WHEN COALESCE(tac.split_percentage, 0) > 0
            THEN ROUND((COALESCE(tac.transaction_gci_before_fees, tac.gci_after_fees_excl_vat, 0)::numeric * 100) / COALESCE(tac.split_percentage, 0)::numeric, 2)
          ELSE COALESCE(tac.transaction_gci_before_fees, tac.gci_after_fees_excl_vat, 0)::numeric
      ...
      WHERE COALESCE(${dateFilterExpr}, tac.effective_reporting_date::date) >= $1::date
        ...
        AND ($6::bigint IS NULL OR tac.associate_id = $6::bigint)
        AND (CARDINALITY($7::text[]) = 0 OR COALESCE(NULLIF(TRIM(mc_tx_primary.source_market_center_id), ''), NULLIF(TRIM(mc_tx.source_market_center_id), ''), NULLIF(TRIM(mc_office.source_market_center_id), ''), NULLIF(TRIM(mc_assoc.source_market_center_id), ''), '') = ANY($7::text[]))
        AND (
          NULLIF($8::text, '') IS NULL
          OR (
            REGEXP_REPLACE(LOWER(TRIM(COALESCE(NULLIF(t.source_team_id, ''), ''))), '[^a-z0-9]+', '', 'g') = REGEXP_REPLACE(LOWER(TRIM($8::text)), '[^a-z0-9]+', '', 'g')
    ...
    const txMcMap = new Map<string, { hasBoth: boolean; hasBuyer: boolean; hasSeller: boolean; hasOther: boolean; hasReferral: boolean; listPrice: number; salesPrice: number }>();
    for (const row of result.rows as Array<MonthEndDetailRow & { transaction_id: string; unit_side?: string }>) {
...
router.get('/top-down-agent', resolvePermissions, async (req, res) => {
  ...
  try {
    ...
    const transactionStatuses = req.query.transaction_status === undefined
      ? ['Registered']
      : parseCsvParam(req.query.transaction_status);
    const saleTypes = req.query.sale_type === undefined
      ? ['For Sale']
      : parseCsvParam(req.query.sale_type);
    ...
    const listingRowsResult = await pool.query<{
      ...
      SELECT
        ...
        market_center_name,
        mc_source_id,
        COALESCE(STRING_AGG(DISTINCT NULLIF(TRIM(listing_number), ''), ', ' ORDER BY NULLIF(TRIM(listing_number), '')), '') AS listing_numbers,
    ...
    const listingRows: TopDownAgentListingRow[] = listingRowsResult.rows.map((row) => ({
      ...
      market_center_name: row.market_center_name,
      mc_source_id: row.mc_source_id,
      listing_numbers: row.listing_numbers,
      ...
      production: {
        ...
        summary_by_market_center: Array.from(
          productionRows.reduce((map, row) => {
            ...
            return map;
          }, new Map<string, { market_center_name: string; mc_source_id: string; contracts: number; units: number; total_gci: number; company_dollar: number }>()).values()
        )
          .map((row) => ({
            ...row,
...
router.get('/mc-dashboard', resolvePermissions, async (req, res) => {
  ...
  try {
    ...
    if (!isPastDailyRefreshTime && latestBeforeToday.rows[0]) {
      ...
      return res.json({
        ...staleCached,
        people: (staleCached.people as typeof emptyPeople | undefined) ?? emptyPeople,
    ...
    const rowsResult = await pool.query<{
      ...
      associate_rows AS (
        SELECT
          ...
          cap_amount,
          GREATEST(cap_amount - cap_remaining, 0)::numeric(18,2) AS cap_achieved,
          cap_remaining,
          CASE
  ...
  try {
    ...
    const result = await pool.query<{
      ...
      SELECT DISTINCT
        ...
        COALESCE(mc.id::text, pmc.market_center_db_id) AS market_center_db_id,
        COALESCE(mc.name, pmc.market_center_name, '') AS market_center_name,
      ...
      WHERE cl.listing_number LIKE 'KWL%'
        AND ($1::text IS NULL OR mc.source_market_center_id = $1)
        AND ($2::bigint IS NULL OR EXISTS (
...
router.get('/listings-location', resolvePermissions, async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query<{
      ...
      SELECT
        cl.listing_number,
        cl.on_market_since_date::text AS list_date,
        (CURRENT_DATE - cl.on_market_since_date)::int AS days_on_market,
        COALESCE(aa.primary_agent, pf.primary_agent, '') AS primary_agent,
        COALESCE(aa.agent_names, pf.agent_names, '') AS agent_names,
        COALESCE(mc.name, pf.market_center_name, '') AS market_center_name,
      ...
      WHERE cl.listing_number LIKE 'KWL%'
        ...
        AND (
          ...
          OR REGEXP_REPLACE(
            LOWER(TRIM(COALESCE(mc.name, pf.market_center_name, ''))),
            '[^a-z0-9]+',
            '',
            'g'
        ...
        AND ($11::text IS NULL OR mc.source_market_center_id = $11)
        AND ($12::bigint IS NULL OR EXISTS (
          SELECT 1 FROM migration.listing_agents la2
          WHERE la2.listing_id = cl.id AND la2.associate_id = $12::bigint
        ))
      ORDER BY cl.on_market_since_date DESC, cl.listing_number
      `,
      [
        listDateFrom,
        listDateTo,
