...
vi.mock('../middleware/permissions.js', () => ({
  resolvePermissions: (req: any, _res: any, next: any) => {
    req.permissions = {
      scope: 'GLOBAL',
      marketCenterId: null,
      homeMcId: null,
...
function getMonthEndHandler(): (req: any, res: any) => Promise<any> {
  const monthEndLayer = (router as any).stack.find(
    (layer: any) => layer.route?.path === '/month-end' && layer.route?.methods?.get
  );
...
describe('month-end report SQL regression guard', () => {
  ...
  it('uses per_tx_mc split CTE and overall totals from per_tx_global', async () => {
    ...
    const rowContractsSum = res.body.rows.reduce((sum: number, row: any) => sum + row.contracts, 0);
