...
export async function buildMarketingPlanDocument({
  marketingPlanData,
  formData,
  logoBuffer,
}: {
...
function buildChildren(data: MarketingPlanData, logoBuffer: Buffer | null): Array<Paragraph | Table> {
  const children: Array<Paragraph | Table> = [];

  children.push(buildLogoParagraph(logoBuffer));
  children.push(sectionHeading('1. Cover Page'));
  children.push(
    centeredTitle(data.coverPage.documentTitle || 'Marketing Plan'),
    centeredSubTitle(safe(data.coverPage.propertyAddress)),
    centeredSubTitle(`Prepared by: ${safe(data.coverPage.preparedBy)}`),
    centeredSubTitle(`Agency / Brand: ${safe(data.coverPage.agencyBrand)}`),
    centeredSubTitle(`Prepared for: ${safe(data.coverPage.preparedFor)}`),
    centeredSubTitle(`Date: ${safe(data.coverPage.datePrepared)}`)
  );
  ...
  children.push(
    tableFromRows(
      ['Metric', 'Value', 'Note'],
      data.pricingAndPositioningSummary.pricingTable.map((r) => [r.metric, r.value, r.note])
    )
  );
  ...
  children.push(
    tableFromRows(
      ['Week', 'Objective', 'Channels', 'Actions', 'Expected Outcome'],
      data.fourWeekMarketingStrategyOverview.weeklyPlan.map((r) => [r.week, r.objective, r.channels, r.actions, r.expectedOutcome])
    )
  );
  ...
  children.push(
    tableFromRows(
      ['Week', 'Platform', 'Theme', 'Visual Idea', 'Caption', 'CTA', 'Hashtags'],
      data.socialMediaAndPaidAdsPlan.posts.map((r) => [r.week, r.platform, r.theme, r.visualIdea, r.caption, r.cta, r.hashtags])
    )
  );
  children.push(subHeading('Paid Ads Concepts'));
  children.push(
    tableFromRows(
      ['Concept', 'Creative Direction', 'Ad Copy', 'Targeting'],
      data.socialMediaAndPaidAdsPlan.paidAds.map((r) => [r.conceptName, r.creativeDirection, r.adCopy, r.targeting])
    )
  );
  ...
  children.push(
    tableFromRows(
      ['Day/Week', 'Channel', 'Objective', 'Script / Message'],
      data.buyerDatabaseAndFollowUpCampaign.timeline.map((r) => [r.dayOrWeek, r.channel, r.objective, r.fullScriptOrMessage])
    )
  );
  ...
  children.push(
    tableFromRows(
      ...
      data.mediaAndAssetChecklist.map((r) => [r.item, r.owner, r.dueDate, r.status, r.notes])
    )
  );
  ...
  children.push(
    tableFromRows(
      ['Timing', 'Activity', 'Output', 'Owner'],
      data.reportingAndFeedbackSchedule.map((r) => [r.timing, r.activity, r.output, r.owner])
    )
  );
...
function centeredTitle(text: string): Paragraph {
  return new Paragraph({
    alignment: AlignmentType.CENTER,
...
function centeredSubTitle(text: string): Paragraph {
  return new Paragraph({
    alignment: AlignmentType.CENTER,
    spacing: { after: 70 },
...
function bodyParagraphs(items: string[] | null | undefined): Paragraph[] {
  const arr = (items ?? []).filter(Boolean);
  if (!arr.length) return [bodyParagraph('[CONFIRM: Add details]')];
  return arr.map((item) => bodyParagraph(item));
}

function bulletParagraphs(items: string[] | null | undefined): Paragraph[] {
  const arr = (items ?? []).filter(Boolean);
  if (!arr.length) return [bodyParagraph('[CONFIRM: Add details]')];
  return arr.map(
    (item) =>
...
function cellMargins(all: number) {
  return {
    top: all,
    bottom: all,
    left: all,
