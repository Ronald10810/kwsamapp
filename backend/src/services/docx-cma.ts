...
import type { CmaData, CmaFormData } from './openai-cma.js';
...
export async function buildCmaDocument({
  cmaData,
  formData,
  logoBuffer,
}: {
...
function buildFooter(formData: CmaFormData): Footer {
  return new Footer({
    children: [
      new Paragraph({
        alignment: AlignmentType.CENTER,
...
function buildLogoParagraph(logoBuffer: Buffer | null): Paragraph {
  if (!logoBuffer) {
...
function buildConclusionParagraphs(conclusion: CmaData['conclusion']): Paragraph[] {
  return [
    new Paragraph({
      spacing: { after: 110 },
      children: [
...
function buildCoverDetailsTable(formData: CmaFormData): Table {
...
function buildPropertyMetricsTable(formData: CmaFormData): Table {
  return new Table({
    width: { size: 100, type: WidthType.PERCENTAGE },
    layout: TableLayoutType.FIXED,
    rows: [
      new TableRow({
        children: ['Bedrooms', 'Bathrooms', 'Parking', 'Key Features'].map((label) =>
          metricHeaderCell(label)
        ),
      }),
      new TableRow({
        children: [
...
function metricHeaderCell(label: string): TableCell {
...
function metricValueCell(value: string | null | undefined, centered = false): TableCell {
  return new TableCell({
    width: { size: 2268, type: WidthType.DXA },
    borders: tableBorders(COLORS.border),
...
function toBodyParagraphs(items: string[] | null | undefined): Paragraph[] {
  const values = (items || []).filter(Boolean);
  if (!values.length) return [bodyParagraph('No additional details were available.')];
...
function toBulletParagraphs(items: string[] | null | undefined): Paragraph[] {
  const values = (items || []).filter(Boolean);
  if (!values.length) return [bodyParagraph('No additional details were available.')];
  return values.map(
    (item) =>
...
function buildThreeColumnSummaryTable(
  marketOverview: CmaData['marketOverview']
): Table {
  const cards = [
...
function buildComparableSalesTable(rows: CmaData['recentComparableSales']): Table {
  const safeRows = rows?.length
    ? rows
    : [{ address: 'Comparable sale data not confirmed', beds: '-', baths: '-', parking: '-', soldPrice: '-', soldDate: '-' }];

  return buildDataTable(
    ['Address', 'Beds', 'Baths', 'Parking', 'Sold Price', 'Sold Date'],
    [2530, 1355, 1355, 1355, 1687, 1687],
    safeRows.map((row) => [row.address, row.beds, row.baths, row.parking, row.soldPrice, row.soldDate])
  );
}

function buildCurrentListingsTable(listings: CmaData['currentMarketListings']): Table {
  const safeListings = listings?.length
    ? listings
    : [{ title: 'No active listings returned', propertyType: '-', beds: '-', baths: '-', price: '-', url: '' }];

  return buildDataTable(
    ['Portal', 'Listing', 'Beds', 'Baths', 'Price', 'Link'],
    [1355, 2530, 1355, 1355, 1355, 1687],
...
function buildPricingTable(pricingStrategy: CmaData['pricingStrategy']): Table {
  return buildDataTable(
    ['Low Estimate', 'Recommended Price', 'High Estimate'],
    [3003, 3002, 3003],
    [[pricingStrategy.lowPrice, pricingStrategy.recommendedPrice, pricingStrategy.highPrice]],
    { centeredColumns: [0, 1, 2], boldCurrencyColumns: [1] }
  );
}

type CellValue = string | { url: string; label: string };

type DataTableOptions = {
  centeredColumns?: number[];
  boldCurrencyColumns?: number[];
};

function buildDataTable(headers: string[], widths: number[], rows: CellValue[][], options: DataTableOptions = {}): Table {
        ...
        new TableRow({
          children: row.map((value, colIndex) =>
            dataBodyCell(
              ...
              widths[colIndex],
              rowIndex,
              centeredColumns.includes(colIndex),
              boldCurrencyColumns.includes(colIndex)
            )
          ),
        })
      ),
...
function dataHeaderCell(text: string, width: number): TableCell {
  return new TableCell({
...
function dataBodyCell(value: CellValue, width: number, rowIndex: number, centered = false, boldCurrency = false): TableCell {
  const fill = rowIndex % 2 === 0 ? COLORS.white : COLORS.lighter;
  const children = isLinkValue(value)
    ? [
        new Paragraph({
          alignment: centered ? AlignmentType.CENTER : AlignmentType.LEFT,
          children: (value as { url: string; label: string }).url
            ? [
                new ExternalHyperlink({
                  children: [new TextRun({ text: (value as { url: string; label: string }).label, color: COLORS.brand, underline: {} })],
                  link: (value as { url: string; label: string }).url,
                }),
              ]
            : [new TextRun({ text: '-' })],
        }),
...
function isLinkValue(value: CellValue): boolean {
  return typeof value === 'object' && value !== null && 'url' in value;
}

function detectPortal(url: string): string {
  if (!url) return '-';
  if (/property24/i.test(url)) return 'Property24';
  if (/privateproperty/i.test(url)) return 'Private Property';
  if (/pam golding|pamgolding/i.test(url)) return 'Pam Golding';
  if (/seeff/i.test(url)) return 'Seeff';
  if (/harcourts/i.test(url)) return 'Harcourts';
  return 'Portal';
}
...
function spacer(space: number): Paragraph {
...
function cellMargins(size: number) {
