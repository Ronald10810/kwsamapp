...
describe('cap refresh audit logger', () => {
  it('writes the expected audit payload shape on start', async () => {
    ...
    const auditId = await startCapRefreshAudit(db as never, {
      actor: {
        associateDbId: '123',
  ...
  it('marks success on complete', async () => {
    ...
    await completeCapRefreshAudit(db as never, '77', new Date('2026-07-08T10:00:10.000Z'), 10000);
    const call = vi.mocked(db.query).mock.calls[0];
  ...
  it('marks failure on fail path', async () => {
    const db = { query: vi.fn().mockResolvedValue({ rows: [] }) };
    await failCapRefreshAudit(db as never, '77', new Date('2026-07-08T10:00:10.000Z'), 10000, 'boom');
    const call = vi.mocked(db.query).mock.calls[0];
