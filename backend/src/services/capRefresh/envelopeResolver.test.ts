...
const {
  fetchRawRowsMock,
  groupByTransactionMock,
  buildPreCapCalculatedRowsWithMetaMock,
} = vi.hoisted(() => ({
...
function makeEntry(capProgressKey: string, cycleStartDate: string, values: { transactionAgentId: number; transactionId: number; transactionNumber?: string | null; associateId?: number | null; sourceAssociateId?: string | null; cycleEndDate?: string; isOutsideAgent?: boolean }) {
  return {
...
describe('cap refresh envelope resolver', () => {
  it('resolves associate envelope from target transaction rows', async () => {
    ...
    const result = await resolveCapRefreshContext({ query: vi.fn() } as never, 1001);
  ...
  it('resolves team envelope from target transaction rows', async () => {
    ...
    const result = await resolveCapRefreshContext({ query: vi.fn() } as never, 2001);
