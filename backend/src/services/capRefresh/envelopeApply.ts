...
function filterRowsForEnvelope(rows: CapRefreshDiffRow[], envelopeKey: string): CapRefreshDiffRow[] {
  return rows.filter((row) => row.envelopeKey === envelopeKey);
}

function buildBeforeSnapshot(rows: CapRefreshDiffRow[]) {
  return rows.map((row) => ({
...
function buildAfterSnapshot(rows: CapRefreshDiffRow[]) {
  return rows.map((row) => ({
    transactionAgentId: row.transactionAgentId,
    transactionId: row.transactionId,
    transactionNumber: row.transactionNumber,
...
export async function acquireEnvelopeLocks(db: Queryable, envelopeKeys: string[]): Promise<void> {
...
export async function persistEnvelopeChanges(db: Queryable, rows: CapRefreshDiffRow[]): Promise<void> {
...
export async function dryRunCapRefresh(db: Queryable, transactionId: number): Promise<CapRefreshDryRunResult> {
  const context = await resolveCapRefreshContext(db, transactionId);
  const currentByAgentId = await fetchCurrentEnvelopeTacRows(
    db,
    context.envelopeEntries.map((entry) => entry.row.transaction_agent_id)
  );
...
export async function applyCapRefresh(db: Queryable, actor: CapRefreshActor, transactionId: number): Promise<CapRefreshApplyResult> {
  const context = await resolveCapRefreshContext(db, transactionId);
