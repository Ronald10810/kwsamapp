...
export interface MarketingPlanData {
  ...
  propertySnapshot: {
    summary: string[];
    keyFacts: Array<{ field: string; value: string }>;
  };
  pricingAndPositioningSummary: {
    summaryParagraphs: string[];
    pricingTable: Array<{ metric: string; value: string; note: string }>;
  };
  uniqueSellingPointsAndTargetBuyerProfiles: {
    uniqueSellingPoints: string[];
    targetBuyerProfiles: Array<{ profileName: string; profileSummary: string; matchingFeatures: string[] }>;
    messagingAngles: string[];
  };
  fourWeekMarketingStrategyOverview: {
    strategySummary: string[];
    weeklyPlan: Array<{ week: string; objective: string; channels: string; actions: string; expectedOutcome: string }>;
  };
  onlineListingCopy: {
  ...
  socialMediaAndPaidAdsPlan: {
    ...
    posts: Array<{ week: string; platform: string; theme: string; visualIdea: string; caption: string; cta: string; hashtags: string }>;
    paidAds: Array<{ conceptName: string; creativeDirection: string; adCopy: string; targeting: string }>;
  };
  buyerDatabaseAndFollowUpCampaign: {
    targetingApproach: string[];
    timeline: Array<{ dayOrWeek: string; channel: string; objective: string; fullScriptOrMessage: string }>;
    complianceNotes: string[];
  };
  mediaAndAssetChecklist: Array<{ item: string; owner: string; dueDate: string; status: string; notes: string }>;
  reportingAndFeedbackSchedule: Array<{ timing: string; activity: string; output: string; owner: string }>;
...
export async function generateMarketingPlanData({
  formData,
  cmaBuffer,
  cmaOriginalName,
  loomReportBuffer,
  ...
  return JSON.parse(outputText) as MarketingPlanData;
}
...
function orConfirm(value: string, label: string): string {
  const v = value?.trim();
  return v ? v : `[CONFIRM: ${label}]`;
}

async function extractDocumentText(buffer: Buffer, originalName: string): Promise<string> {
