...
export function getPublicReadOnlyPgPool() {
    ...
    const rejectUnauthorized = parseBoolean(process.env.PUBLIC_DB_SSL_REJECT_UNAUTHORIZED, defaultRejectUnauthorized(normalized.sslMode, env.isDevelopment));
...
export async function closePublicReadOnlyPgPool() {
    if (!publicReadOnlyPool)
        return;
