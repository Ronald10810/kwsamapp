...
function parseDatabaseClient(value) {
    const normalized = normalizeString(value)?.toLowerCase();
...
function assertLocalUatDbTarget(databaseUrl) {
    let parsed;
    try {
...
const localGoogleClientIdFallback = nodeEnv === 'development'
...
export function getRequiredDatabaseUrl() {
    if (!env.database.url) {
        throw new Error('Database configuration is missing. Set DATABASE_URL, or DB_HOST/DB_PORT/DB_NAME/DB_USER/DB_PASSWORD.');
    }
    if (env.database.client !== 'postgres') {
        throw new Error(`DB_CLIENT=${env.database.client} is not supported by the current backend runtime. The current implementation requires PostgreSQL-compatible drivers and SQL.`);
    }
