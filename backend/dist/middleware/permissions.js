 ...
 */
export async function resolvePermissions(req, res, next) {
    if (!req.user) {
    ...
    try {
        ...
        const assocResult = assocId ? await pool.query(`SELECT id::text, source_market_center_id
        ...
        const [rolesResult, adminMcsResult] = await Promise.all([
            ...
            pool.query(`SELECT source_market_center_id
           FROM migration.associate_admin_market_centers
          WHERE associate_id = $1`, [assoc.id]),
        ]);
        ...
        if (isRegionalAdmin && (activeContextId === 'regional_admin' || !activeContextId)) {
            ...
            const validOfficeAdmin = roles.includes('OFFICE_ADMIN') && claimedMcId === assoc.source_market_center_id;
            if (!validOfficeAdmin) {
