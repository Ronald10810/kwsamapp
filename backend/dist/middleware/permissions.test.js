...
function resolveScopeLogic(input) {
...
function buildPermissionHelpers(activeContext, userEmail) {
    ...
    const canEditListing = (listingMcId, primaryAgentEmail) => {
        if (isRegionalAdmin)
            return true;
        if (isOfficeAdmin)
            return listingMcId !== null && listingMcId === activeMcId;
    ...
    const canEditAssociate = (assocMcId, assocEmail) => {
        if (isRegionalAdmin)
            return true;
        if (isOfficeAdmin)
            return assocMcId !== null && assocMcId === activeMcId;
        // Agent: can only edit own record
        return !!(userEmail && assocEmail && userEmail.toLowerCase() === assocEmail.toLowerCase());
    };
    const canEditMarketCenter = (mcSourceId) => {
        if (isRegionalAdmin)
            return true;
        if (isOfficeAdmin)
            return mcSourceId !== null && mcSourceId === activeMcId;
        return false;
    };
    return {
        canCreateListing,
