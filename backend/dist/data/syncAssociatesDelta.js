...
function groupedStringMap(rows) {
...
function emptyEnrichment() {
    return {
        ...
        roles: new Map(),
        jobTitles: new Map(),
        serviceCommunities: new Map(),
        adminMarketCenters: new Map(),
        adminTeams: new Map(),
    };
}
...
async function fetchAssociateEnrichment(az, ids) {
    if (ids.length === 0)
  ...
  `);
    return {
        business: new Map(businessResult.recordset.map((row) => [row.AssociateId, row])),
        commissions: new Map(commissionResult.recordset.map((row) => [row.AssociateId, row])),
        dates: new Map(dateResult.recordset.map((row) => [row.AssociateId, row])),
        thirdParty: new Map(thirdPartyResult.recordset.map((row) => [row.AssociateId, row])),
        roles: groupedStringMap(roleResult.recordset),
        jobTitles: groupedStringMap(jobTitleResult.recordset),
        serviceCommunities: groupedStringMap(communityResult.recordset),
        adminMarketCenters: groupedStringMap(adminMcResult.recordset),
        adminTeams: groupedStringMap(adminTeamResult.recordset),
    };
}
...
async function loadCheckpoint(client, syncName) {
    const result = await client.query(`SELECT last_source_updated_at::text AS last_source_updated_at, last_source_associate_id::text AS last_source_associate_id
     FROM migration.associate_sync_state
     WHERE sync_name = $1
     LIMIT 1`, [syncName]);
    return result.rows[0] ?? null;
}
async function saveCheckpoint(client, syncName, updatedAt, associateId, runCount) {
    await client.query(`INSERT INTO migration.associate_sync_state (
     ...
     ON CONFLICT (sync_name) DO UPDATE SET
       ...
       last_run_count = EXCLUDED.last_run_count`, [syncName, updatedAt, associateId, runCount]);
}
async function loadExistingAssociates(pg, sourceIds) {
    if (sourceIds.length === 0)
        return new Map();
    const result = await pg.query(`SELECT source_associate_id, first_name, last_name, full_name, national_id, ffc_number, status_name
     FROM migration.core_associates
     WHERE source_associate_id = ANY($1::text[])`, [sourceIds]);
    return new Map(result.rows.map((row) => [row.source_associate_id, row]));
}
async function syncAssociateCollections(pg, associateId, row, enrichment) {
    ...
    for (const community of communities) {
        await pg.query(`INSERT INTO migration.associate_service_communities (associate_id, community_name) VALUES ($1, $2)`, [associateId, community]);
    }
    for (const sourceMarketCenterId of adminMarketCenters) {
        await pg.query(`INSERT INTO migration.associate_admin_market_centers (associate_id, source_market_center_id) VALUES ($1, $2)`, [associateId, sourceMarketCenterId]);
    }
    for (const sourceTeamId of adminTeams) {
        await pg.query(`INSERT INTO migration.associate_admin_teams (associate_id, source_team_id) VALUES ($1, $2)`, [associateId, sourceTeamId]);
    }
}
async function upsertAssociate(pg, row, enrichment) {
    ...
    const upsert = await pg.query(`INSERT INTO migration.core_associates (
    ...
    await pg.query(`INSERT INTO migration.id_map_associates (source_associate_id, core_associate_id, mapped_at)
     VALUES ($1, $2, NOW())
     ON CONFLICT (source_associate_id)
     DO UPDATE SET core_associate_id = EXCLUDED.core_associate_id, mapped_at = NOW()`, [sourceAssociateId, associateId]);
    return associateId;
}
async function main() {
    ...
    `;
        ...
        for (const row of rows) {
            ...
            const profileChanged = incomingFirstName !== normalizeText(current.first_name) ||
...
//# sourceMappingURL=syncAssociatesDelta.js.map