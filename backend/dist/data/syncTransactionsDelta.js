...
async function loadCheckpoint(client, syncName) {
    const result = await client.query(`SELECT last_source_updated_at::text AS last_source_updated_at, last_source_transaction_id::text AS last_source_transaction_id
     FROM migration.transaction_sync_state
     WHERE sync_name = $1
     LIMIT 1`, [syncName]);
    return result.rows[0] ?? null;
}
async function saveCheckpoint(client, syncName, updatedAt, transactionId, runCount) {
    await client.query(`INSERT INTO migration.transaction_sync_state (
     ...
     ON CONFLICT (sync_name) DO UPDATE SET
       ...
       last_run_count = EXCLUDED.last_run_count`, [syncName, updatedAt, transactionId, runCount]);
}
function runOnePassTransactionImport(transactionId) {
    const result = spawnSync(process.execPath, [resolve(backendRoot, 'scripts', 'one-pass-transaction-import.cjs'), String(transactionId)], {
        cwd: backendRoot,
        env: {
            ...process.env,
...
function runTransactionCalculationBackfill() {
    const npmCmd = process.platform === 'win32' ? 'npm.cmd' : 'npm';
    let result = spawnSync(npmCmd, ['run', 'data:recalculate:transaction-calculations'], {
        cwd: backendRoot,
        env: {
            ...process.env,
            PGHOST: String(PG.host),
    ...
    if (result.error && process.platform === 'win32') {
        result = spawnSync('cmd.exe', ['/d', '/s', '/c', 'npm.cmd run data:recalculate:transaction-calculations'], {
            cwd: backendRoot,
            env: {
                ...process.env,
                PGHOST: String(PG.host),
...
async function main() {
    ...
    `;
        ...
        if (delta.recordset.length > 0) {
            const preview = delta.recordset.slice(0, 10).map((row) => ({
        ...
        for (const row of delta.recordset) {
...
//# sourceMappingURL=syncTransactionsDelta.js.map