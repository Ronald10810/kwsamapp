...
async function main() {
    ...
    `);
        ...
        cols.rows.forEach((r) => console.log(`  ${r.column_name.padEnd(45)} ${r.data_type}`));
        // ── 6. Sample row ─────────────────────────────────────────────────────────
    ...
    `);
        ...
        keys.rows.forEach((r) => console.log(`  ${r.key.padEnd(40)} ${r.frequency}`));
    }
    finally {
        client.release();
        await pool.end();
...
//# sourceMappingURL=_auditListings.js.map