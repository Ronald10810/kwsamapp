...
async function main() {
    const report = await withClient(async (client) => {
        const totalResult = await client.query('SELECT COUNT(*)::text AS count FROM migration.transaction_agent_calculations');
        const mcZeroMismatchResult = await client.query(`
    ...
    `);
        const statusBreakdownResult = await client.query(`
...
//# sourceMappingURL=verifyTransactionAgentCalculations.js.map