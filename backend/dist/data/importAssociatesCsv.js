...
function buildPayload(row) {
    ...
    return {
        ...
        kwuid,
        sourceUpdatedAt,
        rawPayload: JSON.stringify(extendedPayload),
    };
}
async function main() {
    const filePath = optionalArg('--file', 'data/incoming/associates.csv');
    const batchId = optionalArg('--batch', buildBatchId());
    const chunkSize = parseChunkSize(optionalArg('--chunk-size', '500'));
    const rows = await readCsvRows(filePath);
    if (rows.length === 0) {
        throw new Error(`No rows found in ${filePath}`);
    }
    const mappedRows = rows
        .map((row) => buildPayload(row))
        .filter((row) => Boolean(row));
    if (mappedRows.length === 0) {
        throw new Error(`No valid associate rows found in ${filePath}`);
    }
    await runInTransaction(async (client) => {
        await client.query('DELETE FROM staging.associates_raw WHERE batch_id = $1', [batchId]);
    });
    const rowChunks = chunk(mappedRows, chunkSize);
    for (let chunkIndex = 0; chunkIndex < rowChunks.length; chunkIndex += 1) {
        const rowsForChunk = rowChunks[chunkIndex];
        await runInTransaction(async (client) => {
        ...
        `, [
                    batchId,
                    row.sourceAssociateId,
                    row.firstName,
                    row.lastName,
                    row.email,
                    row.statusName,
                    row.marketCenterName,
                    row.teamName,
                    row.kwuid,
                    row.sourceUpdatedAt,
                    row.rawPayload,
        ...
        console.log(`Imported chunk ${chunkIndex + 1}/${rowChunks.length} (${Math.min((chunkIndex + 1) * chunkSize, mappedRows.length)}/${mappedRows.length}).`);
    }
    console.log(`Imported ${mappedRows.length} rows into staging.associates_raw (batch: ${batchId}).`);
}
main()
    .catch((error) => {
