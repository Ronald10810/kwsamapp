...
async function main() {
    ...
    const { rows: txCount } = await withClient((client) => client.query(`SELECT COUNT(*) AS cnt FROM migration.core_transactions`));
    const { rows: agentCount } = await withClient((client) => client.query(`SELECT COUNT(*) AS cnt FROM migration.transaction_agents`));
