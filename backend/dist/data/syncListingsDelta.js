...
async function loadCheckpoint(client, syncName) {
    const result = await client.query(`SELECT last_source_updated_at::text AS last_source_updated_at, last_source_listing_id::text AS last_source_listing_id
     FROM migration.listing_sync_state
     WHERE sync_name = $1
     LIMIT 1`, [syncName]);
    return result.rows[0] ?? null;
}
async function saveCheckpoint(client, syncName, updatedAt, listingId, runCount) {
    await client.query(`INSERT INTO migration.listing_sync_state (
     ...
     ON CONFLICT (sync_name) DO UPDATE SET
       ...
       last_run_count = EXCLUDED.last_run_count`, [syncName, updatedAt, listingId, runCount]);
}
function runOnePassImport(listingNumber) {
    const result = spawnSync(process.execPath, [resolve(backendRoot, 'scripts', 'one-pass-listing-import.cjs'), listingNumber], {
        cwd: backendRoot,
        env: {
            ...process.env,
...
//# sourceMappingURL=syncListingsDelta.js.map