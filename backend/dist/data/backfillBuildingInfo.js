...
async function main() {
    ...
    const rows = [];
    const parser = createReadStream(filePath, { encoding: csvEncoding }).pipe(parse({
        columns: true,
    ...
    `);
        ...
        for (let i = 0; i < uniqueRows.length; i += CHUNK) {
            ...
            await client.query(`INSERT INTO _building_backfill (source_listing_id, erf_size, floor_area) VALUES ${values}`, params);
        }
...
//# sourceMappingURL=backfillBuildingInfo.js.map