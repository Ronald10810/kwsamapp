...
import { isRegisteredAssociateEmail, normalizeAuthEmail, normalizeAuthName, resolveAssociateIdForAuth } from './associateAuth.js';
    ...
    describe('isRegisteredAssociateEmail', () => {
        ...
        it('returns false when email is blank, even when name is provided', async () => {
            ...
            const client = {
                query: async () => {
                    queryCalled = true;
                    return { rowCount: 1, rows: [{ id: '1999' }] };
                },
            };
        ...
        it('prefers active records and supports all email fields', async () => {
            const client = {
                query: async (sql, values) => {
                    ...
                    expect(sql).toContain("COALESCE(a.email, '')");
                    expect(sql).toContain("COALESCE(a.private_email, '')");
                    return { rowCount: 1, rows: [{ id: '29496' }] };
                },
            };
...
//# sourceMappingURL=associateAuth.test.js.map