...
let listingAreaDecimalEnsurePromise = null;
...
function normalizeStreetLikePart(value) {
...
async function generateNextValidationCode(db) {
    const result = await db.query(`SELECT COALESCE(MAX((substring(COALESCE(cl.listing_payload->'listing_validation'->>'validationCode', '') FROM '^KWV([0-9]+)$'))::int), 1000) AS max_code
       FROM migration.core_listings cl`);
    const next = (result.rows[0]?.max_code ?? 1000) + 1;
    return `KWV${next}`;
}
async function ensureValidationCode(db, listingId, preferredCode) {
    if (isValidationCode(preferredCode)) {
        const existing = await db.query(`SELECT id::text
         FROM migration.core_listings
        WHERE id <> COALESCE($1::int, -1)
          AND COALESCE(listing_payload->'listing_validation'->>'validationCode', '') = $2
        LIMIT 1`, [listingId, preferredCode]);
...
function deriveApprovedListingStatusTag(saleOrRentRaw, currentTagRaw, preSubmitTagRaw) {
    const saleOrRent = (toText(saleOrRentRaw) ?? '').toLowerCase().trim();
    if (saleOrRent.includes('rent'))
        return 'To Rent';
    if (saleOrRent.includes('sale'))
        return 'For Sale';
...
function mapKwwPropertyType(rawType, rawSubType) {
    ...
    if (sub.includes('apartment') || sub.includes('flat'))
        return { propType: 'Residential', propTypeId: 8, propSubtypeId: 1 };
    if (sub.includes('cluster'))
        return { propType: 'Residential', propTypeId: 8, propSubtypeId: 3 };
    if (sub.includes('duplex'))
        return { propType: 'Residential', propTypeId: 8, propSubtypeId: 6 };
    if (sub.includes('simplex'))
        return { propType: 'Residential', propTypeId: 8, propSubtypeId: 17 };
    if (sub.includes('townhouse') || sub.includes('town house'))
        return { propType: 'Residential', propTypeId: 8, propSubtypeId: 18 };
    if (sub.includes('house') || sub.includes('single family') || sub.includes('freehold'))
        return { propType: 'Residential', propTypeId: 8, propSubtypeId: 15 };
    return { propType: 'Residential', propTypeId: 8, propSubtypeId: 32 };
}
function mapKwwListType(rawMandate, propTypeId) {
...
function mapKwwParkingFeatures(values) {
...
function parseImageUrls(value) {
    const dedupe = (entries) => [...new Set(entries)];
...
async function ensureRentalOptionalColumns() {
    if (!pool || rentalOptionalColumnsEnsured)
        return;
    await pool.query(`
...
function toShowDateTime(dateValue, timeValue, fallbackTime) {
...
async function loadListingShowWindows(pg, listingId) {
    const [showTimesResult, openHouseResult] = await Promise.all([
        pg.query(`SELECT from_date::text, from_time, to_date::text, to_time, sort_order
         FROM migration.listing_show_times
        WHERE listing_id = $1
        ORDER BY sort_order ASC, id ASC`, [listingId]),
        pg.query(`SELECT open_house_date::text, from_time, to_time, sort_order
...
function appendShowWindowSummary(description, windows) {
    const base = description.trim();
    if (windows.length === 0)
        return base;
    if (/Viewing Schedule:/i.test(base) || /ON SHOW/i.test(base))
        return base;
...
function toPpDateTimeValue(value) {
...
function buildPpShowdayEventsXml(propertyId, windows) {
    if (windows.length === 0)
        return '';
    const items = windows
        .map((window) => {
...
async function resolveListingImageUrls(pg, listingId, rawValue) {
    const direct = parseImageUrls(rawValue);
    if (direct.length > 0)
        return direct;
    const fallback = await pg.query(`SELECT file_url
     FROM migration.listing_images
     WHERE listing_id = $1
       AND COALESCE(TRIM(file_url), '') <> ''
     ORDER BY sort_order ASC, id ASC`, [listingId]);
    return [...new Set(fallback.rows
...
function extractMatterportSpaceIdFromUrl(value) {
    const raw = (value ?? '').trim();
    if (!raw)
        return null;
    try {
...
function pickPortalMediaFromMarketingUrls(marketingUrls) {
...
async function optimizePhotoForProperty24(sourceBuffer, mimeType) {
...
async function selectPhotosForProperty24(urls) {
    ...
    for (const url of validUrls) {
        ...
        if (!candidate) {
            ...
            if (warnings.length < 5)
                warnings.push(`Skipped unreadable/oversized image: ${url}`);
            continue;
        }
        if (totalBytes + candidate.byteLength > PROPERTY24_MAX_TOTAL_IMAGE_BYTES) {
            skippedCount += 1;
            if (warnings.length < 5)
                warnings.push(`Skipped to stay under Property24 payload budget: ${url}`);
            continue;
        }
...
function getRequestBaseUrl(req) {
    if (env.uploadsPublicBaseUrl)
        return env.uploadsPublicBaseUrl.replace(/\/$/, '');
...
async function resolveExternalImageUrls(urls, req) {
...
async function ensureListingAreaCountSupportsDecimals(pg) {
    if (listingAreaDecimalEnsurePromise) {
        await listingAreaDecimalEnsurePromise;
        return;
    }
    listingAreaDecimalEnsurePromise = (async () => {
        const columnResult = await pg.query(`SELECT data_type, udt_name, numeric_scale
       ...
       LIMIT 1`);
        ...
        const isIntegerLike = normalizedDataType === 'smallint'
...
const LEGACY_PROPERTY_AREA_TYPE_BY_ID = {
    '1': 'Bedroom',
    '2': 'Bathroom',
    '3': 'Bar',
    '4': 'Closet',
...
function normalizeListingPropertyAreas(areaRows, featureRows) {
    const legacyAreaFeatures = new Map();
    for (const feature of featureRows) {
        const areaType = normalizeAreaType(feature.feature_category);
        const featureValue = toText(feature.feature_value);
        if (!areaType || !featureValue)
            continue;
        if (!LEGACY_PROPERTY_AREA_TYPE_BY_ID[String(Number(feature.feature_category ?? ''))])
            continue;
        if (!legacyAreaFeatures.has(areaType))
            legacyAreaFeatures.set(areaType, new Set());
    ...
    for (const [index, area] of areaRows.entries()) {
        ...
        if (!existing.size) {
            ...
            if (sizeText)
                existing.size = sizeText;
        }
        if (!existing.description) {
            existing.description = toText(area.description);
        }
        for (const entry of parseTextArray(area.sub_features))
            existing.subFeatures.add(entry);
    ...
    for (const [areaType, values] of legacyAreaFeatures.entries()) {
        ...
        for (const value of values)
            existing.subFeatures.add(value);
    }
...
function normalizeListingFeatures(featureRows) {
    const result = [];
    for (const feature of featureRows) {
...
async function storeUploadedFiles(files, subdir = 'listings') {
    const storeUploadedFilesLocally = async () => {
    ...
    const canFallbackToLocalUploads = () => {
        if (env.isProduction)
            return false;
        return true;
    };
    const isRecoverableLocalDevGcsError = (message) => {
...
async function generateNextListingNumber() {
    ...
    try {
        ...
        const maxResult = await client.query(`SELECT MAX(CAST(SUBSTRING(listing_number FROM 4) AS BIGINT))::text AS max_num
       FROM migration.core_listings
       WHERE listing_number ~ '^KWL[0-9]+$'`);
        const maxKwl = Number(maxResult.rows[0]?.max_num ?? 8999);
        await client.query(`SELECT setval('app.kwl_listing_number_seq', GREATEST(
         COALESCE((SELECT last_value FROM app.kwl_listing_number_seq), 8999),
         $1::bigint
       ), true)`, [maxKwl]);
        const nextResult = await client.query(`SELECT nextval('app.kwl_listing_number_seq')::text AS next_num`);
...
function isCanonicalKwlListingNumber(value) {
...
router.post('/validate-new-listing', resolvePermissions, async (req, res) => {
    ...
    `;
        const conflictCandidates = await pool.query(conflictSql, [candidateId ?? null, listingCategory]);
        ...
        if (sellerId) {
            const sellerConflictCandidates = await pool.query(`SELECT cl.id::text AS listing_id,
                cl.street_number,
                cl.street_name,
                cl.unit_number,
                cl.door_number,
         ...
         WHERE cl.id <> COALESCE($1::int, -1)
           ...
           AND COALESCE(cl.is_published, false) = true
           AND COALESCE(cl.is_draft, false) = false
           AND LOWER(TRIM(COALESCE(cl.listing_payload->'listing_validation'->>'sellerId', ''))) = $2`, [candidateId ?? null, sellerId]);
...
router.get('/', resolvePermissions, async (req, res) => {
    ...
    try {
        ...
        let permsMarketCenterDbId = null;
        if (perms.scope === 'MARKET_CENTRE' && perms.marketCenterId) {
            const mcResult = await pool.query(`SELECT id::text FROM migration.core_market_centers WHERE source_market_center_id = $1 LIMIT 1`, [perms.marketCenterId]);
        ...
        const areaSearchTerms = searchInput.includes(',')
            ? [...new Set(searchInput
                    ...
                    .filter((term) => term.length > 0))].slice(0, 5)
            : [];
        ...
        const dataResult = await pool.query(`SELECT id::text, source_listing_id, source_market_center_id, market_center_id::text, listing_number, status_name, listing_status_tag,
       ...
       LIMIT ${limitParam} OFFSET ${offsetParam}`, params);
        ...
        const parsedImageUrlsById = new Map();
        const rowsMissingImageUrls = [];
        ...
        const fallbackImageUrlsById = new Map();
        if (rowsMissingImageUrls.length > 0) {
            const fallbackImagesResult = await pool.query(`SELECT listing_id::text, file_url
           FROM migration.listing_images
          WHERE listing_id = ANY($1::int[])
            AND COALESCE(TRIM(file_url), '') <> ''
          ORDER BY listing_id ASC, sort_order ASC, id ASC`, [rowsMissingImageUrls]);
        ...
        let ownEditableIds = new Set();
        if (perms.scope === 'OWN' && perms.associateDbId && rowIds.length > 0) {
            const ownEditRows = await pool.query(`SELECT DISTINCT listing_id::text
           FROM migration.listing_agents
          WHERE associate_id = $1
            AND listing_id = ANY($2::int[])`, [perms.associateDbId, rowIds]);
            ownEditableIds = new Set(ownEditRows.rows.map((r) => Number(r.listing_id)).filter((id) => Number.isFinite(id)));
        }
        let marketCentreEditableIds = new Set();
        if (perms.scope === 'MARKET_CENTRE' && rowIds.length > 0) {
            if (permsMarketCenterDbId !== null) {
                const mcEditRows = await pool.query(`SELECT DISTINCT cl.id::text AS listing_id
             FROM migration.core_listings cl
...
router.get('/:id', async (req, res) => {
    ...
    try {
        ...
        const optionalColumnsResult = await pool.query(`SELECT column_name
         FROM information_schema.columns
        WHERE table_schema = 'migration'
          AND table_name = 'core_listings'
          AND column_name = ANY($1::text[])`, [['rental_rate', 'lease_period', 'deposit_requirements']]);
       ...
       FROM migration.core_listings WHERE id = $1 LIMIT 1`, [id]);
        ...
        let mcEntegralPortals = [];
        if (mcId) {
            const mcPortalRes = await pool.query(`SELECT entegral_portals FROM migration.core_market_centers WHERE id::text = $1 LIMIT 1`, [mcId]);
        ...
        if (agentRows.length === 0) {
            const fallbackAgents = await pool.query(`SELECT
           la.id::text,
         ...
         ORDER BY la."isPrimary" DESC, la.id`, [id]);
        ...
        if (!row.signed_date && row.listing_payload) {
            ...
            if (payloadDate) {
                ...
                if (m)
                    row.signed_date = `${m[1]}-${m[2]}-${m[3]}`;
            }
        }
        const normalizedFeatures = normalizeListingFeatures(features.rows);
        const normalizedAreas = normalizeListingPropertyAreas(areas.rows, features.rows);
        return res.json({
            ...row,
            ...
            marketing_urls: marketingUrls.rows.map((row) => normalizeMarketingUrlRecord(row)),
...
router.post('/', resolvePermissions, async (req, res) => {
    ...
    if (perms.scope === 'OWN') {
        ...
        if (!toText(b.source_market_center_id)) {
            const mcRes = await pool.query(`SELECT mc.source_market_center_id
         FROM migration.core_associates a
         JOIN migration.core_market_centers mc ON mc.id = a.market_center_id
         WHERE a.id = $1 LIMIT 1`, [perms.associateDbId]);
            if (mcRes.rows[0]?.source_market_center_id) {
                b.source_market_center_id = mcRes.rows[0].source_market_center_id;
            }
        }
    }
...
router.put('/:id', resolvePermissions, async (req, res) => {
    ...
    const perms = req.permissions;
    if (perms.scope !== 'GLOBAL') {
        const existing = await pool.query(`SELECT source_market_center_id, market_center_id::text FROM migration.core_listings WHERE id = $1 LIMIT 1`, [id]);
        if (!existing.rows[0])
            return res.status(404).json({ error: 'Listing not found.' });
        ...
        if (perms.scope === 'MARKET_CENTRE') {
            ...
            let permsMcDbId = null;
            if (perms.marketCenterId) {
                const mc = await pool.query(`SELECT id::text FROM migration.core_market_centers WHERE source_market_center_id = $1 LIMIT 1`, [perms.marketCenterId]);
            ...
            if (!directMatch) {
                const linked = await pool.query(`SELECT 1
             FROM migration.listing_agents la
             LEFT JOIN migration.core_associates a ON a.id = la.associate_id
            ...
            LIMIT 1`, [id, perms.marketCenterId ?? '', permsMcDbId]);
            ...
            const isAgent = await pool.query(`SELECT 1 FROM migration.listing_agents la WHERE la.listing_id = $1 AND la.associate_id = $2 LIMIT 1`, [id, perms.associateDbId]);
    ...
    let listingNumber = incomingListingNumber;
    if (!isCanonicalKwlListingNumber(listingNumber)) {
        const existingListingNumber = await pool.query(`SELECT listing_number FROM migration.core_listings WHERE id = $1 LIMIT 1`, [id]);
    ...
    try {
       ...
       WHERE id=$1 RETURNING id::text`, [id, ...params.slice(1)]);
        ...
        if (perms.scope !== 'OWN' && isPublished && !isWithdrawOrInactive) {
            const approvalRes = await pool.query(`SELECT submitted_by_associate_id, status
          ...
          LIMIT 1`, [id]);
            if (approvalRes.rows.length > 0) {
                ...
                if (perms.associateDbId) {
                    const reviewer = await pool.query(`SELECT full_name FROM migration.core_associates WHERE id = $1 LIMIT 1`, [perms.associateDbId]);
                    reviewerName = reviewer.rows[0]?.full_name ?? 'Admin';
                }
                if (approvalStatus !== 'APPROVED') {
                    await pool.query(`UPDATE migration.listing_approval_requests
                SET status = 'APPROVED',
                    reviewed_by_associate_id = $2,
                    reviewed_by_name = $3,
                    reviewed_at = NOW(),
                    updated_at = NOW()
              WHERE listing_id = $1`, [id, perms.associateDbId ?? null, reviewerName]);
                }
              ...
              SET is_draft = false,
                  ...
                  END,
                  updated_at = NOW()
            WHERE id = $1`, [id, approvedListingStatusTag]);
                await pool.query(`UPDATE migration.in_app_notifications
            ...
            WHERE entity_type = 'listing'
              AND entity_id = $1
              AND notification_type = 'LISTING_APPROVAL_REQUESTED'`, [id]).catch(() => undefined);
                if (approval.submitted_by_associate_id) {
                    try {
                        await createNotification(pool, approval.submitted_by_associate_id, 'LISTING_APPROVAL_APPROVED', 'APPROVED', `Listing approved: ${listingNumber ?? id}`, `Your listing ${listingNumber ?? id} has been approved and published by ${reviewerName}.`, 'listing', id, { listing_id: id, listing_number: listingNumber, reviewer_name: reviewerName, status: 'APPROVED' });
                    }
                    catch {
...
async function saveSubTables(pg, listingId, b) {
    if (Array.isArray(b.agents)) {
        ...
        for (const [i, agent] of b.agents.entries()) {
         ...
         VALUES ($1,$2,$3,$4,$5,$6,$7)`, [listingId, toNumber(agent.associate_id), toText(agent.agent_name), toText(agent.agent_role) ?? 'Primary', toBool(agent.is_primary), toNumber(agent.market_center_id), toInteger(agent.sort_order, i)]);
        }
    }
    if (Array.isArray(b.contacts)) {
        ...
        for (const [i, c] of b.contacts.entries()) {
            await pg.query(`INSERT INTO migration.listing_contacts (listing_id, full_name, phone_number, email_address, sort_order) VALUES ($1,$2,$3,$4,$5)`, [listingId, toText(c.full_name), toText(c.phone_number), toText(c.email_address), toInteger(c.sort_order, i)]);
        }
    }
    if (Array.isArray(b.show_times)) {
        ...
        for (const [i, st] of b.show_times.entries()) {
         ...
         VALUES ($1,$2::date,$3,$4::date,$5,$6,$7)`, [listingId, toDateValue(st.from_date), toText(st.from_time), toDateValue(st.to_date), toText(st.to_time), toText(st.catch_phrase), toInteger(st.sort_order, i)]);
        }
    }
    if (Array.isArray(b.open_house)) {
        ...
        for (const [i, oh] of b.open_house.entries()) {
         ...
         VALUES ($1,$2::date,$3,$4,$5,$6,$7)`, [listingId, toDateValue(oh.open_house_date), toText(oh.from_time), toText(oh.to_time), toText(oh.average_price), toText(oh.comments), toInteger(oh.sort_order, i)]);
        }
    }
    if (Array.isArray(b.marketing_urls)) {
        ...
        for (const [i, mu] of b.marketing_urls.entries()) {
            ...
            if (!toText(normalizedUrl))
                continue;
            await pg.query(`INSERT INTO migration.listing_marketing_urls (listing_id, url, url_type, display_name, sort_order) VALUES ($1,$2,$3,$4,$5)`, [listingId, normalizedUrl, normalizedUrlType, toText(mu.display_name), toInteger(mu.sort_order, i)]);
        }
    }
    if (Array.isArray(b.features)) {
        ...
        for (const [i, f] of b.features.entries()) {
            if (!toText(f.feature_category) || !toText(f.feature_value))
                continue;
            await pg.query(`INSERT INTO migration.listing_features (listing_id, feature_category, feature_value, sort_order) VALUES ($1,$2,$3,$4)`, [listingId, toText(f.feature_category), toText(f.feature_value), toInteger(f.sort_order, i)]);
        }
    }
    if (Array.isArray(b.property_areas)) {
        ...
        for (const [i, pa] of b.property_areas.entries()) {
            ...
            await pg.query(`INSERT INTO migration.listing_property_areas (listing_id, area_type, count, size, description, sub_features, sort_order) VALUES ($1,$2,$3,$4::numeric,$5,$6::jsonb,$7)`, [
                listingId,
                toText(pa.area_type),
                areaCount,
                toNumber(pa.size),
    ...
    if (Array.isArray(b.normalized_images)) {
        ...
        for (const [i, img] of b.normalized_images.entries()) {
            if (!toText(img.file_url))
                continue;
            await pg.query(`INSERT INTO migration.listing_images (listing_id, file_name, file_url, media_type, sort_order, uploaded_by) VALUES ($1,$2,$3,$4,$5,$6)`, [listingId, toText(img.file_name), toText(img.file_url), toText(img.media_type) ?? 'image', toInteger(img.sort_order, i), toText(img.uploaded_by)]);
        }
    }
}
// ---------------------------------------------------------------------------
...
router.delete('/:id/mandate-documents/:docId', async (req, res) => {
    ...
    try {
        const result = await pool.query(`DELETE FROM migration.listing_mandate_documents WHERE id = $1 AND listing_id = $2 RETURNING id`, [docId, id]);
        if (result.rowCount === 0)
...
// ---------------------------------------------------------------------------
function mapListingTypeToProperty24(value) {
    const normalized = (toText(value) ?? '').toLowerCase().trim();
    return normalized.includes('rent') ? 'Rental' : 'Sale';
}
function mapPropertyTypeIdForProperty24(propertyType, propertySubType) {
...
function mapPetsAllowedForProperty24(value) {
    return toBool(value) ? 'Yes' : 'No';
}
function mapFurnishedStatusForProperty24(value) {
    return toBool(value) ? 'Yes' : 'No';
}
function buildRentalRateCandidatesForProperty24(value) {
    const text = (toText(value) ?? '').trim();
    if (!text)
        return [null];
...
function pickNumericString(value) {
    if (typeof value === 'number' && Number.isFinite(value)) {
...
function extractP24AgentIdFromText(value) {
    const text = toText(value);
    if (!text)
        return null;
...
function extractLegacySuburbId(listingPayload) {
    ...
    if (propertyInfo && typeof propertyInfo === 'object') {
        const suburbId = pickNumericString(propertyInfo.suburbId);
        if (suburbId)
            return suburbId;
    }
...
function matchesAreaName(target, candidate, alternateNames) {
    const normalizedTarget = normalizeAreaName(target);
    const compactTarget = compactAreaName(target);
    if (!normalizedTarget)
        return false;
    if (normalizeAreaName(candidate) === normalizedTarget)
        return true;
    if (compactAreaName(candidate) === compactTarget)
        return true;
    return (alternateNames ?? []).some((entry) => {
        const normalizedAlternate = normalizeAreaName(entry?.name);
        if (normalizedAlternate === normalizedTarget)
            return true;
        return compactAreaName(entry?.name) === compactTarget;
    });
}
function areaNameContains(query, candidate, alternateNames) {
    const normalizedQuery = normalizeAreaName(query);
    if (!normalizedQuery)
        return true;
    if (normalizeAreaName(candidate).includes(normalizedQuery))
        return true;
    return (alternateNames ?? []).some((entry) => normalizeAreaName(entry?.name).includes(normalizedQuery));
}
async function resolveSuburbIdFromMetadata(apiBase, headers, provinceName, cityName, suburbName) {
    try {
        ...
        if (!suburbsResponse.ok)
            return null;
        const suburbs = await suburbsResponse.json();
...
async function resolveSuburbIdFromTextHints(apiBase, headers, provinceName, cityName, textHints) {
    ...
    try {
        ...
        const candidates = suburbs
            ...
            .filter((entry) => Boolean(entry.id) && entry.bestMatchLength > 0)
...
function getProperty24MetadataConfig() {
...
async function getAccessibleProperty24AgencyIds(apiBase, headers) {
    try {
        const response = await fetchWithRetries(`${apiBase}/agencies`, { method: 'GET', headers }, { attempts: 2, timeoutMs: 15000 });
        if (!response.ok)
            return [];
...
async function enrichListingPayloadWithResolvedSuburbId(body) {
    ...
    const suburbId = await resolveSuburbIdFromMetadata(property24Config.apiBase, property24Config.headers, provinceName, cityName, suburbName);
    if (!suburbId) {
        body.listing_payload = listingPayload;
        return;
    }
    const propertyInfo = listingPayload.propertyInfo && typeof listingPayload.propertyInfo === 'object'
        ? { ...listingPayload.propertyInfo }
        : {};
    body.listing_payload = {
        ...listingPayload,
        propertyInfo: {
...
router.post('/:id/publish-to-property24', async (req, res) => {
    ...
    try {
        ...
        cl.mandate_type
       FROM migration.core_listings cl WHERE cl.id = $1 LIMIT 1`, [id]);
        if (listingResult.rowCount === 0) {
            return res.status(404).json({ error: 'Listing not found.' });
        }
        const listing = listingResult.rows[0];
        const agentResult = await pool.query(`SELECT a.id AS associate_id, a.market_center_id, a.source_market_center_id, a.source_team_id,
       ...
       LEFT JOIN migration.core_associates a ON a.id = la.associate_id
       WHERE la.listing_id = $1
       ORDER BY la.is_primary DESC, la.sort_order ASC, la.id ASC`, [id]);
        const primaryAgent = agentResult.rows[0];
        const marketCenterLookupId = toText(listing.market_center_id) ??
            toText(primaryAgent?.market_center_id);
        let marketCenter;
        if (marketCenterLookupId) {
            const marketCenterResult = await pool.query(`SELECT id::text, source_market_center_id, name
         FROM migration.core_market_centers
         WHERE id::text = $1
         LIMIT 1`, [marketCenterLookupId]);
            marketCenter = marketCenterResult.rows[0];
        }
        let marketCenterAddress = null;
        if (marketCenterLookupId) {
            const marketCenterColumnsResult = await pool.query(`
        SELECT column_name
        FROM information_schema.columns
        WHERE table_schema = 'migration'
        ...
        `);
            ...
            const optionalSelect = optionalColumns
                ...
                .join(',\n               ');
            const marketCenterAddressResult = await pool.query(`SELECT ${optionalSelect}
         FROM migration.core_market_centers
         WHERE id::text = $1
         LIMIT 1`, [marketCenterLookupId]);
       ...
       GROUP BY area_type`, [id]);
        ...
        const marketingUrlsResult = await pool.query(`SELECT url, url_type, display_name, sort_order
       FROM migration.listing_marketing_urls
       WHERE listing_id = $1
       ORDER BY sort_order ASC NULLS LAST, id ASC`, [id]);
        ...
        const descriptionHeader = toText(listing.property_title) ??
            toText(listing.short_title) ??
            toText(listing.short_description) ??
            description.slice(0, 500);
        const listingCountryName = toText(listing.country) ??
        ...
        let resolvedAgentCountryId = null;
        let resolvedAgentCityId = null;
        try {
        ...
        for (const agentRow of agentResult.rows) {
            if (!agentRow.property24_opt_in)
                continue; // Only register agents who have opted in to P24
            if (agentRow.agent_property24_id)
                continue;
            ...
            try {
                ...
                const mcResult = await pool.query(`SELECT market_center_property24_id FROM migration.core_market_centers
           WHERE source_market_center_id = $1 LIMIT 1`, [mcId]);
                ...
                const assocResult = await pool.query(`SELECT first_name, last_name, kwsa_email, mobile_number, office_number, national_id,
                  ffc_number, image_url, source_associate_id, source_market_center_id, kwuid, status_name
           FROM migration.core_associates WHERE id = $1 LIMIT 1`, [assocId]);
                const ar = assocResult.rows[0];
                if (!ar)
                    continue;
                const jtResult = await pool.query(`SELECT job_title FROM migration.associate_job_titles WHERE associate_id = $1 ORDER BY id ASC`, [assocId]);
                ...
                if (p24UserGroupId)
                    p24AuthHdrs['P24-UserGroupId'] = p24UserGroupId;
                ...
                if (createResp.ok) {
                    ...
                    if (newP24Id > 0) {
                        ...
                        await pool.query(`UPDATE migration.core_associates
               SET agent_property24_id = $1, property24_status = 'Registered', updated_at = NOW()
               WHERE id = $2`, [String(newP24Id), assocId]);
               ...
               WHERE id = $2`, [existingP24AgentId, assocId]);
             ...
             WHERE id = $2`, [`Registration failed: ${p24ErrMsg}`.slice(0, 490), assocId]);
                }
            }
            catch (autoRegErr) {
        ...
        for (const agentRow of agentResult.rows) {
            ...
            try {
                const assocResult = await pool.query(`SELECT first_name, last_name, image_url, kwsa_email, mobile_number, office_number,
           ...
           WHERE source_market_center_id = $1 LIMIT 1`, [mcId]);
                ...
                if (resolvedAgentCountryId)
                    fullAgentPayload.countryId = resolvedAgentCountryId;
                if (resolvedAgentCityId)
                    fullAgentPayload.cityId = resolvedAgentCityId;
        ...
        const contactAgentIds = agentResult.rows
            .filter((row) => row.property24_opt_in)
            .map((row) => {
            const record = row;
        ...
        if (!resolvedSuburbId && listingProvinceName && listingCityName && listingSuburbName) {
            resolvedSuburbId = await resolveSuburbIdFromMetadata(listingApiBase, p24AuthHeaders, listingProvinceName, listingCityName, listingSuburbName);
        }
        if (!resolvedSuburbId && listingProvinceName && listingCityName) {
            resolvedSuburbId = await resolveSuburbIdFromTextHints(listingApiBase, p24AuthHeaders, listingProvinceName, listingCityName, [
                listingSuburbName ?? '',
        ...
        const expiryDateValue = toDateValue(listing.expiry_date) ??
            new Date(Date.now() + (90 * 24 * 60 * 60 * 1000)).toISOString().slice(0, 10);
        const propertyTypeId = mapPropertyTypeIdForProperty24(listing.property_type, listing.property_sub_type);
        const resolvedLatitude = toBool(listing.override_display_location)
            ? toNumber(listing.override_display_latitude) ?? toNumber(listing.latitude)
            : toNumber(listing.latitude);
        const resolvedLongitude = toBool(listing.override_display_location)
            ? toNumber(listing.override_display_longitude) ?? toNumber(listing.longitude)
            : toNumber(listing.longitude);
        const missingFields = [];
        if (!resolvedAgencyId)
            missingFields.push('agencyId');
        if (contactAgentIds.length === 0)
            missingFields.push('contactAgentIds');
        if (!description.trim())
            missingFields.push('description');
        if (!expiryDateValue)
            missingFields.push('expiryDate');
        if (!resolvedSuburbId)
            missingFields.push('propertyInfo.suburbId');
        if (!marketCenter && !toText(listing.market_center_id))
            missingFields.push('listing.market_center_id');
        if (missingFields.length > 0) {
         ...
         WHERE id = $1`, [id, prerequisiteMessage.slice(0, 490)]);
            const hasAgentRegErrors = agentResult.rows
        ...
        const featureTags = [];
        if (hasGardenFeature)
            featureTags.push({ description: 'Garden', featureType: 'Garden', tags: [] });
        if (hasPoolFeature)
            featureTags.push({ description: 'Pool', featureType: 'Pool', tags: [] });
        ...
        const parseP24ResponseBody = (rawText) => {
            try {
        ...
        const hasStatusConversionError = (body, rawText) => {
        ...
        let p24Response = await fetchWithRetries(apiUrl, {
            method: apiMethod,
            headers: p24Headers,
            body: JSON.stringify(p24Payload),
        }, { attempts: 3, timeoutMs: 30000 });
        let responseText = await p24Response.text();
        let responseBody = parseP24ResponseBody(responseText);
        const canRetryWithAlternateRentalRate = !p24Response.ok
            && listingType === 'Rental'
        ...
        if (canRetryWithAlternateRentalRate) {
            for (const rentalRateCandidate of rentalRateCandidates.slice(1)) {
                const baseRentalInfo = p24Payload.rentalInfo;
                ...
                console.warn(`[P24] Retrying listing ${String(listing.listing_number)} with alternate rentalRate=${String(rentalRateCandidate)}`);
                p24Response = await fetchWithRetries(apiUrl, {
                    method: apiMethod,
                    headers: p24Headers,
        ...
        const canRetryWithActiveStatus = p24Response.status === 400
        ...
        if (!p24Response.ok && canRetryWithActiveStatus) {
            ...
            console.warn(`[P24] Retrying listing ${String(listing.listing_number)} with status Active after status conversion error for ${p24Status}`);
            p24Response = await fetchWithRetries(apiUrl, {
                method: apiMethod,
                headers: p24Headers,
        ...
        const returnedRef = (toRef(responseBody.listingNumber ?? responseBody.ListingNumber) ??
       ...
       SET property24_ref1 = COALESCE($2, property24_ref1),
           ...
           is_draft = false,
           updated_at = NOW()
       WHERE id = $1`, [id, returnedRef, syncStatus, !isWithdraw]);
        ...
        try {
            await pool.query(`UPDATE migration.core_listings SET property24_sync_status = $2, updated_at = NOW() WHERE id = $1`, [id, `Error: ${message.slice(0, 490)}`]);
        }
        catch {
            // best-effort
        }
...
function normalizePpProvince(province) {
...
function mapPpHomeType(subType) {
    const s = subType.toLowerCase();
    if (['house', 'cluster', 'simplex', 'georgian', 'duet'].some((v) => s.includes(v)))
        return 'House';
    if (s.includes('townhouse'))
        return 'Townhouse';
    if (['apartment', 'flat', 'bachelor', 'loft', 'penthouse', 'studio'].some((v) => s.includes(v)))
        return 'Apartment';
    if (s.includes('duplex'))
        return 'Duplex';
    if (s.includes('cottage'))
        return 'Garden Cottage';
    return 'House';
}
function mapPpBusinessType(value) {
    const s = value.toLowerCase();
    if (s.includes('warehouse'))
        return 'Warehouse';
    if (s.includes('storage') || s.includes('storeroom'))
        return 'Storage';
    if (s.includes('yard'))
        return 'Yard';
    if (s.includes('smallholding'))
        return 'Smallholding';
    if (s.includes('new development') || s.includes('development'))
    ...
    if (s.includes('office'))
        return 'Office';
    if (s.includes('retail') || s.includes('shop'))
        return 'Retail';
    if (s.includes('distribution'))
        return 'Distribution Centre';
    if (s.includes('guesthouse') || s.includes('guest house') || s.includes('hospitality'))
        return 'Guesthouse';
    // Use a conservative fallback that is broadly accepted across PP branches.
    return 'Office';
}
function mapPpCategory(propertyType, propertySubType, descriptiveFeature) {
    ...
    const normalizeTokens = (value) => ` ${value.toLowerCase().replace(/[^a-z0-9]+/g, ' ').trim()} `;
    const hasAnyToken = (value, tokens) => {
        const normalized = normalizeTokens(value);
        return tokens.some((token) => normalized.includes(` ${token} `));
    };
    const matchesCommercial = (value) => hasAnyToken(value, ['commercial', 'business', 'industrial', 'warehouse', 'factory', 'office', 'retail', 'distribution', 'storage', 'yard', 'guesthouse', 'hotel']);
    const matchesResidential = (value) => hasAnyToken(value, ['residential', 'house', 'apartment', 'flat', 'townhouse', 'cluster', 'duplex', 'simplex', 'freehold', 'sectional', 'cottage', 'studio', 'loft', 'penthouse']);
    ...
    if (subType.includes('land') || subType.includes('vacant'))
        return 'Land';
    if (feature.includes('land') || feature.includes('vacant'))
        return 'Land';
    ...
    if (type.includes('residential'))
        return 'Residential';
    if (matchesCommercial(type))
        return 'Commercial';
    if (subType.includes('farm'))
        return 'Farms';
    if (matchesResidential(subType))
        return 'Residential';
    if (matchesCommercial(subType))
        return 'Commercial';
    if (structured.length === 0) {
        if (feature.includes('farm'))
            return 'Farms';
        if (feature.includes('land') || feature.includes('vacant'))
            return 'Land';
        if (matchesCommercial(feature))
            return 'Commercial';
    }
    return 'Residential';
}
function getPpBusinessTypeFallbacks(preferred) {
    const values = [
        preferred,
        'Factory',
        'Office',
...
function buildPpToken(username, password) {
    const toPpDateTime = (value) => {
...
async function fetchPpReferenceByUniqueId(args) {
    const lookupFromEndpoint = async (endpoint) => {
...
router.post('/:id/publish-to-private-property', async (req, res) => {
    ...
    try {
        const listingResult = await pool.query(`SELECT cl.id, cl.listing_number, cl.market_center_id, cl.source_market_center_id,
       ...
       FROM migration.core_listings cl WHERE cl.id = $1 LIMIT 1`, [id]);
        ...
        const listing = listingResult.rows[0];
        const agentResult = await pool.query(`SELECT la.associate_id::text, la.is_primary, la.sort_order,
       ...
       WHERE la.listing_id = $1
       ORDER BY la.is_primary DESC, la.sort_order ASC, la.id ASC`, [id]);
        const listingAgents = agentResult.rows;
        ...
        if (mcLookupId) {
            try {
                const mcRes = await pool.query(`SELECT private_property_id::text AS pp_id FROM migration.core_market_centers WHERE id::text = $1 LIMIT 1`, [mcLookupId]);
                const mcPpId = toText(mcRes.rows[0]?.pp_id);
                if (mcPpId && /^[0-9a-f-]{36}$/i.test(mcPpId))
                    branchGuid = mcPpId;
            }
        ...
        const areasResult = await pool.query(`SELECT LOWER(TRIM(area_type)) AS area_type, MAX(count)::numeric AS count
       FROM migration.listing_property_areas
       WHERE listing_id = $1
         AND LOWER(TRIM(COALESCE(area_type, ''))) IN ('bedroom', 'bathroom', 'garage')
       GROUP BY LOWER(TRIM(area_type))`, [id]);
        const areaCounts = {};
        for (const row of areasResult.rows) {
            const parsedCount = toNumber(row.count) ?? 0;
        ...
        if (isWithdraw && existingRef) {
            ...
            const imageResolution = await resolveExternalImageUrls(await resolveListingImageUrls(pool, id, listing.listing_images_json), req);
            ...
            if (statusTag.includes('sold') || statusName.includes('sold'))
                propertyStatus = 'Sold';
            else if (statusTag.includes('pending') || statusTag.includes('offer'))
                propertyStatus = 'PendingOffer';
            ...
            if (category === 'Commercial') {
                const businessTypeSource = toText(listing.descriptive_feature) ??
            ...
            if (areaCounts['garage'] > 0)
                attrs.push({ type: 'Garages', value: areaCounts['garage'].toString() });
            const floorArea = toNumber(listing.floor_area);
            if (floorArea && floorArea > 0)
                attrs.push({ type: 'FloorArea', value: Math.round(floorArea).toString() });
            const erfSize = toNumber(listing.erf_size);
            if (erfSize && erfSize > 0 && category !== 'Land')
                attrs.push({ type: 'LandArea', value: Math.round(erfSize).toString() });
            const rates = toNumber(listing.rates_and_taxes);
            if (rates && rates > 0)
                attrs.push({ type: 'Rates', value: Math.round(rates).toString() });
            const levies = toNumber(listing.monthly_levy);
            if (levies && levies > 0)
                attrs.push({ type: 'Levies', value: Math.round(levies).toString() });
            if (toBool(listing.pet_friendly))
                attrs.push({ type: 'PetsAllowed', value: 'Yes' });
            if (toBool(listing.is_furnished))
                attrs.push({ type: 'Furnished', value: 'Yes' });
            if (toBool(listing.has_flatlet))
                attrs.push({ type: 'Flatlet', value: 'Yes' });
            ...
            for (const secondaryAgent of secondaryAgents) {
                ...
                if (!secondaryAgentId)
                    continue;
                const secondaryAgentEmail = toText(secondaryAgent.kwsa_email) ??
        ...
        const soapHeaders = { 'Content-Type': `${contentType}; charset=utf-8` };
        if (!isSoap12)
            soapHeaders['SOAPAction'] = `http://tempuri.org/${soapAction}`;
        const ppUrls = [
            ppBaseUrl,
        ...
        if (/PP100|Security token did not authenticate properly/i.test(responseText) &&
        ...
        const callPpStatusUpdate = async (propertyStatus) => {
        ...
        const callPpShowdayUpdate = async (targetBranchGuid, targetPropertyId, window) => {
        ...
        if (!success &&
            ...
            for (const fallbackBusinessType of fallbackBusinessTypes) {
                if (fallbackBusinessType === selectedBusinessType)
                    continue;
                const retrySoapXml = soapXml.replace(/(<AttributeType>BusinessType<\/AttributeType>\s*<Value>)([\s\S]*?)(<\/Value>)/i, `$1${xmlEscape(fallbackBusinessType)}$3`);
                const retryResponse = await fetchWithRetries(activePpUrl, {
                    method: 'POST',
                    headers: soapHeaders,
                    body: retrySoapXml,
        ...
        if (!success &&
            ...
            const fallbackUnitNumber = toText(listing.unit_number) ??
                toText(listing.street_number) ??
                toText(listing.listing_number) ??
                '1';
            const retrySoapXml = soapXml.replace(/<UnitNumber>[\s\S]*?<\/UnitNumber>/i, `<UnitNumber>${xmlEscape(fallbackUnitNumber)}</UnitNumber>`);
            const retryResponse = await fetchWithRetries(activePpUrl, {
                method: 'POST',
                headers: soapHeaders,
                body: retrySoapXml,
        ...
        if (!success &&
            ...
            for (const candidateBranchGuid of branchCandidates) {
                for (const fallbackAgentId of fallbackAgentIds) {
                    if (candidateBranchGuid === branchGuid && fallbackAgentId === (ppAgentIdUsed ?? ''))
                        continue;
                    const retrySoapXml = soapXml
        ...
        if (success) {
           ...
           WHERE id = $2`, [persistedReference, id]);
            }
        }
...
router.post('/:id/publish-to-kww', async (req, res) => {
    ...
    try {
       ...
       ORDER BY la.is_primary DESC, la.sort_order ASC, la.id ASC`, [id]);
        ...
        let frontdoorId = null;
        let marketCenterName = '';
        if (marketCenterLookupId) {
            try {
                const mcResult = await pool.query(`SELECT name, frontdoor_id::text AS frontdoor_id
           FROM migration.core_market_centers WHERE id::text = $1 LIMIT 1`, [marketCenterLookupId]);
                const mcRow = mcResult.rows[0];
        ...
        let listStatusId = 1;
        if (isWithdraw) {
            listStatus = 'Withdrawn';
            listStatusId = 11;
        }
        else if (statusTag.includes('sold') || statusName.includes('sold')) {
            listStatus = 'Sold';
            listStatusId = 3;
        }
        else if (statusTag.includes('pending') || statusTag.includes('offer')) {
       ...
       GROUP BY LOWER(TRIM(area_type))`, [id]);
        ...
        const parkingAreasResult = await pool.query(`SELECT LOWER(TRIM(area_type)) AS area_type, sub_features
       FROM migration.listing_property_areas
       WHERE listing_id = $1
         AND LOWER(TRIM(COALESCE(area_type, ''))) IN ('parking', 'garage')`, [id]);
        const parkingFeatureInputs = [];
        ...
        if (existingListUuid) {
            payload.list_uuid = existingListUuid;
        }
        ...
        const parseKwwResponse = (rawText) => {
            if (!rawText)
                return null;
            try {
                return JSON.parse(rawText);
        ...
        const extractKwwIdentifiers = (rawJson) => {
            if (!rawJson || typeof rawJson !== 'object')
                return null;
            const root = rawJson;
            const directData = root.data;
            if (directData && typeof directData === 'object') {
                const dataRecord = directData;
            ...
            if (!hitsRoot || typeof hitsRoot !== 'object')
                return null;
            const outerHits = hitsRoot.hits;
            if (!Array.isArray(outerHits) || outerHits.length === 0)
                return null;
            ...
            if (!sourceUuid && !sourceKey && sourceId == null)
                return null;
            return {
                listUuid: sourceUuid,
                listKey: sourceKey,
        ...
        const findExistingKwwListingByKey = async () => {
        ...
        let attemptedListTypes = [String(payload.list_type ?? '')].filter(Boolean);
        ...
        const isKwwAcceptedResponse = (resp, json) => {
            if (!resp.ok)
                return false;
            if (!json)
                return true;
            const explicitSuccess = json.success === true || json.success === 'true';
            const explicitFailure = json.success === false ||
                json.success === 'false' ||
        ...
        let parsedErrorMessage = responseJson?.message ??
            responseJson?.error ??
        ...
        if (!success && /listing number is already in use/i.test(parsedErrorMessage ?? '')) {
            ...
            if (resolvedListUuid) {
                ...
                payload.list_uuid = resolvedListUuid;
                attemptedRequests.push({ method: requestMethod, url: requestUrl });
        ...
        if (!success && /invalid\s+list\s+type/i.test(parsedErrorMessage ?? '')) {
            ...
            ].filter((value) => typeof value === 'string' && !attemptedListTypes.includes(value));
            for (const fallbackType of fallbackTypes) {
                payload.list_type = fallbackType;
                payload.list_type_id = fallbackType === 'Prospective'
                    ? 1
                ...
                if (success)
                    break;
                if (!/invalid\s+list\s+type/i.test(parsedErrorMessage ?? ''))
                    break;
            }
        }
        ...
        const failureDetails = !success
            ? {
                ...
                payloadPreview: {
                    mls_number: listingNumber,
                    list_kw_uid: payload.list_kw_uid,
                    list_type: payload.list_type,
                    list_type_id: payload.list_type_id,
                    prop_type: payload.prop_type,
                    prop_type_id: payload.prop_type_id,
                    prop_subtype: payload.prop_subtype,
                    prop_subtype_id: payload.prop_subtype_id,
                },
                attemptedListTypes,
                attemptedRequests,
            }
        ...
        if (success) {
           ...
           WHERE id = $4`, [publicReferenceId, listUuid ?? existingListUuid ?? null, listKey ?? null, id]);
            }
        }
        return res.status(success ? 200 : 422).json({
            success,
...
router.post('/:id/publish-to-entegral', async (req, res) => {
    ...
    const entegralUrl = (segment) => `${entegralBaseUrl.replace(/\/$/, '')}/${segment}`;
    try {
        // Load listing row
        const listingResult = await pool.query(`SELECT
        cl.id, cl.source_market_center_id, cl.market_center_id,
       ...
       FROM migration.core_listings cl WHERE cl.id = $1 LIMIT 1`, [id]);
        ...
        const statusTag = (toText(listing.listing_status_tag) ?? '').toLowerCase().trim();
        const statusName = (toText(listing.status_name) ?? '').toLowerCase().trim();
        const isWithdraw = statusTag === 'withdrawn' ||
            statusTag === 'withdraw' ||
            statusName === 'withdrawn' ||
            statusName === 'inactive';
        // Load agents linked to this listing
        const agentResult = await pool.query(`SELECT a.id AS associate_id, a.market_center_id, a.source_market_center_id,
              COALESCE(a.full_name, la.agent_name) AS agent_name,
              COALESCE(a.kwsa_email, a.private_email, a.email) AS agent_email,
              COALESCE(a.mobile_number, a.office_number) AS agent_phone,
              a.agent_entegral_id,
       ...
       WHERE la.listing_id = $1
       ORDER BY la.is_primary DESC, la.sort_order ASC, la.id ASC`, [id]);
        const allAgents = agentResult.rows;
        ...
        let entegralPortals = [];
        if (marketCenterLookupId) {
            const mcColResult = await pool.query(`SELECT column_name FROM information_schema.columns
         WHERE table_schema = 'migration' AND table_name = 'core_market_centers'
           AND column_name IN ('source_market_center_id', 'name', 'entegral_portals', 'entegral_url', 'office_number')`);
            ...
            const optSelect = ['entegral_portals', 'entegral_url', 'office_number']
                ...
                .join(', ');
            const mcResult = await pool.query(`SELECT id::text, source_market_center_id, name, ${optSelect}
         FROM migration.core_market_centers WHERE id::text = $1 LIMIT 1`, [marketCenterLookupId]);
            marketCenter = mcResult.rows[0];
       ...
       GROUP BY LOWER(TRIM(area_type))`, [id]);
        ...
        function mapEntegralPropertyType(type, subType) {
            switch (type) {
                case 'Residential':
                    switch (subType) {
       ...
       WHERE id = $3`, [newSyncStatus, !isWithdraw && success, id]);
        return res.status(success ? 200 : 422).json({
            success,
            message: success
...
// ---------------------------------------------------------------------------
async function createNotification(pg, associateId, notificationType, category, title, message, entityType, entityId, metadata) {
    await pg.query(`INSERT INTO migration.in_app_notifications
       (associate_id, notification_type, category, title, message, entity_type, entity_id, metadata)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8::jsonb)`, [associateId, notificationType, category, title, message, entityType, entityId, JSON.stringify(metadata)]);
}
...
router.post('/:id/submit-for-approval', resolvePermissions, async (req, res) => {
    ...
    try {
        ...
        const listingRes = await pool.query(`SELECT market_center_id, listing_number, source_market_center_id FROM migration.core_listings WHERE id = $1 LIMIT 1`, [id]);
        ...
        let submitterEmail = '';
        if (perms.associateDbId) {
            const assocRes = await pool.query(`SELECT full_name, COALESCE(kwsa_email, email, private_email) AS email FROM migration.core_associates WHERE id = $1 LIMIT 1`, [perms.associateDbId]);
        ...
        await pool.query(`DELETE FROM migration.listing_approval_requests WHERE listing_id = $1`, [id]);
        await pool.query(`INSERT INTO migration.listing_approval_requests
         (listing_id, status, submitted_by_associate_id, submitted_by_name, submitted_by_email, submission_comment, submitted_at)
       VALUES ($1, 'PENDING', $2, $3, $4, $5, NOW())`, [id, perms.associateDbId ?? null, submitterName, submitterEmail, comment ?? null]);
        ...
        if (listing.market_center_id) {
            const adminsRes = await pool.query(`SELECT DISTINCT a.id, a.full_name
         FROM migration.core_associates a
         JOIN migration.associate_roles r ON r.associate_id = a.id
         WHERE a.market_center_id = $1
           AND r.role_name IN ('Office Admin', 'OfficeAdmin')`, [listing.market_center_id]);
            for (const admin of adminsRes.rows) {
                try {
                    await createNotification(pool, admin.id, 'LISTING_APPROVAL_REQUESTED', 'PENDING', `Listing approval requested: ${listing.listing_number ?? id}`, `${submitterName} has submitted listing ${listing.listing_number ?? id} for approval.`, 'listing', id, { listing_id: id, listing_number: listing.listing_number, submitter_name: submitterName, submitter_email: submitterEmail, status: 'PENDING' });
                }
        ...
        if (submitterAssociateId && Number.isFinite(submitterAssociateId)) {
            try {
                await createNotification(pool, submitterAssociateId, 'LISTING_APPROVAL_SUBMITTED', 'PENDING', `Listing submitted for approval: ${listing.listing_number ?? id}`, `Your listing ${listing.listing_number ?? id} was submitted and is pending office admin review.`, 'listing', id, { listing_id: id, listing_number: listing.listing_number, status: 'PENDING' });
            }
            catch { /* notification table may not exist */ }
        }
...
router.post('/:id/approve-approval', resolvePermissions, async (req, res) => {
    ...
    const perms = req.permissions;
    if (perms.scope === 'OWN')
        return res.status(403).json({ error: 'Only admins may approve listings' });
    const { comment } = req.body;
    try {
        const approvalRes = await pool.query(`SELECT submitted_by_associate_id, submitted_by_name FROM migration.listing_approval_requests WHERE listing_id = $1 LIMIT 1`, [id]);
        if (!approvalRes.rows.length)
            return res.status(404).json({ error: 'No approval request found for this listing' });
        const approval = approvalRes.rows[0];
        const listingStateRes = await pool.query(`SELECT listing_number, sale_or_rent, listing_status_tag, listing_payload
         FROM migration.core_listings
        WHERE id = $1
        LIMIT 1`, [id]);
        if (!listingStateRes.rows.length)
            return res.status(404).json({ error: 'Listing not found' });
        const listingState = listingStateRes.rows[0];
        const approvalContext = listingState.listing_payload
            && typeof listingState.listing_payload === 'object'
            && listingState.listing_payload.approval_context
            && typeof listingState.listing_payload.approval_context === 'object'
            ? listingState.listing_payload.approval_context
            : null;
        const approvedListingStatusTag = deriveApprovedListingStatusTag(listingState.sale_or_rent, listingState.listing_status_tag, approvalContext?.pre_submit_listing_status_tag);
        // Resolve reviewer
        let reviewerName = 'Admin';
        if (perms.associateDbId) {
            const r = await pool.query(`SELECT full_name FROM migration.core_associates WHERE id = $1 LIMIT 1`, [perms.associateDbId]);
            reviewerName = r.rows[0]?.full_name ?? 'Admin';
        }
        // Update approval record
        await pool.query(`UPDATE migration.listing_approval_requests SET
         status = 'APPROVED', reviewed_by_associate_id = $2, reviewed_by_name = $3,
         review_comment = $4, reviewed_at = NOW(), updated_at = NOW()
       WHERE listing_id = $1`, [id, perms.associateDbId ?? null, reviewerName, comment ?? null]);
        // Publish listing
        await pool.query(`UPDATE migration.core_listings
          SET is_draft = false,
              is_published = true,
        ...
        if (approval.submitted_by_associate_id) {
            try {
                await createNotification(pool, approval.submitted_by_associate_id, 'LISTING_APPROVAL_APPROVED', 'APPROVED', `Listing approved: ${listingState.listing_number ?? id}`, `Your listing ${listingState.listing_number ?? id} has been approved and published by ${reviewerName}.`, 'listing', id, { listing_id: id, listing_number: listingState.listing_number, reviewer_name: reviewerName, status: 'APPROVED' });
            }
            catch { /* notification table may not exist */ }
        }
        await pool.query(`UPDATE migration.in_app_notifications
...
router.post('/:id/reject-approval', resolvePermissions, async (req, res) => {
    ...
    const perms = req.permissions;
    if (perms.scope === 'OWN')
        return res.status(403).json({ error: 'Only admins may reject listings' });
    const { comment } = req.body;
    try {
        const approvalRes = await pool.query(`SELECT submitted_by_associate_id FROM migration.listing_approval_requests WHERE listing_id = $1 LIMIT 1`, [id]);
        if (!approvalRes.rows.length)
            return res.status(404).json({ error: 'No approval request found for this listing' });
        const approval = approvalRes.rows[0];
        let reviewerName = 'Admin';
       ...
       WHERE listing_id = $1`, [id, perms.associateDbId ?? null, reviewerName, comment ?? null]);
        ...
        WHERE id = $1`, [id]);
        if (approval.submitted_by_associate_id) {
            const listingRes = await pool.query(`SELECT listing_number FROM migration.core_listings WHERE id = $1 LIMIT 1`, [id]);
            try {
                await createNotification(pool, approval.submitted_by_associate_id, 'LISTING_APPROVAL_REJECTED', 'REJECTED', `Listing not approved: ${listingRes.rows[0]?.listing_number ?? id}`, `Your listing ${listingRes.rows[0]?.listing_number ?? id} was not approved by ${reviewerName}.${comment ? ` Reason: ${comment}` : ''}`, 'listing', id, { listing_id: id, listing_number: listingRes.rows[0]?.listing_number, reviewer_name: reviewerName, comment, status: 'REJECTED' });
            }
            catch { /* notification table may not exist */ }
        }
        await pool.query(`UPDATE migration.in_app_notifications
