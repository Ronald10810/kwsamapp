...
router.get('/agent-listings/:associateId', resolvePermissions, async (req, res) => {
    ...
    try {
       ...
       ORDER BY cl.listing_number`, [associateId]);
...
router.post('/jobs', resolvePermissions, async (req, res) => {
    ...
    const listingIds = Array.isArray(b.listingIds)
        ? b.listingIds.map(Number).filter((n) => Number.isFinite(n) && n > 0)
        : [];
    ...
    // Resolve agent names
    const agentsRes = await pool.query(`SELECT id::text, full_name FROM migration.core_associates WHERE id = ANY($1::bigint[])`, [[fromAgentId, toAgentId]]);
...
// ─────────────────────────────────────────────────────────────────────────────
async function generateTransferListingNumber(pool) {
    const result = await pool.query(`SELECT MAX(CAST(SUBSTRING(listing_number FROM 4) AS INTEGER))::text AS max_num
...
async function runTransferJob(job, token, activeContext) {
    const pool = getRequiredPgPool();
    job.status = 'running';
    for (const listingIdStr of job.listingIds) {
        ...
        try {
         ...
         FROM migration.core_listings WHERE id = $1 LIMIT 1`, [listingId]);
            ...
            if (hasP24Ref) {
                const pr = { portal: 'Property24', withdrawOk: null, withdrawError: null, publishOk: null, publishError: null };
            ...
            if (hasPpRef) {
                const pr = { portal: 'Private Property', withdrawOk: null, withdrawError: null, publishOk: null, publishError: null };
            ...
            if (hasKwwRef) {
                const pr = { portal: 'KWW', withdrawOk: null, withdrawError: null, publishOk: null, publishError: null };
            ...
            if (hasEntegralRef) {
                const pr = { portal: 'Entegral', withdrawOk: null, withdrawError: null, publishOk: null, publishError: null };
         ...
         WHERE id = $1`, [listingId]);
            ...
            const dupRes = await pool.query(`INSERT INTO migration.core_listings (
         ...
         RETURNING id::text, listing_number`, [listingId, newListingNumber, JSON.stringify(cleanPayload)]);
            ...
            await pool.query(`INSERT INTO migration.listing_property_areas (listing_id, area_type, count, size, description, sub_features, sort_order)
         SELECT $2, area_type, count, size, description, sub_features, sort_order
         FROM migration.listing_property_areas WHERE listing_id = $1`, [listingId, newListingId]);
            await pool.query(`INSERT INTO migration.listing_features (listing_id, feature_category, feature_value, sort_order)
         SELECT $2, feature_category, feature_value, sort_order
         FROM migration.listing_features WHERE listing_id = $1`, [listingId, newListingId]);
            await pool.query(`INSERT INTO migration.listing_contacts (listing_id, full_name, phone_number, email_address, sort_order)
         SELECT $2, full_name, phone_number, email_address, sort_order
         FROM migration.listing_contacts WHERE listing_id = $1`, [listingId, newListingId]);
            await pool.query(`INSERT INTO migration.listing_images (listing_id, file_name, file_url, media_type, sort_order, uploaded_by)
         SELECT $2, file_name, file_url, media_type, sort_order, uploaded_by
         FROM migration.listing_images WHERE listing_id = $1`, [listingId, newListingId]);
            const marketingUrlResult = await pool.query(`SELECT id::text, url, url_type, display_name, sort_order
         FROM migration.listing_marketing_urls WHERE listing_id = $1 ORDER BY sort_order, id`, [listingId]);
            for (const row of marketingUrlResult.rows) {
                const normalized = normalizeMarketingUrlRecord(row);
                await pool.query(`INSERT INTO migration.listing_marketing_urls (listing_id, url, url_type, display_name, sort_order)
           VALUES ($1, $2, $3, $4, $5)`, [newListingId, normalized.url, normalized.url_type, normalized.display_name ?? null, normalized.sort_order ?? 0]);
            }
            await pool.query(`INSERT INTO migration.listing_show_times (listing_id, from_date, from_time, to_date, to_time, catch_phrase, sort_order)
         SELECT $2, from_date, from_time, to_date, to_time, catch_phrase, sort_order
         FROM migration.listing_show_times WHERE listing_id = $1`, [listingId, newListingId]);
            await pool.query(`INSERT INTO migration.listing_open_house (listing_id, open_house_date, from_time, to_time, average_price, comments, sort_order)
         SELECT $2, open_house_date, from_time, to_time, average_price, comments, sort_order
         FROM migration.listing_open_house WHERE listing_id = $1`, [listingId, newListingId]);
            // ── 8. Add to-agent as primary on the new listing ─────────────────────
            await pool.query(`INSERT INTO migration.listing_agents (listing_id, associate_id, agent_name, agent_role, is_primary, sort_order)
         SELECT $2, id, full_name, 'Primary', true, 0
         FROM migration.core_associates WHERE id = $1`, [job.toAgentId, newListingId]);
            ...
            if (feedColsToEnable.length > 0) {
                await pool.query(`UPDATE migration.core_listings SET ${feedColsToEnable.map((c, i) => `${c} = $${i + 2}`).join(', ')}, updated_at = NOW() WHERE id = $1`, [newListingId, ...feedColsToEnable.map(() => true)]);
            }
            for (const def of portalPublishDefs) {
                ...
                pr.publishOk = r.ok;
                if (!r.ok)
                    pr.publishError = extractApiError(r.body);
            }
        }
        catch (err) {
        ...
        try {
            await pool.query(`INSERT INTO migration.listing_transfer_log
...
//# sourceMappingURL=listingTransfer.js.map