...
async function callListingApi(path, method, token, body, activeContext) {
...
async function syncListingToActivePortals(pool, listingId, token, activeContext) {
    const result = [];
    const listingRes = await pool.query(`SELECT feed_to_property24, property24_ref1, property24_ref2,
    ...
    const hasEntegralRef = !!(listing.listing_payload?.['EntegralId'] ||
        listing.listing_payload?.['entegral_id']);
    if (hasP24Ref || listing.feed_to_property24) {
        const pr = { portal: 'Property24', withdrawOk: null, withdrawError: null };
        const r = await callListingApi(`/listings/${listingId}/publish-to-property24`, 'POST', token, {}, activeContext);
        pr.withdrawOk = r.ok;
        if (!r.ok)
            pr.withdrawError = extractApiError(r.body);
        result.push(pr);
    }
    if (hasPpRef || listing.feed_to_private_property) {
        const pr = { portal: 'Private Property', withdrawOk: null, withdrawError: null };
    ...
    if (hasKwwRef || listing.feed_to_kww) {
        const pr = { portal: 'KWW', withdrawOk: null, withdrawError: null };
    ...
    if (hasEntegralRef || listing.feed_to_entegral) {
        const pr = { portal: 'Entegral', withdrawOk: null, withdrawError: null };
...
function checkAdminPermission(perms) {
...
async function resolveMarketCenterSourceId(rawId) {
    const trimmed = String(rawId ?? '').trim();
    if (!trimmed)
        return null;
    try {
        const pool = getRequiredPgPool();
        const result = await pool.query(`SELECT source_market_center_id
...
async function marketCenterIdsMatch(left, right) {
...
function officeAdminAllowedMcId(perms) {
    return perms.marketCenterId ?? perms.homeMcId ?? null;
}
async function resolveAssociateId(rawId) {
    ...
    try {
        const pool = getRequiredPgPool();
        const result = await pool.query(`SELECT id::text
...
function extractP24Message(value) {
    if (!value)
        return null;
    if (typeof value === 'string') {
        const t = value.trim();
...
async function removeAgentFromProperty24(agent) {
    ...
    if (cityId !== null)
        deactivatePayload.cityId = cityId;
    if (idNumber)
        deactivatePayload.idNumber = idNumber;
    if (fidelityFundCertificationNumber)
        deactivatePayload.fidelityFundCertificationNumber = fidelityFundCertificationNumber;
    if (jobTitle)
        deactivatePayload.jobTitle = jobTitle;
    try {
...
async function removeAgentFromPrivateProperty(pool, agent) {
    ...
    const branchGuids = new Set();
    const envBranchGuid = toText(ppDefaultBranchGuid);
    if (envBranchGuid)
        branchGuids.add(envBranchGuid);
    let branchGuid = envBranchGuid ?? '';
    if (agent.source_market_center_id) {
        try {
            const mc = await pool.query(`SELECT private_property_id::text AS pp_id
         FROM migration.core_market_centers
         WHERE source_market_center_id = $1
         LIMIT 1`, [agent.source_market_center_id]);
        ...
        for (const branchId of branchGuids) {
            ...
            for (const endpoint of candidateUrls) {
                try {
                    ...
                    const resultLooksGood = !resultMatch ||
                        resultText === 'true' ||
                        resultText === '1' ||
                        resultText === 'success' ||
...
async function removeAgentFromPortals(pool, agent) {
...
// ─────────────────────────────────────────────────────────────────────────────
async function ensureDeregAuditTable() {
    try {
        const pool = getRequiredPgPool();
        await pool.query(`
...
ensureDeregAuditTable().catch(() => undefined);
async function ensureReactivationAuditTable() {
    try {
        const pool = getRequiredPgPool();
        await pool.query(`
...
router.get('/mc-agents/:mcSourceId', resolvePermissions, async (req, res) => {
    ...
    try {
       ...
       ORDER BY a.full_name`, [effectiveMcSourceId]);
...
router.get('/agent-all-listings/:associateId', resolvePermissions, async (req, res) => {
    ...
    try {
      ...
      WHERE LOWER(TRIM(COALESCE(cl.status_name, ''))) IN ('active', '1')
       ORDER BY la.is_primary DESC, cl.listing_number`, [associateId]);
...
router.post('/withdraw-jobs', resolvePermissions, async (req, res) => {
    ...
    const agentRes = await pool.query(`SELECT id::text, full_name FROM migration.core_associates WHERE id = $1`, [agentId]);
    if (!agentRes.rows[0]) {
    ...
    const listingsRes = await pool.query(`SELECT cl.id::text, la.is_primary
     FROM migration.core_listings cl
     JOIN migration.listing_agents la ON la.listing_id = cl.id AND la.associate_id = $1
     WHERE LOWER(TRIM(COALESCE(cl.status_name, ''))) IN ('active', '1')`, [agentId]);
...
router.post('/deactivate', resolvePermissions, async (req, res) => {
    ...
    try {
        ...
        const activeContext = typeof req.headers['x-active-context'] === 'string'
            ...
            : null;
        const agentRes = await pool.query(`SELECT
         id::text,
         full_name,
         first_name,
         last_name,
       ...
       LIMIT 1`, [agentId]);
        ...
        const activeAssocRes = await pool.query(`SELECT cl.id::text AS listing_id,
              cl.listing_number,
              la.is_primary
         FROM migration.core_listings cl
         JOIN migration.listing_agents la ON la.listing_id = cl.id
        WHERE la.associate_id = $1
          AND LOWER(TRIM(COALESCE(cl.status_name, ''))) IN ('active', '1')`, [agentId]);
        ...
        if (activeSecondary.length > 0) {
            ...
            if (secondaryListingIds.length > 0) {
                await pool.query(`DELETE FROM migration.listing_agents
            WHERE associate_id = $1
              AND is_primary = false
              AND listing_id = ANY($2::bigint[])`, [agentId, secondaryListingIds]);
                if (!token) {
                ...
                for (const listingId of secondaryListingIds) {
                    ...
                    for (const portalResult of portalResults) {
                        if (portalResult.withdrawOk === false) {
                            syncFailures.push(`${portalResult.portal} listing ${listingId}: ${portalResult.withdrawError ?? 'sync failed'}`);
                        }
                    }
                }
        ...
        if (!portalSync.allSucceeded && token) {
            ...
            if (listingNumbers.length > 0) {
                const listingsRes = await pool.query(`SELECT id::text AS id
             FROM migration.core_listings
            WHERE listing_number = ANY($1::text[])`, [listingNumbers]);
        ...
        const updateRes = await pool.query(`UPDATE migration.core_associates
       ...
       RETURNING id::text, full_name`, [agentId, property24Status, privatePropertyStatus]);
        ...
        await pool.query(`DELETE FROM migration.listing_agents
       WHERE associate_id = $1
         AND is_primary = false`, [agentId]);
        // Write deregistration audit log
        await pool.query(`INSERT INTO migration.agent_deregistration_log
         (job_id, associate_id, associate_name, reason, requested_by)
       VALUES ($1, $2, $3, $4, $5)`, [withdrawJobId, agentId, agentName, reason, req.user?.email ?? 'unknown']).catch(() => undefined); // Non-fatal
...
router.post('/reactivate', resolvePermissions, async (req, res) => {
    ...
    try {
        ...
        const existing = await client.query(`SELECT source_market_center_id
       FROM migration.core_associates
       WHERE id = $1
       LIMIT 1`, [agentId]);
        if (!existing.rows[0]) {
        ...
        const mcLookup = await client.query(`SELECT id::text AS id
       FROM migration.core_market_centers
       WHERE source_market_center_id = $1
       LIMIT 1`, [sourceMarketCenterId]);
        const marketCenterId = mcLookup.rows[0]?.id ? Number(mcLookup.rows[0].id) : null;
        const result = await client.query(`UPDATE migration.core_associates
       SET
       ...
       RETURNING id::text, full_name`, [
        ...
        await client.query(`INSERT INTO migration.agent_reactivation_log
...
// ─────────────────────────────────────────────────────────────────────────────
async function runWithdrawalJob(job, listingRows, agentId, token, activeContext) {
    const pool = getRequiredPgPool();
    job.status = 'running';
    for (const row of listingRows) {
        ...
        try {
            if (row.is_primary) {
                ...
                const listingRes = await pool.query(`SELECT id::text, listing_number,
             address_line, street_number, street_name, suburb,
             feed_to_property24, property24_ref1, property24_ref2,
             feed_to_private_property, private_property_ref1,
             feed_to_kww, kww_property_reference, kww_ref1,
             feed_to_entegral, listing_payload
           FROM migration.core_listings WHERE id = $1 LIMIT 1`, [listingId]);
                ...
                await pool.query(`UPDATE migration.core_listings SET status_name = 'Withdrawn', updated_at = NOW() WHERE id = $1`, [listingId]);
           ...
           WHERE id = $1`, [listingId]);
                ...
                const infoRes = await pool.query(`SELECT listing_number, address_line, street_number, street_name, suburb,
                  feed_to_property24, property24_ref1, property24_ref2,
                  feed_to_private_property, private_property_ref1,
                  feed_to_kww, kww_property_reference, kww_ref1,
                  feed_to_entegral, listing_payload
           FROM migration.core_listings WHERE id = $1 LIMIT 1`, [listingId]);
                ...
                await pool.query(`DELETE FROM migration.listing_agents WHERE listing_id = $1 AND associate_id = $2`, [listingId, agentId]);
...
router.post('/reactivation-log', resolvePermissions, async (req, res) => {
    ...
    try {
        await pool.query(`INSERT INTO migration.agent_reactivation_log
         (associate_id, associate_name, old_market_center_id, new_market_center_id, requested_by)
       VALUES ($1, $2, $3, $4, $5)`, [
            agentId,
...
//# sourceMappingURL=agentDeregistration.js.map