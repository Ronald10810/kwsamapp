...
import { applyCapRefresh, canApplyCapRefresh, dryRunCapRefresh, loadCapRefreshRoleNames, } from '../services/capRefresh/index.js';
...
function reportingDateSql(alias) {
    return `CASE
...
function normalizeAssociateSplitPct(rawSplit) {
    if (rawSplit <= 0)
        return 70;
    if (rawSplit <= 1)
        return rawSplit * 100;
...
function normalizeTeamSplitPct(rawSplit) {
    if (rawSplit <= 0)
        return 0;
    if (rawSplit <= 1)
...
function normalizeAgentSplitPct(rawSplit, splitSum, count) {
...
function inferTransactionTypeFromAgents(agents) {
    const roles = new Set(agents
        .map((agent) => toText(agent?.agent_role)?.trim().toLowerCase())
        .filter((role) => Boolean(role)));
    if (roles.has('both') || (roles.has('buyer') && roles.has('seller')))
        return 'Both';
    if (roles.has('buyer'))
        return 'Buyer';
    if (roles.has('seller'))
        return 'Seller';
    if (roles.has('referral'))
        return 'Referral';
    if (roles.has('other'))
        return 'Other';
    return null;
}
function deriveSaleType(transactionType, saleType) {
    if (saleType)
        return saleType;
...
function normalizePartyContactInput(value) {
...
function normalizePartyContactListInput(value) {
...
function buildManualTransactionId() {
...
async function resolveTeamContextSourceId(db, activeContextId, associateDbId) {
    if (!associateDbId)
        return null;
    const teamToken = parseTeamContextToken(activeContextId);
    if (!teamToken)
        return null;
    const allowedTeamsResult = await db.query(`WITH allowed_source_teams AS (
     ...
     INNER JOIN allowed_source_teams a ON a.source_team_id = t.source_team_id`, [associateDbId]);
    const normalizedToken = normalizeKey(teamToken);
    const matched = allowedTeamsResult.rows.find((row) => normalizeKey(row.source_team_id) === normalizedToken || normalizeKey(row.team_db_id) === normalizedToken);
...
async function ensureTransactionAuxTables(db) {
    await db.query(`
...
async function canAccessTransactionByScope(db, transactionId, perms, teamSourceId = null) {
    if (perms.scope === 'GLOBAL')
        return true;
    if (perms.scope === 'OWN') {
        if (teamSourceId) {
            const team = await db.query(`SELECT 1
           FROM migration.transaction_agents ta
           INNER JOIN migration.core_associates ca ON ca.id = ta.associate_id
          WHERE ta.transaction_id = $1
            AND ca.source_team_id = $2
          LIMIT 1`, [transactionId, teamSourceId]);
            return (team.rowCount ?? 0) > 0;
        }
        if (!perms.associateDbId)
            return false;
        const own = await db.query(`SELECT 1
         FROM migration.transaction_agents ta
        WHERE ta.transaction_id = $1
          AND ta.associate_id = $2
        LIMIT 1`, [transactionId, perms.associateDbId]);
    ...
    if (!mcKey)
        return false;
    const mcLookup = await db.query(`SELECT id::text FROM migration.core_market_centers WHERE source_market_center_id = $1 LIMIT 1`, [effectiveScopeMarketCenterId]);
    const mcDbId = mcLookup.rows[0]?.id ? Number(mcLookup.rows[0].id) : null;
    const matched = await db.query(`SELECT 1
      ...
      LIMIT 1`, [transactionId, mcKey, mcDbId]);
    return (matched.rowCount ?? 0) > 0;
}
function buildCapRefreshActor(req, perms, roleNames) {
    return {
...
function validateAgentSplits(agents) {
    if (agents.length === 0) {
        return null;
    }
    let splitSum = 0;
...
async function getNextTransactionNumber(db) {
...
function normalizePocketListingToken(...candidates) {
...
async function getNextPocketListingNumber(db) {
    const result = await db.query(`
    WITH maxima AS (
...
async function pocketListingExistsElsewhere(db, pocketListingNumber, transactionIdToExclude) {
...
async function handleNextPocketListingNumber(_req, res) {
    if (!pool) {
    ...
    try {
      ...
      ''
        ...
        const summarySaleTypeFilterSql = summaryCategoryFilter === 'sales'
            ...
            : 'TRUE';
        const exists = await pool.query(`SELECT to_regclass('migration.core_transactions') AS exists`);
        ...
        const teamSourceId = perms.scope === 'OWN'
            ...
            : null;
        let visibleTransactionIds = null;
        if (perms.scope === 'OWN') {
            if (teamSourceId) {
                const teamResult = await pool.query(`SELECT DISTINCT ta.transaction_id::text
             FROM migration.transaction_agents ta
             INNER JOIN migration.core_associates ca ON ca.id = ta.associate_id
            WHERE ca.source_team_id = $1`, [teamSourceId]);
                ...
                const ownResult = await pool.query(`SELECT DISTINCT transaction_id::text
             FROM migration.transaction_agents
            WHERE associate_id = $1`, [perms.associateDbId]);
            ...
            const mcLookup = await pool.query(`SELECT id::text FROM migration.core_market_centers WHERE source_market_center_id = $1 LIMIT 1`, [mcSource]);
            const mcDbId = mcLookup.rows[0]?.id ? Number(mcLookup.rows[0].id) : null;
            const mcResult = await pool.query(`SELECT DISTINCT ct.id::text
           FROM migration.core_transactions ct
          ...
          WHERE REGEXP_REPLACE(LOWER(TRIM(COALESCE(pmc.source_market_center_id, ''))), '[^a-z0-9]+', '', 'g') = REGEXP_REPLACE(LOWER(TRIM($1)), '[^a-z0-9]+', '', 'g')
             ...
             OR ($2::int IS NOT NULL AND (ct.primary_market_center_id = $2 OR ca.market_center_id = $2))`, [mcSource, mcDbId]);
        ...
        const reportingWindowResult = await pool.query(`
      ...
      `);
        ...
        const totalsParams = scopedIds ? [scopedIds] : [];
        const mtdParams = scopedIds
            ? [...reportingWindowParams, scopedIds]
            : [...reportingWindowParams];
        const statusParams = scopedIds ? [scopedIds] : [];
        const typeParams = scopedIds ? [scopedIds] : [];
        const marketParams = scopedIds
            ? [...reportingWindowParams, scopedIds]
            : [...reportingWindowParams];
        const associateParams = scopedIds
            ? [...reportingWindowParams, scopedIds]
            : [...reportingWindowParams];
        const closingsParams = scopedIds ? [scopedIds] : [];
    ...
    try {
        ...
        if (perms.scope === 'OWN') {
          ...
          WHERE pta.transaction_id = ct.id
            ...
            let mcDbId = null;
            if (mcSourceId) {
                const mcLookup = await pool.query(`SELECT id::text FROM migration.core_market_centers WHERE source_market_center_id = $1 LIMIT 1`, [mcSourceId]);
        ...
        const [totalResult, dataResult] = await Promise.all([
            pool.query(`SELECT COUNT(DISTINCT ct.id)::text AS total
         FROM migration.core_transactions ct
         LEFT JOIN migration.transaction_agents ta ON ta.transaction_id = ct.id
         LEFT JOIN migration.core_associates ca ON ca.id = ta.associate_id
         LEFT JOIN migration.core_market_centers mc ON mc.source_market_center_id = ca.source_market_center_id
         ...
         LEFT JOIN migration.core_associates ca ON ca.id = ta.associate_id
         LEFT JOIN migration.core_market_centers mc ON mc.source_market_center_id = ca.source_market_center_id
         ORDER BY ${reportingDateSql('t')} DESC NULLS LAST, t.id DESC, ta.sort_order ASC`, params),
        ]);
...
router.post('/', resolvePermissions, async (req, res) => {
    ...
    if (!transactionStatus || !ALLOWED_STATUSES.includes(transactionStatus)) {
    ...
    try {
        ...
        const statusChangeDate = perms.isRegionalAdmin
            ...
            : txDate;
        const sourceAssociateIds = Array.from(new Set(agents
            .map((a) => toText(a?.source_associate_id))
            .filter((v) => !!v)));
        const assocRows = sourceAssociateIds.length > 0
            ? await pool.query(`SELECT id::text, source_associate_id, source_market_center_id
             FROM migration.core_associates
            WHERE source_associate_id = ANY($1::text[])`, [sourceAssociateIds])
            : { rows: [] };
        if (perms.scope === 'OWN') {
        ...
        if (agents.length > 0 && agents[0].source_associate_id) {
            const assocLookup = await pool.query(`SELECT source_market_center_id FROM migration.core_associates WHERE source_associate_id = $1 LIMIT 1`, [agents[0].source_associate_id]);
            if (assocLookup.rows[0]?.source_market_center_id) {
                const mcLookup = await pool.query(`SELECT id::text AS id FROM migration.core_market_centers WHERE source_market_center_id = $1 LIMIT 1`, [assocLookup.rows[0].source_market_center_id]);
      ...
      `, [
        ...
        for (let i = 0; i < agents.length; i++) {
            ...
            const outsideAgency = agent?.outside_agency;
            ...
            if (sourceAssociateId) {
                const assocLookup = await pool.query(`SELECT id::text AS id FROM migration.core_associates WHERE source_associate_id = $1 LIMIT 1`, [sourceAssociateId]);
                associateId = assocLookup.rows[0]?.id ? Number(assocLookup.rows[0].id) : null;
            }
            const agentInsert = await pool.query(`INSERT INTO migration.transaction_agents (transaction_id, associate_id, source_associate_id, agent_role, split_percentage, sort_order, created_at, updated_at)
         VALUES ($1, $2, $3, $4, $5, $6, NOW(), NOW()) RETURNING id::text`, [transactionId, associateId, sourceAssociateKey, agentRole, splitPct, i]);
            if (isOutsideReferral) {
                ...
                await pool.query(`INSERT INTO migration.outside_agency_contacts (transaction_agent_id, transaction_id, first_name, last_name, email, phone, agency_name)
        ...
        await pool.query(`INSERT INTO migration.transaction_status_history (
         transaction_id,
         previous_status,
         new_status,
         changed_at,
       ...
       VALUES ($1, NULL, $2, NOW(), $3, $4, $5)`, [transactionId, transactionStatus, changedByName, changedByUserId, changedByEmail]);
...
router.put('/:id', resolvePermissions, async (req, res) => {
    ...
    try {
        ...
        const existing = await pool.query(`SELECT transaction_status, transaction_type, sale_type, listing_number, source_listing_id
       FROM migration.core_transactions
       WHERE id = $1
       LIMIT 1`, [id]);
        const existingPocketListingNumber = normalizePocketListingToken(existing.rows[0]?.listing_number ?? null, existing.rows[0]?.source_listing_id ?? null);
        ...
        const effectiveTransactionType = normalizeTransactionType(transactionType ?? inferTransactionTypeFromAgents(agents) ?? existingTransactionType);
      ...
      `, [
        ...
        if (agents.length >= 0) {
            ...
            for (let i = 0; i < agents.length; i++) {
                ...
                const agentInsert = await pool.query(`INSERT INTO migration.transaction_agents (transaction_id, associate_id, source_associate_id, agent_role, split_percentage, sort_order, created_at, updated_at)
           VALUES ($1, $2, $3, $4, $5, $6, NOW(), NOW()) RETURNING id::text`, [id, associateId, sourceAssociateKey, agentRole, splitPct, i]);
                if (isOutsideReferral) {
                    let firstName = toText(outsideAgency?.first_name);
                    let lastName = toText(outsideAgency?.last_name);
                    const fullName = associateName ?? null;
        ...
        if (statusChanged) {
         ...
         VALUES ($1, $2, $3, NOW(), $4, $5, $6)`, [id, previousStatus, transactionStatus, changedByName, changedByUserId, changedByEmail]);
        }
        ...
        const snapshotExists = await pool.query(`SELECT to_regclass('migration.mc_dashboard_daily_snapshots') AS exists`);
        if (snapshotExists.rows[0]?.exists) {
            await pool.query(`DELETE FROM migration.mc_dashboard_daily_snapshots WHERE snapshot_date = $1::date`, [getTodayInAppTimeZone()]);
        }
...
router.get('/:id/documents', resolvePermissions, async (req, res) => {
    ...
    try {
       ...
       ORDER BY created_at DESC, id DESC`, [id]);
        const sourceTxIdResult = await pool.query(`SELECT source_transaction_id::text, transaction_number::text FROM migration.core_transactions WHERE id = $1 LIMIT 1`, [id]);
        ...
        const stagedExistsResult = await pool.query(`SELECT to_regclass('staging.transaction_documents_raw') AS exists`);
        let stagedRows = [];
        if (sourceTransactionId && stagedExistsResult.rows[0]?.exists) {
            const stagedResult = await pool.query(`SELECT
           source_document_id,
           source_transaction_document_type_id,
           transaction_document_type,
           file_name,
        ...
        const publicDocTablesResult = await pool.query(`SELECT
        ...
        const publicDocsAvailable = Boolean(publicDocTablesResult.rows[0]?.transaction_documents_exists
        ...
        if (txNumber && publicDocsAvailable) {
            const publicResult = await pool.query(`SELECT
...
router.post('/:id/documents/upload', resolvePermissions, async (req, res, next) => {
    ...
    try {
        ...
        const insert = await pool.query(`INSERT INTO migration.transaction_documents (
         transaction_id,
         source_document_id,
         source_transaction_document_type_id,
         transaction_document_type,
       ...
       RETURNING id::text`, [id, sourceDocumentId, sourceDocumentTypeId, documentType, originalName, documentUrl, previewUrl]);
...
router.post('/:id/documents', resolvePermissions, async (req, res) => {
    ...
    try {
       ...
       VALUES ($1,$2,$3,$4,$5,$6,$7,NOW(),NOW())
       RETURNING id::text`, [id, sourceDocumentId, sourceDocumentTypeId, documentType, fileName, documentUrl, previewUrl]);
...
router.delete('/:id/documents/:documentId', resolvePermissions, async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query(`UPDATE migration.transaction_documents
          SET deleted_at = NOW(), updated_at = NOW()
        WHERE id = $1 AND transaction_id = $2 AND deleted_at IS NULL`, [documentId, id]);
...
async function fetchAssociateCurrentCapRemainingByIds(db, associateIds) {
...
async function fetchTeamCurrentCapRemainingByIds(db, teamIds) {
    if (teamIds.length === 0)
        return new Map();
    const result = await db.query(`
    WITH cap_base AS (
...
async function resolveTeamIdsBySourceTeamIds(db, sourceTeamIds) {
    ...
    for (const row of result.rows) {
        ...
        for (const token of candidateTokens) {
            if (!normalizedTokens.includes(token))
                continue;
            if (!lookup.has(token))
                lookup.set(token, teamId);
        }
    }
    return lookup;
}
async function buildQuickSummaryDisplayCapRemainingLookup(db, rows) {
    const sourceTeamIds = Array.from(new Set(rows
        .flatMap((row) => [
    ...
    const associateIds = Array.from(new Set(rows
        .map((row) => Number(row.associate_id))
        .filter((value) => Number.isFinite(value) && value > 0)));
    const teamIds = Array.from(new Set(rows.flatMap((row) => {
    ...
    for (const row of rows) {
        ...
        if (!Number.isFinite(transactionAgentId))
            continue;
        const explicitTeamId = Number(row.associate_team_id);
        ...
        displayLookup.set(transactionAgentId, hasApplicableTeamCap
...
router.post('/preview-cap-remaining', resolvePermissions, async (req, res) => {
    ...
    const associateIdsInput = Array.isArray(req.body?.associate_ids)
        ...
        : [];
    const associateIds = Array.from(new Set(associateIdsInput
    ...
    try {
      ...
      `, [associateIds]);
        ...
        const teamIds = Array.from(new Set(validAssociateIds
            .map((associateId) => associateLookup.get(associateId)?.team_id ?? null)
            .filter((teamId) => teamId != null && Number.isFinite(teamId) && teamId > 0)));
      ...
      `, [id]);
        ...
        `, [id]);
            if (fallbackSource.rows.length > 0) {
                ...
                const associateIds = Array.from(new Set(fallbackSource.rows
                    .map((row) => Number(row.associate_id))
                    .filter((value) => Number.isFinite(value) && value > 0)));
                const teamIds = Array.from(new Set(fallbackSource.rows
                ...
                const fallbackItems = fallbackSource.rows.map((row, index) => {
                    ...
                    const toMoneyText = (value) => value.toFixed(2);
        ...
        const toMoneyText = (value) => value.toFixed(2);
        const displayCapRemainingLookup = await buildQuickSummaryDisplayCapRemainingLookup(pool, result.rows);
