...
router.put('/:id', resolvePermissions, async (req, res) => {
    ...
    try {
        ...
        const existing = await pool.query(`SELECT transaction_status, transaction_type, sale_type, listing_number, source_listing_id, transaction_number
       FROM migration.core_transactions
       WHERE id = $1
       LIMIT 1`, [id]);
...
router.post('/:id/recalculate', resolvePermissions, async (req, res) => {
    ...
    try {
        ...
        const tx = await pool.query(`SELECT transaction_number FROM migration.core_transactions WHERE id = $1 LIMIT 1`, [id]);
