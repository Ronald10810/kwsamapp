...
router.get('/', async (req, res) => {
    ...
    try {
        ...
        if (searchInput.length > 0) {
            ...
            whereClauses.push(`(t.transaction_number ILIKE ${p} OR t.address ILIKE ${p} OR t.city ILIKE ${p} OR a.full_name ILIKE ${p} OR mc.name ILIKE ${p} OR t.listing_number ILIKE ${p})`);
        }
        ...
        const [totalResult, dataResult] = await Promise.all([
            pool.query(`SELECT COUNT(*)::text AS total
...
router.post('/', async (req, res) => {
    ...
    try {
        const assocLookup = sourceAssociateId
            ? await pool.query(`SELECT id::text AS id, source_market_center_id FROM migration.core_associates WHERE source_associate_id = $1 LIMIT 1`, [sourceAssociateId])
            : { rows: [] };
        ...
        const mcLookup = sourceMarketCenterId
            ? await pool.query(`SELECT id::text AS id FROM migration.core_market_centers WHERE source_market_center_id = $1 LIMIT 1`, [sourceMarketCenterId])
            : { rows: [] };
