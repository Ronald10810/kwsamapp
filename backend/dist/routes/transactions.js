    ...
    try {
      ...
      `, [
        ...
        for (let i = 0; i < agents.length; i++) {
           ...
           VALUES ($1, $2, $3, $4, $5, $6, NOW(), NOW()) RETURNING id::text`, [transactionId, associateId, sourceAssociateId, agentRole, splitPct, i]);
        ...
        if (agents.length >= 0) {
            ...
            for (let i = 0; i < agents.length; i++) {
                ...
                if (sourceAssociateId) {
                    ...
                    const associateId = assocLookup.rows[0]?.id ? Number(assocLookup.rows[0].id) : null;
                    const agentInsert = await pool.query(`INSERT INTO migration.transaction_agents (transaction_id, associate_id, source_associate_id, agent_role, split_percentage, sort_order, created_at, updated_at)
             VALUES ($1, $2, $3, $4, $5, $6, NOW(), NOW()) RETURNING id::text`, [id, associateId, sourceAssociateId, agentRole, splitPct, i]);
                    if (agentRole === 'Outside Agency Referral' && agent.outside_agency) {
                        const oa = agent.outside_agency;
                        await pool.query(`INSERT INTO migration.outside_agency_contacts (transaction_agent_id, transaction_id, first_name, last_name, email, phone, agency_name)
               VALUES ($1, $2, $3, $4, $5, $6, $7)`, [
