...
router.get('/summary', async (_req, res) => {
    ...
    `);
      ...
      `);
        ...
        const transactionCalculationsExists = await pool.query(`SELECT to_regclass('migration.transaction_agent_calculations') AS exists`);
        let marketCenterPerformance = { rows: [] };
        let associatePerformance = { rows: [] };
        let teamPerformance = { rows: [] };
        ...
        if (transactionCalculationsExists.rows[0]?.exists) {
          ...
          `);
                ...
                marketCenterPerformance = await pool.query(`
          WITH ${transactionAgentCalculationDedupCte},
          limits AS (
            SELECT
              date_trunc('month', CURRENT_DATE)::date AS month_start,
          ...
          LIMIT 13
          `);
                associatePerformance = await pool.query(`
          WITH ${transactionAgentCalculationDedupCte},
          limits AS (
            SELECT
              date_trunc('month', CURRENT_DATE)::date AS month_start,
          ...
          LIMIT 15
          `);
                teamPerformance = await pool.query(`
          WITH ${transactionAgentCalculationDedupCte},
          limits AS (
            SELECT
              date_trunc('month', CURRENT_DATE)::date AS month_start,
