...
async function getCurrentAssociateId(email) {
    if (!pool)
        return null;
...
async function enqueueListingExpiryRemindersForAssociate(associateId) {
    if (!pool)
        return;
    await pool.query(`WITH reminder_days AS (
...
router.get('/', resolvePermissions, async (req, res) => {
    ...
    if (req.permissions?.scope === 'MARKET_CENTRE' && req.permissions.marketCenterId) {
        ...
        whereClauses.push(`(entity_type <> 'listing' OR EXISTS (
         SELECT 1
    ...
    const countsResult = await pool.query(`SELECT
     ...
     FROM migration.in_app_notifications
     WHERE ${countWhereClauses.join(' AND ')}`, countParams);
    return res.json({
...
router.post('/:id/read', async (req, res) => {
    ...
    const result = await pool.query(`UPDATE migration.in_app_notifications
        SET is_read = true,
            read_at = COALESCE(read_at, NOW()),
            updated_at = NOW()
      WHERE id = $1
        AND associate_id = $2
      RETURNING id::text`, [notificationId, associateId]);
