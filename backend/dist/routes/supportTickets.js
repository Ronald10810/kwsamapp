...
const LINK_TYPES = ['listing', 'transaction', 'associate', 'agent', 'market_centre'];
...
const normalizedSql = (valueSql) => `REGEXP_REPLACE(LOWER(TRIM(COALESCE(${valueSql}, ''))), '[^a-z0-9]+', '', 'g')`;
function canUseSupportTickets(perms) {
    return perms.isOfficeAdmin || perms.isRegionalAdmin;
}
function canUpdateStatus(perms) {
    return perms.isRegionalAdmin;
}
function canCreateTicket(perms) {
...
function resolveFirstName(nameOrEmail) {
...
function resolveDisplayName(nameOrEmail, emailFallback) {
    const raw = String(nameOrEmail ?? '').trim();
    if (raw && !raw.includes('@'))
        return raw;
...
function entityPath(type, id) {
    ...
    if (type === 'listing')
        return `/listings?review=${encoded}`;
    if (type === 'transaction')
        return `/transactions?edit=${encoded}`;
    if (type === 'associate' || type === 'agent')
        return `/agents?edit=${encoded}`;
    return `/market-centers?search=${encoded}`;
}
async function resolveMarketCentreName(sourceMarketCentreId) {
    const pool = getRequiredPgPool();
    const result = await pool.query(`SELECT COALESCE(NULLIF(TRIM(mc.name), ''), mc.source_market_center_id) AS name
      ...
      LIMIT 1`, [sourceMarketCentreId]);
    return result.rows[0]?.name?.trim() || sourceMarketCentreId;
}
function normalizeLinkedPathForEdit(linkedPath) {
    if (!linkedPath)
        return null;
    try {
...
function requiredLinkedTypeForSection(section) {
...
function resolveSourceMarketCenterId(perms, requestedSourceMc) {
...
function applyScopeFilter(perms, sourceMcSql, whereClauses, params, requestedMc) {
...
async function sendSupportTicketCreatedEmail(input) {
    if (!env.support.enabled)
        return { sent: false, error: 'Support email notifications are disabled.' };
    if (!env.support.fromEmail)
        return { sent: false, error: 'Support sender email is not configured.' };
    try {
...
async function nextTicketNumber() {
    const pool = getRequiredPgPool();
    const seqResult = await pool.query(`SELECT nextval('app.support_ticket_number_seq')::text AS seq`);
    const seq = Number(seqResult.rows[0]?.seq ?? '0');
    return `MAPP-${String(Number.isFinite(seq) ? seq : 0).padStart(6, '0')}`;
}
async function readTicketWithAccess(ticketId, perms) {
    const pool = getRequiredPgPool();
    const params = [ticketId];
    const where = ['t.id::text = $1'];
    applyScopeFilter(perms, 't.source_market_center_id', where, params);
    const result = await pool.query(`SELECT t.id::text, t.ticket_number, t.source_market_center_id, t.title, t.status,
...
async function resolveLinkedEntity(type, id) {
    const pool = getRequiredPgPool();
    if (type === 'listing') {
        const result = await pool.query(`SELECT id::text, listing_number, address_line, suburb, city, source_market_center_id
        ...
        if (!row)
            throw new Error('Linked listing not found.');
    ...
    if (type === 'transaction') {
        const result = await pool.query(`SELECT id::text, source_transaction_id, transaction_number, address, source_market_center_id
    ...
    if (type === 'associate' || type === 'agent') {
        const result = await pool.query(`SELECT id::text, source_associate_id, full_name, email, source_market_center_id
        ...
        const row = result.rows[0];
        if (!row)
            throw new Error(`Linked ${type} not found.`);
        return {
            label: `${type === 'agent' ? 'Agent' : 'Associate'}: ${row.full_name ?? row.email ?? row.id}`,
            sourceMarketCenterId: row.source_market_center_id,
        };
    }
    const result = await pool.query(`SELECT source_market_center_id, name
       FROM migration.core_market_centers
      WHERE source_market_center_id = $1 OR id::text = $1
      LIMIT 1`, [id]);
    const row = result.rows[0];
    if (!row)
        throw new Error('Linked market centre not found.');
    return {
...
router.get('/linked-records/search', async (req, res) => {
    ...
    if (type === 'listing') {
      ...
      OR COALESCE(cl.source_listing_id, '') ILIKE $${params.length}
        ...
        params.push(limit);
        const result = await pool.query(`SELECT cl.id::text AS id,
    ...
    if (type === 'transaction') {
      ...
      OR ct.id::text ILIKE $${params.length}
        ...
        params.push(limit);
        const result = await pool.query(`SELECT ct.id::text,
    ...
    if (type === 'associate' || type === 'agent') {
      ...
      OR ca.id::text ILIKE $${params.length}
        ...
        params.push(limit);
        const result = await pool.query(`SELECT ca.id::text,
    ...
    params.push(limit);
    const result = await pool.query(`SELECT mc.id::text, mc.source_market_center_id, mc.name
       FROM migration.core_market_centers mc
      WHERE ${where.join(' AND ')}
...
router.get('/', async (req, res) => {
    ...
    const [countResult, dataResult] = await Promise.all([
        pool.query(`SELECT COUNT(*)::text AS total FROM app.support_tickets t ${whereSql}`, countParams),
        pool.query(`SELECT t.id::text,
              t.ticket_number,
...
router.post('/', async (req, res) => {
    ...
    const pool = getRequiredPgPool();
    const insert = await pool.query(`INSERT INTO app.support_tickets (
       ticket_number,
    ...
    await pool.query(`INSERT INTO app.support_ticket_activity (
       ticket_id,
       activity_type,
       body,
       created_by_email,
...
router.patch('/:id/assignment', async (req, res) => {
    ...
    const pool = getRequiredPgPool();
    let allocatedName = null;
    if (allocatedEmail) {
        const assignee = await pool.query(`SELECT LOWER(TRIM(COALESCE(a.kwsa_email, a.private_email, a.email, ''))) AS email,
              NULLIF(TRIM(COALESCE(a.full_name, CONCAT_WS(' ', a.first_name, a.last_name), a.email, '')), '') AS name
         FROM migration.associate_roles ar
         JOIN migration.core_associates a ON a.id = ar.associate_id
        WHERE UPPER(REPLACE(ar.role_name, ' ', '_')) = 'REGIONAL_ADMIN'
    ...
    const updated = await pool.query(`UPDATE app.support_tickets
      ...
      RETURNING id::text, ticket_number, source_market_center_id, section, title, description,
                ...
                linked_entity_type, linked_entity_id, linked_entity_label, linked_entity_path,
                created_at::text, updated_at::text`, [req.params.id, allocatedEmail, allocatedName]);
    await pool.query(`INSERT INTO app.support_ticket_activity (
       ticket_id,
       activity_type,
       body,
...
router.get('/:id', async (req, res) => {
    ...
    const [ticketResult, activityResult, attachmentsResult] = await Promise.all([
        ...
        ORDER BY a.created_at DESC, a.id DESC`, [req.params.id]),
        pool.query(`SELECT at.id::text,
              at.file_name,
              at.mime_type,
              at.file_size::text,
              at.file_url,
        ...
        ORDER BY at.created_at DESC, at.id DESC`, [req.params.id]),
    ]);
    return res.json({
...
router.patch('/:id/status', async (req, res) => {
    ...
    const updated = await pool.query(`UPDATE app.support_tickets
      ...
      RETURNING id::text, ticket_number, source_market_center_id, section, title, description,
                ...
                allocated_to_email, allocated_to_name, resolved_by_email, resolved_by_name, resolved_at::text,
                linked_entity_type, linked_entity_id, linked_entity_label, linked_entity_path,
                created_at::text, updated_at::text`, [req.params.id, nextStatus, isResolvedState, req.user?.email ?? 'unknown@kwsa.co.za', req.user?.name ?? null]);
...
router.post('/:id/attachments', async (req, res) => {
    ...
    const noteParsed = attachmentSchema.safeParse(req.body ?? {});
    ...
    if (storageConfig.localUploadsEnabled) {
        ...
        const uploadResult = await uploadToGcs(file.buffer, file.originalname || 'attachment.bin', 'support-ticket', file.mimetype || 'application/octet-stream');
        fileUrl = uploadResult.publicUrl;
        gcsObjectName = uploadResult.objectName;
    }
    const pool = getRequiredPgPool();
    const insert = await pool.query(`INSERT INTO app.support_ticket_attachments (
       ticket_id,
       file_name,
       mime_type,
       file_size,
