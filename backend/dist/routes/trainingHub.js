...
function isTrainingHubEnabled() {
    return env.trainingHub.enabled;
}
function isRegionalAdminOnly(perms) {
...
function uploadFieldNameAllowed(fieldName, mimetype) {
    if (fieldName === 'video') {
...
async function storeUploadedMedia(file, prefix, localSubdir) {
...
router.post('/finalize', resolvePermissions, async (req, res) => {
    ...
    const pool = getRequiredPgPool();
    try {
        const seriesResult = await pool.query(`SELECT id::text
       ...
       LIMIT 1`, [seriesId]);
        ...
        const result = await pool.query(`INSERT INTO migration.training_hub_videos (
...
router.get('/', resolvePermissions, async (_req, res) => {
    ...
    try {
        const [seriesResult, videosResult] = await Promise.all([
            pool.query(`SELECT id::text, name, description, sort_order, created_at::text
         FROM migration.training_hub_series
         ORDER BY sort_order ASC, name ASC, created_at ASC`),
            pool.query(`SELECT v.id::text,
                v.series_id::text,
...
router.post('/series', resolvePermissions, async (req, res) => {
    ...
    const pool = getRequiredPgPool();
    try {
        const duplicate = await pool.query(`SELECT id::text
       FROM migration.training_hub_series
       ...
       LIMIT 1`, [name]);
        ...
        const result = await pool.query(`INSERT INTO migration.training_hub_series (name, description, sort_order, created_by)
       VALUES ($1, $2, $3, $4)
       RETURNING id::text, name, description, sort_order, created_by, created_at::text, updated_at::text`, [name, description, sortOrder, getCreatedByName(req)]);
        return res.status(201).json(result.rows[0]);
    }
    catch (error) {
        const message = error instanceof Error ? error.message : String(error);
...
router.patch('/series/:id', resolvePermissions, async (req, res) => {
    ...
    try {
       ...
       WHERE LOWER(TRIM(name)) = LOWER(TRIM($1))
         AND id <> $2
       LIMIT 1`, [name, seriesId]);
        if (duplicate.rows[0]) {
            return res.status(409).json({ error: 'A series with this name already exists.' });
        }
        const result = await pool.query(`UPDATE migration.training_hub_series
       SET name = $1,
           description = $2,
           sort_order = $3,
           updated_at = NOW()
       WHERE id = $4
       RETURNING id::text, name, description, sort_order, created_by, created_at::text, updated_at::text`, [name, description, sortOrder, seriesId]);
        if (!result.rows[0]) {
...
router.delete('/series/:id', resolvePermissions, async (req, res) => {
    ...
    const pool = getRequiredPgPool();
    try {
        const existing = await pool.query(`SELECT id::text
       FROM migration.training_hub_series
       WHERE id = $1
       LIMIT 1`, [seriesId]);
        if (!existing.rows[0]) {
        ...
        const linkedVideos = await pool.query(`SELECT COUNT(*)::text AS count
...
router.patch('/:id', resolvePermissions, async (req, res) => {
    ...
    const pool = getRequiredPgPool();
    try {
        const result = await pool.query(`UPDATE migration.training_hub_videos v
       SET title = $1,
           description = $2,
           sort_order = $3,
           updated_at = NOW()
       ...
       RETURNING v.id::text,
                 ...
                 v.sort_order,
                 v.created_by,
                 v.created_at::text`, [title, description, sortOrder, videoId]);
        if (!result.rows[0]) {
...
router.delete('/:id', resolvePermissions, async (req, res) => {
    ...
    const pool = getRequiredPgPool();
    try {
        const existing = await pool.query(`SELECT video_local_path, thumbnail_local_path
       FROM migration.training_hub_videos
       WHERE id = $1
       LIMIT 1`, [videoId]);
        if (!existing.rows[0]) {
...
//# sourceMappingURL=trainingHub.js.map