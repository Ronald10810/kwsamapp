...
export function decryptToken(data) {
    ...
    return (decipher.update(Buffer.from(encHex, 'hex'), undefined, 'utf8') +
        decipher.final('utf8'));
}
export async function saveToken(userEmail, t) {
    await ensureTable();
    await getRequiredPgPool().query(`INSERT INTO public.loom_user_tokens
...
export async function loadToken(userEmail) {
    await ensureTable();
    const { rows } = await getRequiredPgPool().query(`SELECT access_enc, refresh_enc, expires_at, loom_email, consent_accepted
     FROM public.loom_user_tokens WHERE user_email = $1`, [userEmail]);
    if (!rows[0])
        return null;
    const r = rows[0];
    return {
        accessToken: decryptToken(r.access_enc),
        refreshToken: r.refresh_enc ? decryptToken(r.refresh_enc) : null,
        expiresAt: r.expires_at ? new Date(r.expires_at) : null,
        loomEmail: r.loom_email,
        consentAccepted: r.consent_accepted ?? false,
    };
}
export async function setConsentAccepted(userEmail) {
    await ensureTable();
    await getRequiredPgPool().query(`UPDATE public.loom_user_tokens SET consent_accepted = TRUE, updated_at = NOW()
     WHERE user_email = $1`, [userEmail]);
}
export async function removeToken(userEmail) {
    await ensureTable();
    await getRequiredPgPool().query(`DELETE FROM public.loom_user_tokens WHERE user_email = $1`, [userEmail]);
}
...
function sameIssuerHost(lhs, rhs) {
    try {
...
async function reacquireAndSaveClientCredentialsToken(userEmail, loomEmail) {
...
async function requestToken(body, context) {
    try {
        const res = await axios.post(`${LOOM_IDP_URL}/connect/token`, body.toString(), {
...
export function buildLoomAuthUrl(state) {
 ...
 */
export async function acquireTokenRopc() {
...
// ---------------------------------------------------------------------------
async function resolveAccessToken(userEmail) {
    ...
    if (!stored) {
        ...
        if (serviceStored.expiresAt && Date.now() > serviceStored.expiresAt.getTime() - 60000) {
            const fresh = await acquireTokenRopc();
            const newExpiry = fresh.expires_in
                ? new Date(Date.now() + fresh.expires_in * 1000)
                : null;
    ...
    if (stored.refreshToken && stored.expiresAt) {
        if (Date.now() > stored.expiresAt.getTime() - 60000) {
...
// ---------------------------------------------------------------------------
function loomHeaders(accessToken, headerEmail = env.loom.integrationEmail) {
    const headers = {
...
function readAbpErrorMessage(text) {
    try {
...
function isNoMatchingUserMessage(msg) {
    return typeof msg === 'string' && /no\s+matching\s+user\s+found/i.test(msg);
}
// ---------------------------------------------------------------------------
// API call wrappers
// ---------------------------------------------------------------------------
export async function loomGet(userEmail, path, params) {
    ...
    if (!res.ok) {
        ...
        const abp = parsed;
        const errMsg = abp?.error?.message;
        const errDetail = abp?.error?.details;
...
export async function loomPost(userEmail, path, body) {
    let token = await resolveAccessToken(userEmail);
    let headerEmail = env.loom.integrationEmail;
...
export async function loomDelete(userEmail, path) {
...
export async function loomGetBinary(userEmail, path, params) {
    let token = await resolveAccessToken(userEmail);
    let headerEmail = env.loom.integrationEmail;
    const url = new URL(`${LOOM_API_BASE}${path}`);
    if (params) {
...
//# sourceMappingURL=loom-client.js.map