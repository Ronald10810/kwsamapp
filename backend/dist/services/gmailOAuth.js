...
function getOAuthClient() {
    ...
    return new google.auth.OAuth2(env.communications.googleClientId, env.communications.googleClientSecret, env.communications.googleRedirectUri);
}
...
export function isGmailOAuthConfigured() {
    return Boolean(env.communications.googleClientId
...
export async function saveGmailToken(input) {
    ...
    await getRequiredPgPool().query(`INSERT INTO ${TOKEN_TABLE}
...
export async function loadGmailToken(userEmail) {
    await ensureTable();
    const result = await getRequiredPgPool().query(`SELECT google_subject, connected_email, access_enc, refresh_enc, expires_at, scope_text, updated_at
       FROM ${TOKEN_TABLE}
      WHERE user_email = $1`, [userEmail.toLowerCase()]);
    if (!result.rows[0]) {
        return null;
    }
...
export async function removeGmailToken(userEmail) {
    await ensureTable();
