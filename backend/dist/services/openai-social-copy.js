...
export async function generateListingSocialCopy(input) {
...
function buildUserPrompt(input, attempt = 0, previousDraft = '', previousReasons = []) {
...
export function validateGeneratedSocialCopy(text, input) {
...
export function sanitizeSocialCopyText(text, listingPrice) {
...
function buildFallbackSocialCopy(input) {
    const lines = [];
...
function buildStatusIntro(input, emoji) {
    const marker = emoji ? ' ✨' : '';
    const status = input.statusPill.trim();
    const suburb = input.suburb?.trim() || input.cityProvince?.trim() || '';
    if (/^sold$/i.test(status))
        return `Sold${suburb ? ` in ${suburb}` : ''}${marker}`;
    if (/^price reduced$/i.test(status))
        return `Price Reduced${suburb ? ` in ${suburb}` : ''}${marker}`;
    if (/^under offer$/i.test(status) || /^pending$/i.test(status))
        return `${status}${suburb ? ` in ${suburb}` : ''}${marker}`;
    if (/^to let$/i.test(status) || /^new rental$/i.test(status))
        return `${status}${suburb ? ` in ${suburb}` : ''}${marker}`;
    if (/^rented$/i.test(status))
        return `Rented${suburb ? ` in ${suburb}` : ''}${marker}`;
    return `${status}${suburb ? ` in ${suburb}` : ''}${marker}`;
}
...
function buildHashtagLine(input) {
    const values = [
        input.statusPill,
        input.suburb,
        input.cityProvince,
...
function normalizePriceMentions(text, canonicalPrice) {
    if (!canonicalPrice)
        return text;
...
function buildFacebookFeatureNarrative(features, description) {
...
//# sourceMappingURL=openai-social-copy.js.map