...
function proposedValueSet(entry) {
    return {
...
function deltaValueSet(current, proposed) {
    return {
...
function valueSetsDiffer(current, proposed) {
...
export async function fetchCurrentEnvelopeTacRows(db, transactionAgentIds) {
...
export function buildCapRefreshDiffRows(currentByAgentId, proposedEntries, projected) {
    const rows = [];
    for (const entry of proposedEntries) {
        ...
        if (!valueSetsDiffer(current, proposed))
            continue;
        rows.push({
...
export function checkProtectedFieldMismatches(currentByAgentId, proposedEntries) {
    const violations = [];
    for (const entry of proposedEntries) {
...
//# sourceMappingURL=envelopeDiff.js.map