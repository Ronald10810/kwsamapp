import { buildPreCapCalculatedRowsWithMeta, fetchRawRows, groupByTransaction, } from '../transactionCalculations.js';
function buildEnvelopeKey(entry) {
    return `${entry.cap_progress_key}|${entry.row.cap_cycle_start_date}`;
}
function buildEnvelopeDescriptor(entry) {
...
export async function resolveCapRefreshContext(db, transactionId) {
    ...
    const envelopeKeySet = new Set(targetEntries
    ...
    const impactedEnvelopeMap = new Map();
...
export function buildEnvelopeKeyForEntry(entry) {
