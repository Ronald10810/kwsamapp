...
const { resolveCapRefreshContextMock, fetchCurrentEnvelopeTacRowsMock, } = vi.hoisted(() => ({
    resolveCapRefreshContextMock: vi.fn(),
    fetchCurrentEnvelopeTacRowsMock: vi.fn(),
}));
vi.mock('./envelopeResolver.js', async () => {
    const actual = await vi.importActual('./envelopeResolver.js');
    return {
        ...actual,
...
vi.mock('./envelopeDiff.js', async () => {
    const actual = await vi.importActual('./envelopeDiff.js');
    return {
        ...actual,
...
describe('cap refresh dry-run safety', () => {
    it('dry-run does not write TAC', async () => {
        ...
        const result = await dryRunCapRefresh(db, 1001);
    ...
    it('apply throws on protected field assertion failure before TAC persistence', async () => {
        ...
        await expect(applyCapRefresh(db, {
            associateDbId: '1',
...
//# sourceMappingURL=envelopeApply.test.js.map