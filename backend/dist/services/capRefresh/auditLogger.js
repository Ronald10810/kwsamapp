export async function startCapRefreshAudit(db, input) {
...
export async function completeCapRefreshAudit(db, auditId, completedAt, durationMs) {
    await db.query(`
...
export async function failCapRefreshAudit(db, auditId, completedAt, durationMs, errorMessage) {
    await db.query(`
    UPDATE migration.cap_refresh_audit
    SET success = false,
        completed_at = $2,
