...
function NavIcon({ name }: { name: NavIconName }) {
  ...
  switch (name) {
    ...
    case 'trainingHub':
      return (
        <svg viewBox="0 0 24 24" fill="none" className={base} aria-hidden="true">
          <path d="M4 6.5A2.5 2.5 0 0 1 6.5 4H20v16H6.5A2.5 2.5 0 0 1 4 17.5v-11Z" stroke="currentColor" strokeWidth="1.7" />
...
export default function Sidebar({ isOpen = false, onClose }: SidebarProps) {
  ...
  const links = [
    ...
    ...(isAdminRole ? [{ path: '/rentals', label: 'Rentals', icon: 'rentals' as NavIconName }] : []),
    ...
    ...(trainingHubEnabled ? [{ path: '/training-hub', label: 'Training Hub', icon: 'trainingHub' as NavIconName }] : []),
  ];
