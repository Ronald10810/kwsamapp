...
export type ListingFlyerMetric = {
  key: string;
  label: string;
  value: string;
  icon: 'bed' | 'bath' | 'garage' | 'parking' | 'erf' | 'floor';
};

export type ListingFlyerFact = {
  key: string;
  label: string;
  value: string;
};

export type ListingFlyerData = {
  title: string;
  reference: string;
  price: string;
  statusLabel: string;
  ...
  metrics: ListingFlyerMetric[];
  facts: ListingFlyerFact[];
  disclaimer?: string;
  associate: {
    name: string;
    phone: string;
    ...
    marketCenterLogoUrl: string | null;
  };
};
...
const ListingFlyerPreview = forwardRef<HTMLDivElement, ListingFlyerPreviewProps>(function ListingFlyerPreview(
  ...
  const [descriptionLineClamp, setDescriptionLineClamp] = useState(14);
  ...
  return (
    ...
    >
      <div className="relative flex h-full flex-col bg-[#f4f4f4]">
        <section className="relative h-[345px] overflow-hidden bg-slate-200">
          {activeHeroImageUrl ? (
            <img
        ...
        <section className="grid grid-cols-3 divide-x divide-slate-300 border-b border-slate-300 bg-slate-300">
          {gallery.length > 0 ? gallery.map((url, idx) => (
            <div key={`${url}-${idx}`} className="bg-white p-2">
              <div className="h-[108px] overflow-hidden rounded border border-slate-200 bg-slate-100">
                <img src={url} alt={`Gallery ${idx + 2}`} className="h-full w-full object-cover" />
              </div>
            </div>
          )) : (
            <div className="col-span-3 flex h-[124px] items-center justify-center bg-white text-[10px] text-slate-500">No additional images</div>
          )}
        </section>

        {data.metrics.length > 0 ? (
          <section className="border-b border-slate-300 bg-[#3f3f46] text-white">
            <div className="grid grid-cols-3">
              {Array.from({ length: 3 }).map((_, idx) => {
                const metric = statTiles[idx];
                return (
                  <div key={metric?.key ?? `top-empty-${idx}`} className={`flex items-center justify-center gap-2 px-2 py-2.5 ${idx > 0 ? 'border-l border-slate-300' : ''}`}>
                    {metric ? (
                      <>
                        <FlyerMetricIcon kind={metric.icon} />
                        <span className="text-[11px] font-semibold tracking-[0.03em]">{metric.value} {metric.label}</span>
                      </>
                    ) : null}
                  </div>
                );
            ...
            <div className="grid grid-cols-3 border-t border-slate-300">
              {Array.from({ length: 2 }).map((_, idx) => {
                const metric = statTiles[idx + 3];
                return (
                  <div key={metric?.key ?? `bottom-empty-${idx}`} className={`flex items-center justify-center gap-2 px-2 py-2.5 ${idx > 0 ? 'border-l border-slate-300' : ''}`}>
                    {metric ? (
                      <>
                        <FlyerMetricIcon kind={metric.icon} />
                        <span className="text-[11px] font-semibold tracking-[0.03em]">{metric.value} {metric.label}</span>
              ...
              <div className="flex items-center justify-center border-l border-slate-300 px-2 py-2.5">
                <span className="text-[11px] font-semibold tracking-[0.03em]">Ref: {data.reference}</span>
              </div>
            </div>
          </section>
        ) : null}

        <section className="grid min-h-0 flex-1 grid-cols-[1.55fr_0.95fr] items-stretch gap-3 bg-white px-5 py-4">
          <article ref={descriptionArticleRef} className="h-full min-h-0 rounded-lg border border-slate-200 bg-white p-3">
          ...
          <aside className="grid h-full min-h-0 grid-rows-[auto_1fr] gap-3">
            ...
            <section className="flex h-full min-h-0 flex-col rounded-lg border border-slate-200 bg-[#fafafa] p-3">
...
export default ListingFlyerPreview;